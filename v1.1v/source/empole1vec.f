...
c
      ...
      real*8 xdfield,ydfield,zdfield
      real*8 time0,time1
...
!DIR$ ATTRIBUTES ALIGN:64:: demvecx
      real*8 demvecx(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: demvecy
      real*8 demvecy(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: demvecz
      real*8 demvecz(nblocloop)
c
...
!DIR$ ASSUME (mod(npolelocloop,16).eq.0)
            do k  = 1, npolelocloop
               if(k.le.npoleloc) then
                  demvecx(k) = 2.0d0 * term * civec(k) * xd
                  demvecy(k) = 2.0d0 * term * civec(k) * yd
                  demvecz(k) = 2.0d0 * term * civec(k) * zd
              endif
            enddo
...
!DIR$ VECTOR ALIGNED
            do k  = 1, npolelocloop
               if(k.le.npoleloc) then
                  dem(1,ivec(k)) = dem(1,ivec(k)) + demvecx(k)
                  dem(2,ivec(k)) = dem(2,ivec(k)) + demvecy(k)
                  dem(3,ivec(k)) = dem(3,ivec(k)) + demvecz(k)
               endif
            enddo
...
!DIR$ ATTRIBUTES ALIGN:64:: xvec
      real*8  xvec(maxelst,npolelocnlloop)
!DIR$ ATTRIBUTES ALIGN:64:: yvec
      real*8  yvec(maxelst,npolelocnlloop)
!DIR$ ATTRIBUTES ALIGN:64:: zvec
      real*8  zvec(maxelst,npolelocnlloop)
...
!DIR$ ATTRIBUTES ALIGN:64:: temx
      real*8 temx(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: temy
      real*8 temy(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: temz
      real*8 temz(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: demvecx
      real*8 demvecx(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: demvecy
      real*8 demvecy(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: demvecz
      ...
      if(rank.eq.20.and.tinkerdebug)write(*,*) 'emreal1cvec4'
...
c
      ...
      iipoledefault = -1                   ! Impossible value
...
!DIR$ ASSUME (mod(neloop8,8).eq.0)
         do k = 1,neloop8
            ...
            rr11vec(k) = 9.0d0 * rr9vec(k) * invr2vec(k)
         enddo
...
!DIR$ ASSUME (mod(neloop8,8).eq.0)
         do k = 1, neloop8
            ...
            rr11vec(k) = bn5vec(k) - (1.0d0 - mscalevec(k)) * rr11vec(k)
         enddo
c
...
!DIR$ SIMD
         do k = 1, neloop8
               demvecx(i) = demvecx(i) + frcvecx(k)
               demvecy(i) = demvecy(i) + frcvecy(k)
               demvecz(i) = demvecz(i) + frcvecz(k)
...
!DIR$ VECTOR ALIGNED
         do k = 1, neloop8
            if(k.le.nnelst1) then
               demvecx(kbisvec1(k)) = demvecx(kbisvec1(k)) - frcvecx(k)
               demvecy(kbisvec1(k)) = demvecy(kbisvec1(k)) - frcvecy(k)
               demvecz(kbisvec1(k)) = demvecz(kbisvec1(k)) - frcvecz(k)
            endif
         enddo
...
!DIR$ ASSUME (mod(npolelocnlloop,16).eq.0)
      do k = 1, npolelocnlloop
         if (iaxvec(k) > 0.and.iglobvec(k)>0) then
...
!DIR$ ASSUME (mod(npolelocnlloop,16).eq.0)
      do k = 1, npolelocnlloop
         if (iayvec(k) > 0.and.iglobvec(k)>0) then
...
!!DIR$ NOFUSION
      do k = 1, npolelocnlloop
         if (iazvec(k) > 0.and.iglobvec(k)>0) then
...
!DIR$ VECTOR ALIGNED
       do k = 1, nblocloop
          if(k.le.nbloc) then
            dem(1,k) = dem(1,k) + demvecx(k)
            dem(2,k) = dem(2,k) + demvecy(k)
            dem(3,k) = dem(3,k) + demvecz(k)
         endif
      enddo
      time2=mpi_wtime() - time0
      if(rank.eq.20.and.tinkertime) write(*,*) 'time emrealvec = ',
     &                 time2+time1
      return
      end
