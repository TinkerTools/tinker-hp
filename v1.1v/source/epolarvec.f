...
c
      ...
      real*8,allocatable :: divec(:,:), uivec(:,:),uiivec(:)
      !DIR$ ATTRIBUTES ALIGN:64::dvec,uvec
      real*8,allocatable :: dvec(:), uvec(:)
...
c
     ...
     &                      )
        ...
        enddo
        e = sum(uiivec(1:npoleloc)) * term * fterm / 3.0d0
        ep = ep + e
c
...
c
      ...
      real*8, allocatable :: qivec(:,:),qritmp(:),qrktmp(:)
...
cold           rr1 = f / r
cold           rr3 = rr1 / r2
cold           rr5 = 3.0d0 * rr3 / r2
cold           rr7 = 5.0d0 * rr5 / r2
         rr3vec(1:nnelst1) =          f / rvec(1:nnelst1)**3 !         1
         rr5vec(1:nnelst1) =  3.0d0 * f / rvec(1:nnelst1)**5 !     3 * 1
         rr7vec(1:nnelst1) = 15.0d0 * f / rvec(1:nnelst1)**7 ! 5 * 3 * 1
...
cold           alsq2 = 2.0d0 * aewald**2
cold           alsq2n = 0.0d0
cold           if (aewald .gt. 0.0d0)  alsq2n = 1.0d0 / (sqrtpi*aewald)
cold           exp2a = exp(-ralpha**2)
cold           do j = 1, 3
cold              bfac = dble(j+j-1)
...
c
         ...
         bnvec(1:nnelst1,1)  = (  1.0d0 * bnvec(1:nnelst1,0)
     &                         + alsq2 * alsq2n * exp2avec(1:nnelst1)
     &                         )    / r2vec1(1:nnelst1)
         bnvec(1:nnelst1,2)  = (  3.0d0 * bnvec(1:nnelst1,1)
     &                         + alsq2**2 * alsq2n * exp2avec(1:nnelst1)
     &                         )    / r2vec1(1:nnelst1)
         bnvec(1:nnelst1,3)  = (  5.0d0 * bnvec(1:nnelst1,2)
...
c
         ...
         enddo
         where (dampvec(1:nnelst1) .ne. 0.0d0)
                dampvec1 =   - pgammavec * (rvec/dampvec)**3
           where (dampvec1(:). gt.-50.0d0)
             expdampvec1 = exp(dampvec1)
             sc3vec =   1.0d0 - expdampvec1
             sc5vec =   1.0d0 - (1.0d0 - dampvec1) * expdampvec1
             sc7vec =   1.0d0 - (1.0d0 - dampvec1 + 0.6d0 * dampvec1**2)
     &                         * expdampvec1
            end where
         end where
...
c
         psc3vec(1:nnelst1) =   1.0d0 
     &                       -  sc3vec          (1:nnelst1)
     &                        * pscale(kglobvec1(1:nnelst1))
         psc5vec(1:nnelst1) =   1.0d0
     &                       -  sc5vec          (1:nnelst1)
     &                        * pscale(kglobvec1(1:nnelst1))
         psc7vec(1:nnelst1) =   1.0d0
     &                       -  sc7vec          (1:nnelst1)
     &                        * pscale(kglobvec1(1:nnelst1))
...
c
     ...
     &                        - ci               * urkvec(1:nnelst1)
     &                        + duikvec(1:nnelst1)
         term2vec(1:nnelst1) =  2.0d0             * quikvec(1:nnelst1)
...
c
      ...
      real*8,allocatable :: fuindvec(:,:),iipolevec(:),a(:,:),nfft(:,:)
      !DIR$ ATTRIBUTES ALIGN:64::evec,fphirecvec,fuindvec2
      real*8,allocatable :: evec(:,:),fphirecvec(:,:),fuindvec2(:,:)
