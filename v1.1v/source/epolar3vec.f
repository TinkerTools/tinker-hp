...
!DIR$ NOFUSION
     ...
     &                    + divecz(k) * uivecz(k)
            ...
            ep = ep + uiivec(k) * term * fterm / 3.0d0
         enddo
...
!DIR$ ASSUME (mod(neloop8,8).eq.0)
         do k = 1,neloop8
               ...
               sc5vec(k) = 1.0d0 -  (1.0d0 - dampvec1(k))
     &                            * expdampvec1(k)
...
!DIR$ SIMD
         ...
         do k = 1, neloop8
            psr3vec(k) =  bn1vec(k)
     &                  - rr3vec(k) * (1.0d0 - sc3vec(k) * pscalevec(k))
            psr5vec(k) =  bn2vec(k)
     &                  - rr5vec(k) * (1.0d0 - sc5vec(k) * pscalevec(k))
            psr7vec(k) =  bn3vec(k)
     &                  - rr7vec(k) * (1.0d0 - sc7vec(k) * pscalevec(k))
         enddo
c
...
!DIR$ VECTOR ALIGNED
     ...
     &               + term3vec(k) * psr7vec(k)
               if (pscalevec(k) /= 0.0d0) ne = ne + 1
            endif
         enddo
c
      end do
      nep = nep + ne ! moved outside of the preceeding loop
      ep  = ep + e   ! moved outside of the preceeding loop
                    ! as it prevents loop vectorization !
      time1 = MPI_WTIME()
      if(rank.eq.0.and.tinkertime) write(*,*) 'Time epolar3cvec',
     &        time1 -time0
      return
      end
