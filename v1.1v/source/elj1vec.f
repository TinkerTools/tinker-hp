...
!DIR$ ATTRIBUTES ALIGN:64:: dev1vec
      real*8  dev1vec(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: dev2vec
      real*8  dev2vec(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64:: dev3vec
      real*8  dev3vec(nblocloop)

      character*6 mode
c
 1000 format(' Warning, system moved too much since last neighbor list'
     $   ' update, try lowering nlupdate VDW')
      if(rank.eq.0.and.tinkerdebug) write(*,*) 'elj1cvec'
c
c
...
c
     ...
     &do ii = 1, nvdwlocnl
         ...
         iv   = ired(iglob)
         redi = merge (1.0d0, kred(iglob),(iglob.eq.iv))
         ivloc = loc(iv)
         it = jvdw(iglob)
         xi = xred(i)
         yi = yred(i)
...
!DIR$ VECTOR ALIGNED
         do k = 1, nvloop8
            p6vec(k) = rvvec2(k)**6 * rik2vec(k) ** (- three )
            evec(k)  = epsvec2(k) * p6vec(k) * (p6vec (k) - 2.0d0)
     &                * vscalevec(k)
            devec(k) = - 12.0d0 * epsvec2(k) * p6vec(k)
     &                                       * (p6vec(k) - 1.0d0)
     &                                       * invrikvec(k)
     &                * vscalevec(k)
...
!DIR$ ASSUME (MOD(nvloop8,8).eq.0)
     ...
     &                      + c1 * rikvec(k) + c0           
               dtapervec(k)  =  5.0d0 * c5 * rik4vec(k)
     &                        + 4.0d0 * c4 * rik3vec(k)
     &                        + 3.0d0 * c3 * rik2vec(k)
     &                        + 2.0d0 * c2 * rikvec(k)
     &                        +         c1
...
!DIR$ VECTOR ALIGNED
         do k = 1, nvloop8
           ...
           dev1vec(ivloc) = dev1vec(ivloc) + (1.0d0 - redi) * dedxvec(k)
           dev2vec(ivloc) = dev2vec(ivloc) + (1.0d0 - redi) * dedyvec(k)
           dev3vec(ivloc) = dev3vec(ivloc) + (1.0d0 - redi) * dedzvec(k)
         enddo
      end do MAINLOOP
c
