...
!             dep1(j) = dep1(j) + buffer(1,j,i)
!             dep2(j) = dep2(j) + buffer(2,j,i)
!             dep3(j) = dep3(j) + buffer(3,j,i)
            end do
          else
            do j = 1, nloc
              iglob = glob(j)
...
!             dep1(j) =dep1(j)+deprec(1,ilocrec)
!             dep2(j) =dep2(j)+deprec(2,ilocrec)
!             dep3(j) =dep3(j)+deprec(3,ilocrec)
...
!     buffers(1,(nloc+1):nbloc) = dep1((nloc+1):nbloc)
!     buffers(2,(nloc+1):nbloc) = dep2((nloc+1):nbloc)
!     buffers(3,(nloc+1):nbloc) = dep3((nloc+1):nbloc)
c
      do i = 1, nbig_recep
        tag = nproc*pbig_recep(i) + rank + 1
        call MPI_ISEND(buffers(1,bufbeg(pbig_recep(i)+1)),
