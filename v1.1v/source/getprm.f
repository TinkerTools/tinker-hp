...
c     Try to read $TINKER_DEBUG from environment
c     get_environment_variable is fortran intrinsic
      call get_environment_variable ('TINKER_DEBUG', debugval,length)
c     if lenght is 0, the TINKER_DEBUG variable is not set
      if (length.ne.0) tinkerdebug = .true.
c     Try to read $TINKER_TIME from environment
c     get_environment_variable is fortran intrinsic
      call get_environment_variable ('TINKER_TIME', timeval,length)
c     if lenght is 0, the TINKER_TIME variable is not set
      if (length.ne.0) tinkertime = .true.
c
