...
c     ############################################################################################
c
c     nproctot  total number of MPI process (on all the beads)
c     ranktot  total rank of the MPI process within MPI_COMM_WORLD 
c     COMM_BEAD local bead mpi communicator
c
c     nproc     local bead number of MPI processes
c     rank      rank of the current MPI process within COMM_BEAD
...
c
c
c     lplen : number of lone pairs in each process
c     bufbeglp : index of the first lp concerned by each process
c     acceptlen : number of acceptors in each process
c     bufbegacc : index of the first atom concerned by each process
c
      module domdec
      implicit none
      integer nproctot,ranktot,COMM_BEAD
      ...
      integer, allocatable :: lplen(:),acceptlen(:)
      integer, allocatable :: bufbeglp(:),bufbegacc(:)
