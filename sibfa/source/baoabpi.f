...
c
      subroutine aoapi(istep,dt,ekvir,presvir)
      use atoms
      ...
      integer i,j,k,tag,status(MPI_STATUS_SIZE),ierr,iproc, ibead,l
      integer :: modstep
      !real*8, allocatable :: eigmat(:,:),eigmattr(:,:),omkpi(:),WORK(:)
      real*8 :: eigx0,eigv0
      real*8 :: dt2, a1,a2, sqrtmass
      real*8, allocatable :: pos_full(:,:,:),vel_full(:,:,:)!, centroid(:,:)
      real*8, allocatable :: forces_full(:,:,:)
      real*8 :: normal
      !real*8 :: hbar
      real*8 :: buffer_energy(3)
      real*8 :: ekprim,ekvir,presvir,dedv_mean
      real*8 :: mpitime1, mpitime2
      real*8 :: time_tot, time_com
      real*8 :: acentroid,a1p,a2p,scale,third
      real*8 :: dens
...
c
      ...
      if (ranktot.eq.0) then
        do i = 1, nproctot-1
          tag = nproctot*ranktot + i + 1
          call MPI_IRECV(nbeadscomm(i+1),1,
     $     MPI_INT,i,tag,MPI_COMM_WORLD,reqrec(i),ierr)
          call MPI_WAIT(reqrec(i),status,ierr)
c          write(*,*) 'nbeads of ',i,' = ',nbeadscomm(i+1)
        end do
      else
        tag = ranktot + 1
        call MPI_ISEND(nbeadsloc,1,MPI_INT,0,tag,MPI_COMM_WORLD,
     $   reqsend(1),ierr)
...
c
        call MPI_ISEND(nlocpi(1),1,MPI_INT,0,tag,MPI_COMM_WORLD,
     $   reqsend(1),ierr)
        call MPI_WAIT(reqsend(1),status,ierr)
      end if
c
c     get their indexes and positions
c
      if (ranktot.eq.0) then
     ...
     $     MPI_REAL8,i,tag,MPI_COMM_WORLD,reqrec(i),ierr)
          call MPI_WAIT(reqrec(i),status,ierr)
...
c
        ...
        call MPI_ISEND(buffer,10*nlocpi(1)*nbeadsloc,MPI_REAL8,0,tag,
...
!        deallocate(indexposcomm)
        ...
        call compute_observables_pi(pos_full,forces_full
...
c
     ...
     $     nbeadscomm(i+1),MPI_REAL8,i,tag,MPI_COMM_WORLD,reqsend(i),
     $     ierr)
...
c     the proc having more than 1 beads have necessarily the whole system for all these beads
c
        call MPI_IRECV(buffer,10*nlocpi(1)*nbeadsloc,MPI_REAL8,0,tag,
...
c            write(*,*) 'j = ',glob(j),'k = ',k,'x = ',pospi(1,glob(j),k)
          end do
        end do
      end if
c
