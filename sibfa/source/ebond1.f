...
c                 dt2 = dt * dt
c                 e = bndunit * force * dt2 * (1.0d0+cbnd*dt+qbnd*dt2)
c                 deddt = 2.0d0 * bndunit * force * dt
c     &                      * (1.0d0+1.5d0*cbnd*dt+2.0d0*qbnd*dt2)
c                 dt =  force*dt
                  e = 0d0
                  rab = 0d0
               else
c               write(*,*) 'gogo',force,ideal,xab,x(ia),x(ib),yab,zab
                 dt =  force*dt
                 e  =  dt*dt - dt*dt*dt + dble(7d0/12d0)*dt*dt*dt*dt
                 e  =  bndunit*Dr*e
                 deddt = 2*force*dt-3*force*dt*dt + dble(7d0/12d0)*4*
     $             force*dt*dt*dt
                 deddt = bndunit*Dr*deddt
               end if
            end if
c
