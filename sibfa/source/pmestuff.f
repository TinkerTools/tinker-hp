...
c
      do i = 1, nrecdir_recep
        if (precdir_recep(i).ne.rank) then
          tagmpi = nproc*rank + precdir_recep(i) + 1
          call MPI_IRECV(repartrec(bufbeg(precdir_recep(i)+1)),
     $     domlen(precdir_recep(i)+1),MPI_INT,precdir_recep(i),tagmpi,
...
c
      do i = 1, nrecdir_send
        if (precdir_send(i).ne.rank) then
          tagmpi = nproc*precdir_send(i) + rank + 1
          call MPI_ISEND(repartrec,
     $    nloc,MPI_INT,precdir_send(i),tagmpi,COMM_BEAD,
     $    req(tagmpi),ierr)
        end if
      end do
c
