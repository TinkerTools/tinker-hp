...
c
      ...
      real*8, allocatable :: nderep(:,:)
      real*8, allocatable :: ndexdisp(:,:)
...
c         else
            write (iout,180)
c  180       format (/,'  Energy',7x,'EB',14x,'EA',14x,'EBA',13x,'EUB',
c     &              11x,'EX',
c     &              /,'  Terms',8x,'EAA',13x,'EOPB',12x,'EOPD',
c     &                 12x,'EID',11x,'EC',
c     &              /,15x,'EIT',13x,'ET',14x,'EPT',13x,'EBT',
c     &              11x,'EREP',/,15x,'ETT',13x,'EV',14x,'EM',14x,'EP',
c     &               12x,'EXDISP')
c            write (iout,190)  eb,ea,eba,eub,ex,eaa,eopb,eopd,eid,ec,
c     &                        eit,et,ept,ebt,0d0,ett,ev,em,ep,0d0
c  190       format (/,6x,5f15.4,/,6x,5f15.4,/,6x,5f15.4,/,6x,5f15.4)
c         end if
  180       format (/,'  Energy',7x,'EB',14x,'EA',14x,'EBA',13x,'EUB',
     ...
     &               12x,'EXDISP')
            write (iout,190)  eb,ea,eba,eub,ec,eaa,eopb,eopd,eid,ect,
     &                        eit,et,ept,ebt,erep,ett,ev,em,ep,exdisp
...
c
  ...
  200    format (/,' Cartesian Gradient Breakdown by Individual',
     &                 ' Components :')
         if (digits .ge. 8) then
c            write (iout,210)
c  210       format (/,2x,'Atom',9x,'d EB',11x,'d EA',11x,'d EBA',
c     &                 10x,'d EUB',8x,'d EX',
c     &              /,2x,'Axis',9x,'d EAA',9x,'d EOPB',10x,'d EOPD',
c     &                 9x,'d EID',8x,'d EC',
c     &              /,2x,'Type',9x,'d EIT',10x,'d ET',11x,'d EPT',
c     &                 10x,'d EBT',8x,'d EREP',
c     &              /,15x,'d ETT',10x,'d EV',11x,'d EM',11x,'d EP',9x,
c     &               'd EXDISP')
  210       format (/,2x,'Atom',9x,'d EB',11x,'d EA',11x,'d EBA',
     ...
     &               'd EXDISP')
            ...
            write (iout,220)
c  220       format (/,2x,'Atom',9x,'d EB',11x,'d EA',11x,'d EBA',
c     &                 10x,'d EUB',8x,' d EX',
c     &              /,2x,'Axis',9x,'d EAA',9x,'d EOPB',10x,'d EOPD',
c     &                 9x,'d EID',8x,'d EC',
c     &              /,2x,'Type',9x,'d EIT',10x,'d ET',11x,'d EPT',
...
c     &                'd EXDISP')
  220       format (/,2x,'Atom',9x,'d EB',11x,'d EA',11x,'d EBA',
     ...
     &                'd EXDISP')
            ...
            write (iout,230)
c  230       format (/,2x,'Atom',9x,'d EB',11x,'d EA',11x,'d EBA',
c     &                 10x,'d EUB',8x,'d EX',
c     &              /,2x,'Axis',9x,'d EAA',10x,'d EOPB',9x,'d EOPD',
c     &                 9x,'d EID',8x,'d EC',
c     &              /,2x,'Type',9x,'d EIT',10x,'d ET',11x,'d EPT',
c     &                 10x,'d EBT',8x,'d EREP',
c     &              /,15x,'d ETT',10x,'d EV',11x,'d EM',11x,'d EP',9x,
...
c
      ...
      allocate (nderep(3,nloc))
      allocate (ndexdisp(3,nloc))
...
c
               if (repart(i).eq.rank) then
                 ...
                 nderep(j,iloc) = (erep - erep0) / eps
                 ndexdisp(j,iloc) = (exdisp - exdisp0) / eps
...
c                  else
c                     write (iout,260) i,deb(j,iloc),dea(j,iloc),
c     &                        deba(j,iloc),deub(j,iloc),dex(j,iloc),
c     &                        axis(j),
c     &                        deaa(j,iloc),deopb(j,iloc),deopd(j,iloc),
c     &                        deid(j,iloc),dec(j,iloc),deit(j,iloc),
c     &                        det(j,iloc),
c     &                        dept(j,iloc),debt(j,iloc),0d0,
c     &                        dett(j,iloc),dev(j,iloc),dem(j,iloc),
c     &                        dep(j,iloc),0d0
                     write (iout,260) i,deb(j,iloc),dea(j,iloc),
     &                        deba(j,iloc),deub(j,iloc),dec(j,iloc),
     &                        axis(j),
     &                        deaa(j,iloc),deopb(j,iloc),deopd(j,iloc),
     &                        deid(j,iloc),dect(j,iloc),deit(j,iloc),
     &                        det(j,iloc),
     &                        dept(j,iloc),debt(j,iloc),derep(j,iloc),
     &                        dett(j,iloc),dev(j,iloc),dem(j,iloc),
     &                        dep(j,iloc),dexdisp(j,iloc)
  260                format (/,i6,5f15.4,/,5x,a1,5f15.4,/,' Anlyt',
     &                          5f15.4,/,6x,5f15.4)
c  260                format (/,i6,5f15.4,/,5x,a1,5f15.4,/,' Anlyt',
c     &                          5f15.4,/,6x,5f15.4)
c                  end if
               end if
c
...
c     &                          5f14.6,/,6x,5f14.6,/,6x,4f14.6)
c                  else
c                     write (iout,290) i,ndeb(j,iloc),ndea(j,iloc),
c     &                           ndeba(j,iloc),ndeub(j,iloc),
c     &                           ndex(j,iloc),axis(j),
c     &                           ndeaa(j,iloc),ndeopb(j,iloc),
c     &                           ndeopd(j,iloc),ndeid(j,iloc),
c     &                           ndec(j,iloc),ndeit(j,iloc),
c     &                           ndet(j,iloc),
c     &                           ndept(j,iloc),ndebt(j,iloc),
c     &                           0d0,ndett(j,iloc),
c     &                           ndev(j,iloc),ndem(j,iloc),ndep(j,iloc),
c     &                           0d0
