MODULE deconvolution
  ...
  ABSTRACT INTERFACE
    FUNCTION convolution_kernel(omega,omega0,gamma,kT)
      IMPLICIT NONE
      REAL*8, intent(in) :: omega,omega0,gamma,kT
      REAL*8 :: convolution_kernel
...
SUBROUTINE deconvolute_spectrum_symmetrize(s_in,domega,nIterations, &
  ...
  REAL*8,INTENT(in) :: s_in(:),domega,kT,gamma,thr
  INTEGER, INTENT(in) :: nIterations
  REAL*8, INTENT(inout) :: s_out(:)
  LOGICAL, INTENT(in) :: trans
  REAL*8, INTENT(inout), optional :: s_rec(:)
  LOGICAL, INTENT(in), optional :: verbose
  PROCEDURE(convolution_kernel):: kernel
  integer :: nom,k
  REAL*8, allocatable :: s_in_sym(:),omsym(:),s_out_sym(:)
  REAL*8, allocatable :: s_rec_sym(:)
...
SUBROUTINE deconvolute_spectrum(s_in,omega,nIterations, &
                ...
                verbose)
  IMPLICIT NONE
  REAL*8,INTENT(in) :: s_in(:),omega(:),kT,gamma,thr
  INTEGER, INTENT(in) :: nIterations
  REAL*8, INTENT(inout) :: s_out(:)
  REAL*8, INTENT(inout), optional :: s_rec(:)
  LOGICAL, INTENT(in) :: trans
  ...
  REAL*8, ALLOCATABLE, SAVE :: D(:,:),K(:,:)
  REAL*8, ALLOCATABLE :: h(:),s_next(:),Ktr(:,:)
  REAL*8, ALLOCATABLE, SAVE :: K_norm(:,:),omega_norm(:)  
  REAL*8 :: domega,om,om0,den,rn,ln,diff
  LOGICAL :: verb,compute_kernel
  LOGICAL, save :: trans_save=.FALSE.
  real*8, save :: gamma_save=-1.d0
...
SUBROUTINE convolution(kernel,s_in,domega,s_out,op)
  IMPLICIT NONE
  REAL*8, INTENT(in) :: kernel(:,:),s_in(:),domega
  REAL*8, INTENT(inout) :: s_out(:)
...
SUBROUTINE compute_rn_ln(s0,sn,K,domega,rn,ln)
  IMPLICIT NONE
  REAL*8, INTENT(in) :: s0(:),sn(:),K(:,:),domega
  REAL*8, INTENT(out) :: rn,ln
