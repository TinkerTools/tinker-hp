...
c
      subroutine netcdfamber(istep,dt)
      use atoms
      use atmtyp
      use bound
      use boxes
      ...
      integer counttab1(2),counttime(1)
      logical init,exist
...
c
c        status = nf90_create(netcdffile,nf90_clobber,ncid)
        status = nf90_create_par(netcdffile,nf90_clobber,
     $    COMM_TINKER,info,ncid)
c        status = nf90_create(netcdffile,IOR(nf90_clobber,nf90_netcdf4),
c     $      ncid,COMM_TINKER_MPI_INFO_NULL)
c
...
c
c        status = nf90_open(netcdffile,nf90_write,ncid)
        status = nf90_open_par(netcdffile,nf90_write,COMM_TINKER,
     $   info,ncid)
      endif
c
