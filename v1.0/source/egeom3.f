...
c            if (use_bounds .and. intermol)  call image (xr,yr,zr)
            if (use_bounds)  call image (xr,yr,zr)
            r = sqrt(xr*xr + yr*yr + zr*zr)
            force = dfix(1,i)
            df1 = dfix(2,i)
...
c
     ...
     &                              use(ic) .or. use(id))
         if (proceed) then
            ...
            aeg(ibloc) = aeg(ibloc) + 0.25d0*e
            aeg(icloc) = aeg(icloc) + 0.25d0*e
            aeg(idloc) = aeg(idloc) + 0.25d0*e
            huge = (e .gt. 10.0d0)
            if (debug .or. (verbose.and.huge)) then
               if (header) then
                  header = .false.
      ...
      if (use_basin) then
         ...
         do i = 1, nbloc
            ...
            do k = 1, nbloc
               ...
               proceed = .true.
               if (use_group)  
     $            call groups (proceed,fgrp,iglob,kglob,0,0,0,0)
...
c
      if (use_wall) then
         ...
         do i = 1, nloc
            iglob = glob(i)
            proceed = .true.
            if (use_group)  call groups (proceed,fgrp,iglob,0,0,0,0,0)
