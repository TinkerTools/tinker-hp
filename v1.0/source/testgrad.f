...
c
      ...
      call nblist(0)
c
      if (.not.associated(demrec)) allocate (demrec(3,nblocrec))
      if (.not.associated(deprec)) allocate (deprec(3,nblocrec))
      torquerec = 0d0
      torquedir = 0d0
      torquerecp = 0d0
      torquedirp = 0d0
...
c
         call reduceen(etot)
c
c     add torques to forces
c
         do i = 1, nloc
           iglob = glob(i)
           do j = 1, 3
             dem(j,i) = dem(j,i) + torquedir(j,i)
             dep(j,i) = dep(j,i) + torquedirp(j,i)
           end do
         end do
      end if
c
...
c         else
  ...
  180       format (/,'  Energy',7x,'EB',14x,'EA',14x,'EBA',13x,'EUB',
     ...
     &               12x,'EXDISP')
            write (iout,190)  eb,ea,eba,eub,eaa,eopb,eopd,eid,0d0,
     &                        eit,et,ept,ebt,0d0,ett,ev,em,ep,0d0
  190       format (/,6x,4f15.4,/,6x,5f15.4,/,6x,5f15.4,/,6x,5f15.4)
c         end if
      end if
c
...
c                  else
     ...
     &                           ndeba(j,iloc),ndeub(j,iloc),axis(j),
  ...
  290                format (/,i6,4f15.4,/,5x,a1,5f15.4,/,' Numer',
     &                          5f15.4,/,6x,5f15.4)
c                  end if
               end if
            end do
