...
c
      ...
      integer mode,next
      real*8 dt,dtdump,wtime0,wtime1,omp_get_wtime
      logical exist,query
...
c$    wtime0 = omp_get_wtime()
      do istep = 1, nstep
         ...
         if ((mod(istep,iprint).eq.0).and.(rank.eq.0)) then
c$         wtime1 = omp_get_wtime()
           write(6,1000) wtime1-wtime0, (wtime1-wtime0)/dble(iprint)
           write(6,1010) 86400*dt*dble(iprint)/(1000*(wtime1-wtime0))
           wtime0 = wtime1
         end if
      end do
c
