...
c
     ...
     $     MPI_INT,iproc,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
        end do
        do iproc = 1, nproc-1
...
c
     ...
     $     3*domlen(iproc+1),
     $     MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
        end do
      else
        tagmpi = rank + 1
     ...
     $     MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
        ...
        tagmpi = rank + 1
        call MPI_ISEND(vtemp,3*nloc,MPI_REAL8,0,tagmpi,MPI_COMM_WORLD,
     $  req(tagmpi),ierr)
      end if
      if (rank.eq.0) then
        do iproc = 1, nproc-1
     ...
     $     MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
        ...
        tagmpi = rank + 1
        call MPI_ISEND(atemp,3*nloc,MPI_REAL8,0,tagmpi,MPI_COMM_WORLD,
     $  req(tagmpi),ierr)
      end if
      if (rank.eq.0) then
        do iproc = 1, nproc-1
...
c
c       put the system back in the unit cell before writing frame
c
...
c
      if (rank.eq.0) then
        do iproc = 1, nproc-1
          tagmpi = iproc + 1
          call MPI_ISEND(x,n,MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,
     $    req(tagmpi),ierr)
        end do
      else
        tagmpi = rank + 1
          call MPI_IRECV(x,n,
     $     MPI_REAL8,0,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
      end if
      if (rank.eq.0) then
        do iproc = 1, nproc-1
          tagmpi = iproc + 1
      ...
      if (rank.eq.0) then
        do iproc = 1, nproc-1
          tagmpi = iproc + 1
          call MPI_ISEND(y,n,MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,
     $    req(tagmpi),ierr)
        end do
      else
        tagmpi = rank + 1
     ...
     $     MPI_REAL8,0,tagmpi,MPI_COMM_WORLD,req(tagmpi),ierr)
      ...
      if (rank.eq.0) then
        do iproc = 1, nproc-1
          tagmpi = iproc + 1
          call MPI_ISEND(z,n,MPI_REAL8,iproc,tagmpi,MPI_COMM_WORLD,
     $    req(tagmpi),ierr)
        end do
      else
        tagmpi = rank + 1
