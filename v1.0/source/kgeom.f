...
c
        if (associated(xpfix)) deallocate (xpfix)
        allocate (xpfix(n))
        if (associated(ypfix)) deallocate (ypfix)
        allocate (ypfix(n))
        if (associated(zpfix)) deallocate (zpfix)
        allocate (zpfix(n))
        if (associated(pfix)) deallocate (pfix)
        allocate (pfix(2,n))
        if (associated(dfix)) deallocate (dfix)
        allocate (dfix(3,n))
        if (associated(afix)) deallocate (afix)
        allocate (afix(3,n))
        if (associated(tfix)) deallocate (tfix)
        allocate (tfix(3,n))
        if (associated(gfix)) deallocate (gfix)
        allocate (gfix(3,n))
        if (associated(chir)) deallocate (chir)
        allocate (chir(3,n))
        if (associated(ipfix)) deallocate (ipfix)
        allocate (ipfix(n))
        if (associated(kpfix)) deallocate (kpfix)
        allocate (kpfix(3,n))
        if (associated(idfix)) deallocate (idfix)
        allocate (idfix(2,n))
        if (associated(iafix)) deallocate (iafix)
        allocate (iafix(3,n))
        if (associated(itfix)) deallocate (itfix)
        allocate (itfix(4,n))
        if (associated(igfix)) deallocate (igfix)
        allocate (igfix(2,n))
        if (associated(ichir)) deallocate (ichir)
        allocate (ichir(4,n))
c
...
c
      if (associated(npfixglob)) deallocate(npfixglob)
      allocate (npfixglob(nloc))
      if (associated(ndfixglob)) deallocate(ndfixglob)
      allocate (ndfixglob(nloc))
      if (associated(nafixglob)) deallocate(nafixglob)
      allocate (nafixglob(nloc))
      if (associated(ntfixglob)) deallocate(ntfixglob)
      allocate (ntfixglob(nloc))
      if (associated(ngfixglob)) deallocate(ngfixglob)
      allocate (ngfixglob(nloc))
      if (associated(nchirglob)) deallocate(nchirglob)
...
c
      ...
      do i = 1, ngfix
        ia = igfix(1,i)
        sizegroup = igrp(2,ib)-igrp(1,ib)+1
