...
!
!> @brief 
!> assigns the parameters to be used in computing the
!> van der Waals interactions and processes any new or changed
!> values for these parameters
!> @param no params
subroutine kvdw
...
!   end if
!
!     move zero-sized atoms from the list of local vdw sites
!
   nvdw = 0
   do i = 1, n
...
!
!     subroutine kvdw_update: update local vdw
!
!> @brief 
!> update local vdw parameters
!> @param no params
subroutine kvdw_update(istep)
   use atmlst
   use atmtyp
   use atoms
   use couple
...
!
!     subroutine dealloc_shared_vdw : deallocate shared memory pointers for vdw
!     parameter arrays
!
!> @brief 
!> deallocate shared memory pointers for vdw
!> parameter arrays
!> @param no params
...
!
!> @brief 
!> allocate shared memory pointers for vdw
!> parameter arrays
!> @param no params
