...
!
!> @brief 
!> limited memory BFGS quasi-newton nonlinear
!> optimization routine
!> @param[in] n: number of parameters in the objective function
!> @param[in] x0: starting point upon input, upon return best point found
!> @param[in] minimum: during optimization contains best current function value; returns final best function value
!> @param[in] grdmin: normal exit if rms gradient gets below this value
!> @param[in] ncalls: total number of function/gradient evaluations
!> @param[in] fgvalue: function to evaluate function and gradient values
!> @param[in] optsave: subroutine to write out info about current status
...
!
!     print header information about the optimization method
!
