...
!
!> @brief 
!> calculates the induced dipole polarization energy
!> and derivatives with respect to Cartesian coordinates
!> @param no params
subroutine epolar1
   use inform
...
!
!> @brief 
!> calculates the dipole polarization energy and
!> derivatives with respect to Cartesian coordinates using
!> particle mesh Ewald summation and a neighbor list
!> @param no params
subroutine epolar1c
...
!
!     allocate derivative of permanent direct field wrt to lambda
!
...
!
   ...
   if (use_lambdadyn) then
!
!       get diagonal contribution to delambdae
!
      do i = 1, npoleloc
         iipole = poleglob(i)
         iglob = ipole(iipole)
...
!
!       get contribution of the derivative of the permanent field to delambdae
!
      do i = 1, npoleloc
         iipole = poleglob(i)
         do j = 1, 3
...
!
!     get group polarization if necessary
!
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine epreal1c  --  Ewald real space derivs via list  ##
!     ##                                                             ##
!     #################################################################
!
!
...
!     compute the short, or full real space part of the summation
...
!
   ...
   allocate (decfz(nbloc))
!
!     set exclusion coefficients and arrays to store fields
!
   pscale = 1.0d0
   dscale = 1.0d0
   uscale = 1.0d0
...
!
!     get permanent field to get polarization energy
!
            if (use_thole) then
...
!
!     reset Thole values when alternate direct damping is used
!
            if (use_tholed) then
               sc3 = 1.0d0
               sc5 = 1.0d0
