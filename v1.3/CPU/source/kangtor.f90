...
!     ##################################################################
!     ##                                                              ##
!     ##  subroutine kangtor  --  angle-torsion parameter assignment  ##
!     ##                                                              ##
!     ##################################################################
!
!
...
!
!> @brief 
!> assigns parameters for angle-torsion interactions
!> and processes new or changed parameter values
!> @param no params
subroutine kangtor
   use angtor
...
!
!
!     process keywords containing angle-torsion parameters
!
   blank = '                '
   zeros = '0000'
   header = .true.
...
!
!     assign the angle-torsion parameters for each torsion
!
   nangtor = 0
   if (nat .ne. 0) then
      do i = 1, ntors
...
!
!     turn off the angle-torsion potential if it is not used
!
   if (nangtor .eq. 0)  use_angtor = .false.
   return
end
!
!     subroutine kangtor_update: local update of angtor
!
!> @brief 
