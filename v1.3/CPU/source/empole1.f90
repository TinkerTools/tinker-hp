...
!
!> @brief 
!> driver for calculation of the multipole and dipole polarization
!> energy and derivatives with respect to Cartesian coordinates
!> @param no params
subroutine empole1
   use domdec
...
!
!> @brief 
!> calculates the multipole energy and derivatives
!> with respect to Cartesian coordinates using particle mesh Ewald
!> summation and a neighbor list
!> @param no params
subroutine empole1c
...
!
!     compute self contribution to lambda derivative for lambda dynamics
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine emrecip1  --  PME recip multipole energy & derivs  ##
!     ##                                                                ##
!     ####################################################################
!
!
...
!
!> @brief 
!> evaluates the reciprocal space portion of the particle
!> mesh Ewald summation energy and gradient due to multipoles
!> @param no params
subroutine emrecip1
   use atmlst
...
!
!     communicate reciprocal potential to get local values
!
...
!
!     compute recip contribution to lambda derivative for lambda dynamics
!
   if (use_lambdadyn) then
      do i = 1, npolerecloc
         iipole = polerecglob(i)
...
!
!     compute the short, long, or full real space part of the summation
...
!
   ...
   allocate (decfz(nbloc))
!
!     initialize connected atom scaling and torque arrays
!
   mscale = 1.0d0
   tem = 0.0d0
   pot = 0d0
...
!
!     fix the s and ds factors, depending on the range
!
            if(shortrange) then
               facts  =         s
               factds =      - ds
...
!
   do ii = 1, npolelocnl
      ...
      ixloc = loc(ix)
      iyloc = loc(iy)
      if ((izloc.lt.0).or.(izloc.gt.nbloc)) cycle
      if ((ixloc.lt.0).or.(ixloc.gt.nbloc)) cycle
      if ((iyloc.lt.0).or.(iyloc.gt.nbloc)) cycle
