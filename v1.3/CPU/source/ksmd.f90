...
!     ###############################################################
!     ##                                                           ##
!     ##  subroutine ksmd  --  initialize smd procedure            ##
!     ##                                                           ##
!     ###############################################################
!
!
!     "ksmd" checks the smd activation and assign the necessary
!     parameters enters in the key file
!
!
!> @brief 
!> checks the smd activation and assign the necessary
!> parameters enters in the key file
!> @param no params
...
!
!
!     Check if initialization or not (LOOP1)
!
   if (init) then
!
...
!######################################################################
!
!     Check the smd key activation for constant velocity SMD
!
      do i = 1, nkey
         next = 1
         record = keyline(i)
...
!
!     Check the smd key activation for constant force SMD
!
      do i = 1, nkey
         next = 1
         record = keyline(i)
...
!######################################################################
!
!     Assign the desired parameters for the SMD (LOOP2)
!
      if (use_smd_velconst .or. use_smd_forconst) then
!
!     Elastic constant asignment SMDk (own to the both SMD)
!
...
!
!    Transverse elastic constant SMDk2 (own to the both SMD)
!
         if (use_smd_velconst) then
            do i = 1, nkey
               next = 1
...
!
!    Constant velocity (only own to the constant velocity SMD)
!
         if (use_smd_velconst) then
            do i = 1, nkey
               next = 1
...
!
!    Constant force (only own to the constant force SMD)
!
...
!
!    Rescaling of the SMD velocity if integrate /= VERLET
!
               do j = 1, nkey
                  next = 1
                  record = keyline(j)
...
!
!    Number of atoms assigned to the center of mass (only to the
!    constant velocity SMD) or number of atoms assigned to the
!    constant force (only to the constant force SMD)
!
!    set defaults for the numbers and lists of SMD atoms
!
         ncsmd = -1000
         cpt1 = 1
         allocate (tcsmd(n))
...
!
!      1st case: SMD atoms are depicted one by one
!
         if (j .gt. 0) then
            do i = 1, nkey
               next = 1
...
!
!      2nd case: SMD atoms are depicted like a group: -1 100
!      => Useful to use it for a large number of SMD atoms
!
         if (j .lt. 0) then
            do i = 1, nkey
               next = 1
...
!
!    Manual parametrization of the center of mass (only for the constant
!    velocity SMD
!
         if (use_smd_velconst) then
            do i = 1, nkey
...
!
!    Direction of the steered motion (own to the both SMDs)
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!    Atom to print in a specific file during the simulation (own to the both SMDs)
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!    Assign the frequency output of the data (own to the both SMDs)
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     Need to adjust it if RESPA, RESPA1, BAOABRESPA or
!     BAOABRESPA1 is used
!
         if ((integrate .eq. 'RESPA') .or. (integrate .eq.&
...
!
!    Restart process of the SMD procedure
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!######################################################################
!
!    Check the validity of the SMD parameters
!
         quit = .false.
10       continue
! #### Check here if any problem of lectures in key file ####
         if (quit) then
            if (rank.eq.0) then
               write(ismdout,11)
...
!
! #### Check the validity of mandatory variables ####
!
! Titantron
!
...
23             format(3x,'WARNING about SMDVel:',&
               ...
               warning = .true.
! => We find here a single position restrain !
            end if
...
!
! #### Check the validity of optional variables ####
! CVSMD case
         if (use_smdk2) then
...
!
!#############################################################
!    Sum of the used parameters within the smd output file
!#############################################################
!
         if (rank.eq.0) then
            write (ismdout,500)
500         format(3x,44('*'),/,&
            ...
            &3x,44('*'),/)
!
!    Type of SMD used (CVSMD or CFSMD)
!
...
!
!    Type of integrator used (and rescaling if RESPA is used)
!
...
5074           format(6x,'RESPA1/BAOABRESPA1 factor : ',/,9x,'nalt = '&
               &,i2,/,9x,'nalt2 = ',i2)
!             write (ismdout,5075) SMDdt
! 5075        format(6x,'New SMDdt : ', f12.6)
            end if
!
!    Restart process
!
...
!
!    Periodicity of the printage in the output file
!
...
!
!    End of the init condition (LOOP2)
!
      end if
!
!    End of the init condition (LOOP1)
!
   end if
!
!######################################################################
!
!    Assignation of the corresponded atoms to the right processor
!
!######################################################################
!
...
!
!    => FROM HERE, PART WHICH IS UPLOADED EVERY STEP
...
!
!     the rank of the process that will compute the com data is sent to the master
!
      if (rank.eq.0) then
!
...
!
!#####################################################
!### Current COM calculation for the SMD procedure ###
!#####################################################
!
   if (use_smd_velconst .or. use_smd_forconst) then ! LOOP1
      cur_xcom = 0.0d0
...
!
!    => HERE IS THE END OF THE UPLOADED PROCEDURE !
!
!######################################################################
!
!    Calculation of the initial center of mass coordinates
!
!######################################################################
!
!    Initial center of mass coordinates
!
   if (init) then ! LOOP1
      if (use_smd_velconst .and. calccom) then ! LOOP2
         ...
         zcom = zcom/mtotcom
!
!    Update the coordinates within the smd outputfile
!
         if (rank.eq.0) then
            write (ismdout,525)
...
!
!    Normalisation of the steered motion vector
!
   if (init) then ! LOOP1
      if (    use_smd_velconst&
...
!
!    End of the conditional loop and the initializationn
!
      end if ! LOOP2
!
!    Initialization of the main SMD output file
!
...
!
!    Initialization of SMD output files for atoms
!
