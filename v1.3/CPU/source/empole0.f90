...
!
!
!     "empole0" calculates the electrostatic energy due to
!     atomic multipole and dipole polarizability interactions
!
!
!> @brief 
!> calculates the electrostatic energy due to
!> atomic multipole and dipole polarizability interactions
!> @param no params
subroutine empole0
   use inform
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine empole0c  --  Ewald multipole energy via list  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!
!> @brief 
!> calculates the atomic multipole interaction energy
!> using particle mesh Ewald summation and a neighbor list
!> @param no params
subroutine empole0c
   use sizes
...
!
!
!     zero out the total atomic multipole energy
!
   em = 0.0d0
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine emreal0d  --  real space mpole energy via list  ##
!     ##                                                             ##
!     #################################################################
!
!
...
!> @brief 
!> evaluates the real space portion of the Ewald sum
!> energy due to atomic multipoles using a neighbor list
!> @param no params
subroutine emreal0d
   use sizes
...
!
!     fix the s factor, depending on the range
!
            if(shortrange) then
               facts  =         s
            else if(longrange) then
