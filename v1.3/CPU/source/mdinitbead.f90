...
!     ###############################################################
!     ##                                                           ##
!     ##  subroutine mdinitbead  --  initialize a pimd trajectory  ##
!     ##                                                           ##
!     ###############################################################
!
!
!     "mdinitbead" initializes the velocities and positions
!     for a molecular pimd trajectory, including restarts
!
!
!> @brief 
!> initializes the velocities and positions
!> for a molecular pimd trajectory, including restarts
!> @param[in] polymer: current ring polymer
...
!                   if(i==1 .AND. j==1) write(0,*) ranktot,v(j,iglob)
               end do
            else
               do j = 1, 3
                  v(j,iglob) = 0.0d0
