...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine rattle  --  RATTLE distance constraint method  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!
!> @brief 
!> implements the first portion of the RATTLE algorithm
!> by correcting atomic positions and half-step velocities to
!> maintain interatomic distance and absolute spatial constraints
...
subroutine rattle (dt)
   ...
   use mpi
   implicit none
   integer i,iloc,iglob,ierr
   integer ia,ib,ialoc,ibloc
...
!
   ...
   call commrattleend
!
!    faire routine qui envoie chaque partie des contraintes convergees a tous les voisins
!
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine rattle2  --  RATTLE atom velocity corrections  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!
!> @brief 
!> implements the second portion of the RATTLE algorithm
!> by correcting the full-step velocities in order to maintain
!> interatomic distance constraints
!> @param[in] dt: timestep
...
!
!    subroutine initmpirattle : build the arrays to communicate positions of atoms
!    during the calculation of the constrains using the rattle algorithm
!
!
!> @brief 
!> build the arrays to communicate positions of atoms
!> during the calculation of the constrains using the rattle algorithm
!> @param no params
...
!
   count = 0
!
!     deal with constrains communications
!
...
!
!     subroutine commrattleinit : deal with necessary comms before rattle iterations
!
!> @brief 
!> deal with necessary comms before rattle iterations
...
!
   ...
   buffermpi2 = 0d0
!
!     Begin the reception of the constrained atoms
!
   do i = 1, nneig_recep
      proc = pneig_recep(i)
...
!
!     subroutine commrattleend : deal with necessary comms after rattle iterations
!
!> @brief 
...
!
!     subroutine commrattleinit2 : deal with necessary comms before rattle2 iterations
!
!> @brief 
...
!
!     subroutine commrattleend2 : deal with necessary comms after rattle2 iterations
!
!> @brief 
