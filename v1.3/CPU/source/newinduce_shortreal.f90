...
!> of the induced dipoles, for two input vectors mu and mu2
!> @param[in] nrhs: number of right hand side
!> @param[in] dodiag: compute diagonal contribution of matrix-vector product or not
!> @param[in] mu: first set of induced dipoles
!> @param[in] mu2: second set of induced dipoles
!> @param[in] efi: electrif field due to the first set of induced dipoles
!> @param[in] efi2: electrif field due to the second set of induced dipoles
subroutine dbletmatxb_pme(nrhs,dodiag,mu, mu2, efi, efi2)
!
!     Compute the direct space contribution to the electric field due to the current value
!     of the induced dipoles, for two input vectors mu and mu2
!
!     if shortrange, makes short range evaluation
!
   use sizes
   use atmlst
   use atoms
