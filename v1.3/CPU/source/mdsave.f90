...
!> @param[in] epot: value of potential energy
!> @param[in] derivs: derivatives of potential energy wrt cartesian coordinates
...
!
!     Send positions,velocities and accelerations to the master
!
!
!     wrap coordinates in unit cell
!
   !call molecule(.false.)
...
!
!    master get size of all domains
!
   if (rank.eq.0) then
      do iproc = 1, nproc-1
         tagmpi = iproc + 1
...
!      end if
!
!     put the array in global order to be written
!
   if (rank.eq.0) then
      if (use_pmecore) then
         nprocloc = ndir
...
!
!     The master broadcasts the system (back in the unit cell)
!
...
!
!     Put the proper coordinates (potentially unwrapped) in xwrite,ywrite and zwrite
!
   if (rank.eq.0) then
      if (pbcunwrap) then
...
!
!     if multiple replicas, then number the traj outputs
!
   exten=''
   if (use_reps) then
...
!
   call prtdyn(exten)
!
!     save coordinates to an archived, numbered structure file, or dcd file
!
   if (dcdio) then
...
!     only correct for single time step Cartesian integrators
!
