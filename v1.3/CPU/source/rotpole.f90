...
!
!> @brief 
!> constructs the set of atomic multipoles in the global
!> frame by applying the correct rotation matrix for each site
!> @param no params
subroutine rotpole
   use atmlst
...
!
!> @brief 
!> finds the rotation matrix that converts from the local
!> coordinate system to the global frame at a multipole site
!> @param[in] iipole: multipole index
!> @param[in] iglob: atomic index
!> @param[out] a: rotation matrix
...
!
!> @brief 
!> computes the atomic multipoles at a specified site
!> in the global coordinate frame by applying a rotation matrix
!> @param[in] iipole: multipole index
