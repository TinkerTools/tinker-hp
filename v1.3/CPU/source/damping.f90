...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine dampewald  --  find Ewald damping coefficients  ##
!     ##                                                             ##
!     #################################################################
!
!
!     "dampewald" finds coefficients for error function damping used
!     for Ewald real space interactions
!
!
!> @brief 
!> finds coefficients for error function damping used
!> for Ewald real space interactions
!> @param no params
...
!
!
!     initialize the Ewald damping factor coefficients
!
   do i = 1, rorder
      dmpe(i) = scale
   end do
!
!     compute the successive Ewald damping factors
!
   ra = aewald * r
   bn(0) = erfc(ra) / r
   dmpe(1) = scale * bn(0)
   ...
   afac = 0.0d0
   if (aewald .gt. 0.0d0)  afac = 1.0d0 / (sqrtpi*aewald)
   niter = (rorder-1) / 2
   do i = 1, niter
      bfac = dble(2*i-1)
      afac = aesq2 * afac
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine dampthole  --  find Thole damping coefficients  ##
!     ##                                                             ##
!     #################################################################
!
!
...
!
!     B. T. Thole, "Molecular Polarizabilities Calculated with a
!     Modified Dipole Interaction", Chemical Physics, 59, 341-350 (1981)
!
!
!> @brief 
!> generates coefficients for the Thole damping
!> function for powers of the interatomic distance
!> @param no params
...
!
!
!     initialize the Thole damping factors to a value of one
!
   do j = 1, rorder
      dmpik(j) = 1.0d0
   end do
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine damptholed  --  alternate Thole damping values  ##
!     ##                                                             ##
!     #################################################################
!
!
!     "damptholed" finds coefficients for the original Thole damping
!     function used by AMOEBA or for the alternate direct polarization
!     damping used by AMOEBA+
!
!     literature reference:
!
!     B. T. Thole, "Molecular Polarizabilities Calculated with a
...
!
!> @brief 
!> finds coefficients for the original Thole damping
!> function used by AMOEBA or for the alternate direct polarization
!> damping used by AMOEBA+
!> @param no params
...
!
!     use original Thole polarization model damping factors
!
   else
      it = jpolar(iglob)
      kt = jpolar(kglob)
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine dampthole2  --  original Thole damping values  ##
!     ##                                                            ##
!     ################################################################
!
!
!     "dampthole2" finds coefficients for the original Thole damping
!     function used by AMOEBA and for mutual polarization by AMOEBA+
!
!     literature reference:
!
!     B. T. Thole, "Molecular Polarizabilities Calculated with a
...
!
!     assign original Thole polarization model damping factors
!
   damp = pdamp(i) * pdamp(k)
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine damppole  --  penetration damping coefficents  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine dampdir  --  direct field damping coefficents  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!
!> @brief 
!> generates coefficients for the direct field damping
!> function for powers of the interatomic distance
!> @param no params
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine dampmut  --  mutual field damping coefficents  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!
!> @brief 
!> generates coefficients for the mutual field damping
!> function for powers of the interatomic distance
!> @param no params
...
!     ###############################################################
!     ##                                                           ##
!     ##  subroutine damppot  --  electrostatic potential damping  ##
!     ##                                                           ##
!     ###############################################################
!
!
...
!
!> @brief 
!> generates coefficients for the charge penetration
!> damping function used for the electrostatic potential
!> @param no params
...
!
!
!     compute common exponential factors for damping
!
   dampk = alphak * r
   expk = exp(-dampk)
!
...
!     ##################################################################
!     ##                                                              ##
!     ##  subroutine damprep  --  find repulsion damping coefficents  ##
!     ##                                                              ##
!     ##################################################################
!
!
...
!
!> @brief 
!> generates coefficients for the Pauli repulsion
!> damping function for powers of the interatomic distance
!> @param no params
