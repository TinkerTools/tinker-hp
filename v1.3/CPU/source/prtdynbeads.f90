...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine prtdynbeads  --  output of MD restart information  ##
!     ##                                                                ##
!     ####################################################################
!
!
!     "prtdynbeads" writes out the information needed to restart a
!     molecular dynamics trajectory to an external disk file while
!     using PIMD
!
!
!> @brief 
!> writes out the information needed to restart a
!> PIMD trajectory to an external disk file while
!> using PIMD
!> @param[in] polymer: current polymer
!> @param[in] ibead_beg: first index of the local beads
!> @param[in] ibead_end: last index of the local beads
