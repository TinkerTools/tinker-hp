...
!> @brief 
!> build blocks for DC-JI/DIIS polarization
!> @param no params
subroutine cluster1
   use atmlst
   use atoms
...
!
   ...
   if (.not. allocated(means))then
      ...
      tk=1
!
!       initialize the means of our blocks
!
      do ii = 1, km
         iipole = poleglob(tk)
         iglob = ipole(iipole)
...
!
!     build group list for all atoms
!
   do i = 1, n
      grplst(i) = -1
   end do

!      if(rank .eq. 0)print*,km," blocks per proc.",step
...
!
!     Kofst is where each Zmat begins in the array
!
   maxdim = npergrp(1)
...
!
   ...
   do i = 1, 1000
!
!   begin neighbors list
!
      if(iter .eq. 0 .or. inc/km .gt. reinit) then
!          if(iter .ne. 0)print*,"reinit"
         inc = 0.0d0
         loca = 1
         do ii = 1, npoleloc
            iipole = poleglob(ii)
...
!
!   end neighbors list begin K-means
!
      do ii = 1, npoleloc
         iipole = poleglob(ii)
         iglob = ipole(iipole)
...
!
   ...
   if (.not. allocated(grplst))then
      ...
      call pcluster
!        print*,"proc",rank,"K grid",dcx,dcy,dcz
      allocate(grplst(n))
      allocate(npergrp(km))
   end if
...
!
!    normailze the axis and get difference between the longest and the two shorter
!
...
!
!    build up and try to get as close as possible to
!    the desired km while keeping the number of
!    segments of each axis proportional to its length
!
...
!      print*,"Requested block size", npoleloc/km
   km = list2(3)*list2(2)*list2(1)
!      print*,"Used block size", npoleloc/km
...
!
!> @brief 
!> gets the partitioning of the system into groups
!> and stores a list of the group to which each atom belongs
!> @param no params
subroutine cluster
   use atoms
