...
!     #############################################################
!     ##                                                         ##
!     ##  subroutine kdisp  --  dispersion parameter assignment  ##
!     ##                                                         ##
!     #############################################################
!
!
...
!
!> @brief 
!> assigns C6 coefficients and damping parameters for
!> dispersion interactions and processes any new or changed
!> values for these parameters
!> @param no params
subroutine kdisp
...
!
!
!     process keywords containing damped dispersion parameters
!
   header = .true.
   do i = 1, nkey
      next = 1
...
!
!     assign the dispersion C6 values and alpha parameters
!
   do i = 1, n
      ic = class(i)
      csix(i) = dspsix(ic)
...
!
!     process keywords containing atom specific dispersion parameters
!
   header = .true.
   do i = 1, nkey
      next = 1
...
!
!     remove zero and undefined dispersion sites from the list
!
   ndisp = 0
   do i = 1, n
...
!
!     compute pairwise sum of C6 coefficients needed for PME
!
   csixpr = 0.0d0
   if (use_dewald) then
      do i = 1, ndisp
...
!
!     turn off the dispersion potential if not used
!
   if (ndisp .eq. 0)  then
      use_disp = .false.
      use_dlist = .false.
...
!
!     subroutine dealloc_shared_disp : deallocate shared memory pointers for dispersion
!     parameter arrays
!
!> @brief 
!> deallocate shared memory pointers for dispersion
!> parameter arrays
!> @param no params
...
!
!     subroutine alloc_shared_disp : allocate shared memory pointers for dispersion
!     parameter arrays
!
!> @brief 
!> allocate shared memory pointers for dispersion
!> parameter arrays
!> @param no params
