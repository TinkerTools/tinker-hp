...
!
!> @brief 
!> computes a numerical approximation to the value of
!> the error function via a Chebyshev approximation
!> @param[in] x: value to compute erf on
function erf (x)
   implicit none
   integer mode
   real*8 erf,x
...
!> @brief 
!> computes a numerical approximation to the value of
!> the complementary error function via a Chebyshev approximation
!> @param[in] x: value to compute erf on
function erfc (x)
   implicit none
   integer mode
...
!> returns erfc; uses rational functions that approximate erf(x)
!> and erfc(x) to at least 18 significant decimal digits
!> @param[in] arg: value to compute the function on
!> @param[in] result: result of the evaluation
!> @param[in] mode: use erf or erfc
...
!> an argument in the range (-1,1) using a rational function
!> approximation followed by cycles of Newton-Raphson correction
!> @param[in] x: value to evaluate function on
function erfinv (x)
   use iounit
   use math
   implicit none
