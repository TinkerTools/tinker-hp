...
!     ###############################################################
!     ##                                                           ##
!     ##  subroutine evcorr  --  long range vdw energy correction  ##
!     ##                                                           ##
!     ###############################################################
!
!
...
!     Liquids, 2nd Ed.", Oxford University Press, 2017, Section 2.8
!
!
!> @brief 
!> computes the long range van der Waals correction
!> to the energy via numerical integration
!> @param no params
...
!
!
!     zero out the long range van der Waals correction
!
   elrc = 0.0d0
!
!     only the master computes the vdw correction
!
   if (rank.ne.0) return
!
!     only applicable if periodic boundaries are in use
!
...
!
   call switch (mode)
!
!     set number of steps and range for numerical integration
!
   nstep = 2
   range = 100.0d0
...
!
   allocate (mvt(n))
!
!     count the number of types and their frequencies
!
   nvt = 0
   do i = 1, n
...
!
!     find the correction energy via double loop search
!
   do i = 1, nvt
      it = ivt(i)
      ...
      do k = i, nvt
         ...
         fkm = dble(mvt(k))
!
!     set decoupling or annihilation for intraligand interactions
!
...
!     ##############################################################
!     ##                                                          ##
!     ##  subroutine evcorr1  --  long range vdw energy & virial  ##
!     ##                                                          ##
!     ##############################################################
!
!
...
!
!
!     zero out the long range van der Waals corrections
!
   elrc = 0.0d0
   vlrc = 0.0d0
!
