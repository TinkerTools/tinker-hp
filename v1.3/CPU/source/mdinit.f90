...
!
!> @brief 
!> initializes the velocities and accelerations
!> for a molecular dynamics trajectory, including restarts
...
!          if (rank.eq.0) then
!            write(0,*)"  CREATING PLUMED FROM THE PROGRAM"
!          endif
!
!          conversion factors to go from TINKER units to PLUMED
         ...
         if (use_reps) then
           call plumed_f_gcmd("GREX setMPIFIntracomm"//char(0),
     $        COMM_ROOT2ROOT)
         end if
...
!
!     lambda-dynamics not compatible with pmecore
!
      if (use_pmecore) then
         if (ranktot.eq.0) then
...
!
!     keywords fr respa and respa1 integrators
!
   do i = 1, nkey
      next = 1
      record = keyline(i)
