...
!
!> @brief 
!> setup the arrays for the computations of 
!> 3d ffts with 2decomp&FFT
!> @param[in] nrec: number of cores dedicated to reciprocal space
!> @param[in] rank_bis: rank of the local process
!> @param[in] *start*: starting indexes of the grid along the 3 directions
!> @param[in] *end*: endig indexes of the grid along the 3 directions
!> @param[in] nx,ny,nz: dimension of the grid along the 3 directions
!> @param[in] comm_loc: local MPI communicator
!> @param[in] ngrid1,ngrid2: 2d processor grid for MPI parallelism 2d processor grid for MPI parallelism
...
implicit none
integer :: nrec,rank_bis,comm_loc,ngrid1,ngrid2
...
!
!> @brief 
!> compute forward 3dFFT in parallel with 2decomp&FFT
!> @param[in] qgridin: FFT grid taken (input)
!> @param[in] qgridin: transposed FFT grid (output)
!> @param[in] n*mpimax: dimensions of the grid
...
!
!> @brief 
!> compute backward 3dFFT in parallel with 2decomp&FFT
!> @param[in] qgridin: FFT grid taken (input)
!> @param[in] qgridin: transposed FFT grid (output)
!> @param[in] n*mpimax: dimensions of the grid
...
!
!> @brief 
!> finalize FFT simulation with 2decompFFT library
!> @param no params
