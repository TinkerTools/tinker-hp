...
!     ##########################################################################
!     ##                                                                      ##
!     ##  subroutine erepel1  --  Pauli repulsion energy & first derivatives  ##
!     ##                                                                      ##
!     ##########################################################################
!
!
!     "erepel1" calculates the Pauli repulsion energy and first derivatives
!
...
subroutine erepel1
   ...
   implicit none
!
   if (deb_Path) write(iout,*), 'erepel1 '
!
!
!
...
!     ######################################################################################
!     ##                                                                                  ##
!     ##  subroutine erepel1c  --  Pauli repulsion energy and first derivatives via list  ##
!     ##                                                                                  ##
!     ######################################################################################
!
!
!     "erepel1c" calculates the Pauli repulsion energy and first derivatives
!
!
!> @brief 
!> calculates the Pauli repulsion energy and first derivatives
...
!
   if (deb_Path) write(iout,*), 'erepel1c '
!
...
!
!
!     zero out the Pauli repulsion energy and derivatives
!
   er = 0.0d0
   der = 0d0
!
...
!
!     set lambda scaling for decoupling or annihilation
!
               mutik = .false.
...
!
!     get energy and force via soft core lambda scaling
!
               if (mutik) then
                  ...
                  if (use_lambdadyn) then
                    delambdavtemp = e*r/sqrt(term)*(1+vlambda/(2d0*term))
                  end if
                  e = soft * e
               end if
!
...
!
               ...
               frcx = frcx*facts + factds*xr/r
               frcy = frcy*facts + factds*yr/r
               frcz = frcz*facts + factds*zr/r
               do j = 1, 3
                  ttri(j) = ttri(j) * facts
                  ttrk(j) = ttrk(j) * facts
               end do
