...
!     ##################################################################
!     ##                                                              ##
!     ##  module subInform  --  submodule for Inform                  ##
!     ##                                                              ##
!     ##################################################################
!
...
!> @brief 
!> initialize the debuging internal variable based on environment variables
!> @param no params
...
!> @brief 
!> print information on the system and the simulation (number of atoms,
...
17    format(A20,G15.4E1)
      ...
      write(iout,15), 'integrator', integrate
      ...
      if (use_repuls) then
         write(iout,14), 'repulsion cutoff'   , repcut
         write(iout,14), 'repulsion short cutoff'   , repshortcut
         write(iout,14), 'repulsion taper'    , reptaper
         write(iout,14), 'shortheal'    , shortheal
      end if
...
!> @param[in] array: array in which to search
!> @param[in] find: true if value found
!> @param[in] ind: index of the array where the value is found (if true)
...
!> @brief 
!> compute min, max and L1 norm of an array
!> @param[in] vector: vector to compute the norms of
...
!> @brief 
!> compute L2 norm (and p norm if asked) of a vector
!> @param[in] array: array to compute the norms of
!> @param[in] n: size to compute the norms of
!> @param[in] value: computed norm
!> @param[in] p: integer related to the p-norm to compute
...
!> @brief 
!> compute sums of element of an array
!> @param[in] array: array to compute the norms of
!> @param[in] n: size to compute the norms of
...
!> @brief 
!> compute sums of element of an array and max
!> @param[in] array: array to compute the norms of
!> @param[in] n: size to compute the norms of
...
!
!> @brief 
!> Print information on position, velocities and aceleration sums of element of an array
!> @param[in] opt: if 1 (default 0), also print aalt arrays
...
!> @brief 
!> Print information on forces
!> @param[in] rule: if 0 bonded forces, if 1 short range non bonded, if 2 non bonded
