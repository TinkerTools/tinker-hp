# Build Tinker-HP (GPU) {#build}

## Direct Build
For most users, we recommend this relatively accessible method which lies on a bash installation script named `/tinker-hp/GPU/ci/install/sh`. After setting your environment according to the @ref prerequisites "Prerequisites", change directory to the GPU folder, edit the configuration section of the installation script to select your build and proceed with the commands below.
...
@warning Requires a CUDA python environment consistent with the installation one before building.
...
### Some targets
  ...
  - `create_build` ;  
    ...
    @note Depending on the precision you might have to rebuild the `2decomp_fft` library.  
  ...
  - `2decomp_fft_rebuild_single` ;  
    Same as previous target but except from the precision's construct which happen here to in single. There is only two precision modes for 2decomp_fft Library. Tinker mixed precision building requires 2decomp_fft single precision library.
