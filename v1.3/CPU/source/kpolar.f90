...
!
!> @brief 
!> assigns atomic dipole polarizabilities to the atoms
!> within the structure and processes any new or changed values
!> @param no params
subroutine kpolar
   use atmlst
...
!
!     subroutine kpolar_update: update local polar
!
!> @brief 
...
!         end if
         if (domlen(pbig_recep(iproc)+1).ne.0) then
            bufbegpole(pbig_recep(iproc)+1) = npolebloc + 1
         else
            bufbegpole(pbig_recep(iproc)+1) = 1
         end if
!         do i = 1, domlen(p_recep1(iproc)+1)
         do i = 1, domlen(pbig_recep(iproc)+1)
!            iglob = glob(bufbeg(p_recep1(iproc)+1)+i-1)
            iglob = glob(bufbeg(pbig_recep(iproc)+1)+i-1)
            iipole = pollist(iglob)
!
!   skip atom if it is not in the multipole list
!
...
!         end if
         if (domlen(pbig_recep(iproc)+1).ne.0) then
            domlenpole(pbig_recep(iproc)+1) =&
            &npolebloc-bufbegpole(pbig_recep(iproc)+1)+1
         else
            domlenpole(pbig_recep(iproc)+1) = 0
         end if
      end do
!
!
...
!
!> @brief 
!> generates members of the polarization group of
!> each atom and separate lists of the 1-2, 1-3 and 1-4 group
!> connectivities
!> @param no params
subroutine polargrp
...
!
!     subroutine dealloc_shared_polar : deallocate shared memory pointers for polar
!     parameter arrays
!
!> @brief 
!> deallocate shared memory pointers for polar
!> parameter arrays
!> @param no params
...
!
!> @brief 
!> allocate shared memory pointers for polar
!> parameter arrays
!> @param no params
...
!> @brief 
!> finds the best omega parameter for TCG polarization with peak step
!> @param no params
subroutine omegafit
   use atmlst
   use atoms
...
!
!     send the converged value of omega to everybody
!
   call MPI_BCAST(tcgomega,1,MPI_REAL8,0,COMM_TINKER,ierr)
!
!     store converged dipole values for rms
!
   do i = 1, npoleloc
      iipole = poleglob(i)
      musave(:,1,i) = uind(:,iipole)
