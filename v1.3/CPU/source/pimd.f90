...
!
   ...
   if (.not.exist)  then
     if (rank.eq.0) write (iout,*) 'You Need To Enter the Number of Steps'
...
!
   ...
   call nextarg (string,exist)
   if (.not.exist)  then
     if (rank.eq.0) write (iout,*) 'You Need To Enter the Time Step Length in Femtoseconds'
     call MPI_BARRIER(COMM_TINKER,ierr)
     call fatal
   else
...
!
   ...
   call nextarg (string,exist)
   if (.not.exist) then
     if (rank.eq.0) write (iout,*) 'You Need To Enter the Time Between Dumps in Picoseconds'
     call MPI_BARRIER(COMM_TINKER,ierr)
     call fatal
   else
...
!
   if (use_bounds) then
      ...
      call nextarg (string,exist)
      if (.not.exist) then
        if (rank.eq.0) write (iout,*) 'You Need To Enter the Statistical Ensemble:'
...
!      end do
      if (mode.eq.2 .or. mode.eq.4) then
         ...
         call nextarg (string,exist)
         if (.not.exist) then
           if (rank.eq.0) write (iout,*) 'You Need To Enter the Temperature in Kelvin'
           call MPI_BARRIER(COMM_TINKER,ierr)
           call fatal
         else
...
!         endif
      ...
      if (mode.eq.4) then
         ...
         call nextarg (string,exist)
         if (.not.exist) then
           if (rank.eq.0) write (iout,*) 'You Need To Enter the Pressure in Atm'
           call MPI_BARRIER(COMM_TINKER,ierr)
           call fatal
         else 
