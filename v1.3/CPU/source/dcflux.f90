...
!     ##############################################################
!     ##                                                          ##
!     ##  subroutine dcflux  --  charge flux gradient chain rule  ##
!     ##                                                          ##
!     ##############################################################
!
!
...
!
!> @brief 
!> takes as input the electrostatic potential at each
!> atomic site and calculates gradient chain rule terms due to
!> charge flux coupling with bond stretching and angle bending
!> @param[in] pot: electrostatic potential at each site
!> @param[in] dcfx: derivative contribution on x coordinate
!> @param[in] dcfy: derivative contribution on y coordinate
!> @param[in] dcfz: derivative contribution on z coordinate
...
!
!     set tolerance for minimum distance and angle values
!
   eps = 0.0001d0
!
