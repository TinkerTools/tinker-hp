...
!     F Celerse, L Lagardere, E Derat, JP Piquemal
!     Journal of Chemical Theory and Computation 15 (6), 3694-3709
!
!
!> @brief 
!> assigns the biased forced on each concerned atoms and
!> stores the smd and derivatives energy components
subroutine esmd1
   use msmd
   use potent
...
!
!##########################################
!#### Write results in the outputfile ####
!##########################################
!
      do a = 1, ncsmd
         b = tcsmd(a)
...
!
!     Checking with respect of the SMDOUTPUTFREQ
!
               c = modulo(tpass,SMDoutputFreq)
               if (c == 0) then      ! LOOP5 CVSMDK1
!
!     Write in the SMD_output file
!
...
!
!##########################################
!#### Writte results in the outputfile ####
!##########################################
!
      do a = 1, ncsmd
         b = tcsmd(a)
!
!     Check of the use of CVSMD
!
         do j = 1, atfol
