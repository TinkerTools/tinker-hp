...
!
!> @brief 
!> searches an input string from left to right for an
!> integer and puts the numeric value in "number"; returns zero
!> with "next" unchanged if no integer value is found
!> @param[in] string: input character string to be searched
!> @param[in] number: output with the first integer in the string
!> @param[in] next: input with first position of search string; output with the position following the number
!
