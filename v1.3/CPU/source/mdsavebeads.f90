...
!     ######################################################################
!     ##                                                                  ##
!     ##  subroutine mdsavebeads  --  save trajectory and restart files   ##
!     ##                                                                  ##
!     ######################################################################
!
!
!     "mdsavebeads" writes molecular dynamics trajectory snapshots and
!     auxiliary files with velocity, force or induced dipole data;
!     also checks for user requested termination of a simulation while
!     using path pimd
!
!
!> @brief 
!> writes molecular dynamics trajectory snapshots and
!> auxiliary files with velocity, force or induced dipole data;
!> also checks for user requested termination of a simulation while
!> using path pimd
!> @param[in] istep: index of timestep
!> @param[in] dt: timestep
!> @param[in] poymer: current polymer
...
!> @brief 
!> necessary communications for output of PIMD simulation
!> @param no params
...
!          write(0,*) ranktot,iproc-1,nbeads_write
!     &       ,(ibead_end_s-ibead_beg_s+1)
...
!        if (ranktot==0) then
!          count=0
!          do iproc=nproc_write+1,nproctot
!            count=count+1
!            call MPI_ISEND(polymer%eigpos(1,1,1),3*n,MPI_REAL8
!     &         ,iproc-1,0,MPI_COMM_WORLD,reqs(count),ierr)
!            count=count+1
!            call MPI_ISEND(pbcwrapindex,3*n,MPI_INT
!     &         ,iproc-1,1,MPI_COMM_WORLD,reqs(count),ierr)
!          enddo
!          call MPI_WAITALL(count,reqs,MPI_STATUSES_IGNORE,ierr)
!
!        elseif(ranktot>nproc_write) then
!          call MPI_IRECV(polymer%eigpos(1,1,1),3*n,MPI_REAL8
!     &          ,0,0,MPI_COMM_WORLD,reqr(1),ierr)
!          call MPI_IRECV(pbcwrapindex,3*n,MPI_INT
!     &          ,0,1,MPI_COMM_WORLD,reqr(2),ierr)
