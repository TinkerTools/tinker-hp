...
!> @param[in] i: atomic site
!> @param[in] trq: 'torque' on the site 
!> @param[in] frcx: x-component of force contribution due to torque
!> @param[in] frcy: y-component of force contribution due to torque
!> @param[in] frcz: z-component of force contribution due to torque
!> @param[in] de: overall array parameter
...
!
   ib = ipole(i)
   if (ib.le.0) return
   ibloc = locgroup(loclocgroup(ib))
!
!     go from atom to local group frame
!
   ia = zaxis(i)
   if (ia.le.0) return
   ialoc = locgroup(loclocgroup(ia))
   if (ialoc.le.0) return
   ic = xaxis(i)
   if (ic.le.0) ic = ib
   icloc = locgroup(loclocgroup(ic))
   if (icloc.le.0) icloc = ibloc
   id = yaxis(i)
   if (id.le.0) id = ib
   idloc = locgroup(loclocgroup(id))
   if (idloc.le.0) idloc = ibloc
   axetyp = polaxe(i)
   if (axetyp .eq. 'None')  return
!
