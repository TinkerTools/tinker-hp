...
module qtb
   ...
   type(compteur_type) :: compteur
   type(compteur_type) :: compteur_bis
   integer :: plan_qtb
   integer :: nloc_plan=-1
...
!
   ...
   real*8 :: adqtb_tau_avg_default
   real*8 :: adqtb_tau_adapt
   logical :: adqtb_correct_bias
   logical :: adqtb_win_corr
   real*8, allocatable :: adqtb_tau_avg(:)
   real*8, allocatable :: A_gamma(:)
   ...
   type qtbdat_t
      ...
      real, allocatable :: fad(:,:,:)
      real, allocatable :: vad(:,:,:)
...
!
   subroutine convolseg()
      ...
      use spectra
      implicit none
      integer :: i,j,k,nloc_,ibegbeg,iendend, ibead
      integer :: iglob
...
!
   ...
   &,max_atoms_recv, max_atoms_send )
      ...
      integer :: nseg_comm,i,ineighbor,ierr,iseg,iglob
...
!          enddo
!          if(n_data_s(ineighbor)>max_atoms_s)
!     &        max_atoms_s=n_data_s(ineighbor)
!        enddo
...
!          if(qtbdata(i)%currently_loc) then
...
!-------------------------------------------------------------
   subroutine broadcast_piqtb_noise()
      ...
      use spectra
      implicit none
      integer :: i,iproc,maxsend,nsend,maxloc
