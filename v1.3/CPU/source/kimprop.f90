...
!
!> @brief 
!> assigns potential parameters to each improper
!> dihedral in the structure and processes any changed values
!> @param no params
subroutine kimprop
   use atmlst
...
!
!     process keywords with improper dihedral specific parameters
!
      header = .true.
      do i = 1, nkey
         next = 1
   ...
   90             format (/,' Additional Improper Dihedral Specific',&
               ...
               if (.not. silent) then
                  if (rank.eq.0) write (iout,100)  ia,ib,ic,id,tk,tv
...
!
!     subroutine kimprop_update: update local improp
!
!> @brief 
!  update local improp paramters
!> @param no params
