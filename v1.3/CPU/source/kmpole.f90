...
!
!> @brief 
!> assigns atomic multipole moments to the atoms of
!> the structure and processes any new or changed values
!> @param no params
subroutine kmpole
   use sizes
...
!
!     nd new charge penetration parameters in the keyfile
!
   header = .true.
   do i = 1, nkey
      next = 1
...
!
!     copy original multipole values that won't change during mutation
!
   pole_orig = pole
   return
end
...
!> @param no params
subroutine kmpole_update(istep)
   use sizes
   use atmlst
   use atmtyp
   use atoms
...
!
!     subroutine dealloc_shared_mpole : deallocate shared memory pointers for mpole
!     parameter arrays
!
!> @brief 
!> deallocate shared memory pointers for mpole
!> parameter arrays
!> @param no params
...
!
!     subroutine alloc_shared_mpole : allocate shared memory pointers for mpole
!     parameter arrays
!
!> @brief 
!> allocate shared memory pointers for mpole
!> parameter arrays
!> @param no params
