...
!
!     subroutine mdstattime: compute averages of various timings and
!      output them if necessary
!
!> @brief 
!> compute averages of various timings and
!>  output them if necessary
!> @param[in] istep: index of timestep
...
!
   modstep = mod(istep,iprint)
!
!     sum the timing values to get average over the cores
!
   if (modstep.eq.0) then
      buffer(1) = timeinte
      buffer(2) = timereneig
...
!
!     display the different times of the computation
!
