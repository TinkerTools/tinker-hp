...
!
!> @brief 
!> sets the coeffcients used by the fifth and seventh
!> order polynomial switching functions for spherical cutoffs
!> @param[in] mode: which non bonded interaction is being computed
...
!
   ...
   elseif (mode(1:11) .eq. 'SHORTCHGTRN') then
      ...
      off = dewaldcut
      cut = dewaldcut
   else if (mode(1:11) .eq. 'SHORTDEWALD') then
      off = dewaldshortcut
      cut = dewaldshortcut
   else
...
!
!> @brief 
!> sets the coeffcients used for SIBFA-like charge penetration
!> @param no params
