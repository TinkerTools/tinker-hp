!
!     Sorbonne University
!     Washington University in Saint Louis
!     University of Texas at Austin
!
#include "tinker_macro.h"
function sprod(n,v,w)
   implicit real(t_p) (a-h,o-z)
   integer,parameter::ti_p=t_p
   dimension v(*), w(*)
   sprod=0.0_ti_p
   do j=1,n
      sprod=sprod+v(j)*w(j)
   enddo
   return
end
subroutine sprodgpu(n,v,w,r)
   implicit real(t_p) (a-h,o-z)
   integer,parameter::ti_p=t_p
   dimension v(*), w(*)
!$acc serial async present(r)
   r=0.0_ti_p
!$acc end serial
!$acc parallel loop async present(v,w,r)
   do j=1,n
      r=r+v(j)*w(j)
   end do
end
!
subroutine aaddgpu(n,a,b,c)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*),c(*)
!$acc parallel loop gang vector_length(32) &
!$acc present(a,b,c)
   do j = 1, n
      c(j) = a(j) + b(j)
   enddo
   return
end
!
subroutine aaddgpuAsync(n,a,b,c)
   use utilgpu, only : rec_queue
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*),c(*)
!$acc parallel loop &
!$acc  present(a(1:n),b(1:n),c(1:n)) async(rec_queue)
   do j = 1, n
      c(j) = a(j) + b(j)
   enddo
   return
end
!
subroutine aadd(n,a,b,c)
!$acc routine gang
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*),c(*)
   do j = 1, n
      c(j) = a(j) + b(j)
   enddo
   return
end
!
subroutine asub(n,a,b,c)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*),c(*)
   do j = 1, n
      c(j) = a(j) - b(j)
   enddo
   return
end
!
subroutine acasb(n,a,b,c,s)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*),c(*)
   do j = 1, n
      c(j) = a(j) + s*b(j)
   enddo
   return
end
!
subroutine acasb2(n,a,b1,b2,c,s1,s2)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b1(*),b2(*),c(*)
   do j = 1, n
      c(j) = a(j) + s1*b1(j) + s2*b2(j)
   enddo
   return
end
!
subroutine amove(n,a,b)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = a(j)
   enddo
   return
end
!
subroutine amove1(n,a,b)
   implicit real(r_p) (a-h,o-z)
   dimension a(*),b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = a(j)
   enddo
   return
end
!
subroutine imove(n,a,b)
   implicit integer (a-z)
   dimension a(*),b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = a(j)
   enddo
   return
end
!
subroutine aneg(n,a,b)
   implicit real(t_p) (a-h,o-z)
   dimension a(*),b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = -a(j)
   enddo
   return
end
!
subroutine aset2(n,s,a)
   implicit real(t_p) (a-h,o-z)
   dimension a(*)
!$acc parallel loop async default(present)
   do j = 1, n
      a(j) = s
   enddo
   return
end
!
subroutine asasbc(n,a,b,c,s1,s2)
   implicit real(t_p) (a-h,o-z)
   dimension a(*), b(*), c(*)
!$acc parallel loop async default(present)
   do j = 1, n
      c(j) = s1*a(j) + s2*b(j)
   enddo
   return
end
!
subroutine ascale(n,s,a,b)
   implicit real(t_p) (a-h,o-z)
   dimension a(*), b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = s*a(j)
   enddo
   return
end
!
subroutine ascale_mp(n,s,a,b)
   implicit real(r_p) (a-h,o-z)
   dimension a(*), b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = s*a(j)
   enddo
   return
end
!
subroutine iscale(n,s,a,b)
   implicit integer (a-z)
   dimension a(*), b(*)
!$acc parallel loop async default(present)
   do j = 1, n
      b(j) = s*a(j)
   enddo
   return
end
!
integer function iarsum(istart,n,ia)
   implicit integer(a-z)
   dimension ia(*)
!
   sum = 0
   do i = 1, n
      sum = sum + ia(istart+i-1)
   end do
   iarsum = sum
   return
end
!
function arrsum(n,a)
   implicit real(t_p)(a-h,o-z)
   dimension a(*)
   arrsum = 0.d0
   do j = 1, n
      arrsum = arrsum + a(j)
   end do
   return
end
!
subroutine aclear(n,a)
   implicit real(t_p) (a-h,o-z)
   integer,parameter::ti_p=t_p
   dimension a(*)
   save zero
   data zero/0.0_ti_p/
!
   do j = 1, n
      a(j) = zero
   end do
   return
end
!
subroutine iclear(n,a)
   implicit integer (a-z)
   dimension a(*)
   save zero
   data zero/0/
!
   do j = 1, n
      a(j) = zero
   end do
   return
end
!
subroutine ione(n,a)
   implicit integer (a-z)
   dimension a(*)
   save one
   data one/1/
!
   do j = 1, n
      a(j) = one
   end do
   return
end
!
subroutine rmsvec(n,v,vrms,vmax)
   implicit real(t_p)(a-h,o-z)
   integer,parameter::ti_p=t_p
   dimension v(*)
   save zero
   data zero/0.0_ti_p/
!
   vmax=zero
   vrms=zero
   do j = 1, n
      if(abs(v(j)).gt.vmax) vmax=abs(v(j))
      vrms = vrms + (v(j)**2)
   end do
   vrms=sqrt(vrms/real(n,t_p))
   return
end
!
subroutine ylmbas(lmax,x,vfac,basis,vplm,vcos,vsin)
   use math
   implicit none
!
!   calculates a basis of real spherical harmonics up to order lmax.
!   the routine computes as a first the generalized legendre polynomials
!   and then weighs them with sine and cosine coefficients.
!   this lasts are evaluated starting from cos(phi) = y/sqrt(1-z**2)
!   using chebyshev polynomials.
!
!   input:  lmax  ... maximum angular momentum of the spherical harmonics
!                     basis
!           x     ... unit vector (x \in s_2) where to evaluate the basis
!           vfac  ... scratch array dimensioned 2*nbasis. Contains the
!                     normalization factors for the spherical harmonics.
!
!   output: basis ... spherical harmonics at x. basis is dimensioned (lmax+1)^2
!                     and the harmonics are ordered for increasing l and m
!                     (i.e.: 0,0; 1,-1; 1,0; 1,1; 2,-2; ...)
!
!   scratch: vplm ... scratch array dimensioned (lmax + 1)^2. it hosts the
!                     generalized legendre polynomials.
!            vcos ... scratch array dimensioned lmax + 1. it hosts the cos(m*\phi)
!                     values
!            vsin ... scratch array dimensioned lmax + 1. it hosts the sin(m*\phi)
!                     values.
!
   integer,parameter::ti_p=t_p
   integer lmax
   real(t_p) basis(*), x(*), vfac(2,*)
   real(t_p) vcos(*), vsin(*), vplm(*)
!
   integer l, m, ind
   real(t_p)  zero, one, cthe, sthe, cphi, sphi, plm
   save zero, one
   data zero/0.0_ti_p/, one/1.0_ti_p/
!
!   get cos(\theta), sin(\theta), cos(\phi) and sin(\phi) from the cartesian
!   coordinates of x.
!
   cthe = x(3)
   sthe = sqrt(one - cthe*cthe)
   if(sthe.ne.zero) then
      cphi = x(1)/sthe
      sphi = x(2)/sthe
   else
      cphi = one
      sphi = zero
   endif
!
!   evaluate cos(m*phi) and sin(m*phi) arrays. notice that this is
!   pointless if z = 1, as the only non vanishing terms will be the
!   ones with m=0.
!
   if(sthe.ne.zero) then
      call trgev(lmax,cphi,sphi,vcos,vsin)
   else
      do m = 1, lmax + 1
         vcos(m) = one
         vsin(m) = zero
      end do
   end if
!
!   evaluate the generalized legendre polynomials.
!
   call polleg(lmax,cthe,sthe,vplm)
!
!   now build the spherical harmonics. we will distinguish m=0,
!   m>0 and m<0:
!
   do l = 0, lmax
      ind = l**2 + l + 1
!       m = 0
      basis(ind) = vfac(1,ind)*vplm(ind)
      do m = 1, l
         plm = vplm(ind+m)
!         m > 0
         basis(ind+m) = vfac(1,ind+m)*plm*vcos(m+1)
!         m < 0
         basis(ind-m) = vfac(2,ind-m)*plm*vsin(m+1)
      end do
   end do
   return
end
!
subroutine dbasis(lmax,x,vfac,basis,dbas,vplm,vcos,vsin)
   use math
   implicit none
!
!   calculates a basis of real spherical harmonics up to order lmax.
!   the routine computes as a first the generalized legendre polynomials
!   and then weighs them with sine and cosine coefficients.
!   this lasts are evaluated starting from cos(phi) = y/sqrt(1-z**2)
!   using chebyshev polynomials.
!
!   input:  lmax  ... maximum angular momentum of the spherical harmonics
!                     basis
!           x     ... unit vector (x \in s_2) where to evaluate the basis
!           vfac  ... scratch array dimensioned lmax + 1. it hosts the first 2*lmax
!                     + 1 factorials. this is to be filled somewhere at the
!                     beginning of the program, as it is pointless to recompute
!                     it all the times. notice that 0! = vfac(1) according to
!                     fortran conventions.
!
!   output: basis ... spherical harmonics at x. basis is dimensioned (lmax+1)^2
!                     and the harmonics are ordered for increasing l and m
!                     (i.e.: 0,0; 1,-1; 1,0; 1,1; 2,-2; ...)
!           dbas  ... spherical harmonics cartesian gradient.
!
!   scratch: vplm ... scratch array dimensioned (lmax + 1)^2. it hosts the
!                     generalized legendre polynomials.
!            vcos ... scratch array dimensioned lmax + 1. it hosts the cos(m*\phi)
!                     values
!            vsin ... scratch array dimensioned lmax + 1. it hosts the sin(m*\phi)
!                     values.
!
   integer lmax
   integer,parameter::ti_p=t_p
   real(t_p) basis(*), x(*), dbas(3,*), vfac(2,*),&
      &vcos(*), vsin(*), vplm(*)
   integer l, m, ind
   real(t_p)  zero, pt5, one, cthe, sthe, cphi, sphi,&
      &et(3), ep(3), fln, dbthe, fnorm, plm, pp1, pm1, pp, dbphi
   save zero, pt5, one
   data zero/0.0_ti_p/, pt5/0.5_ti_p/, one/1.0_ti_p/
!
!     get cos(\theta), sin(\theta), cos(\phi) and sin(\phi) from the cartesian
!     coordinates of x.
!
   cthe = x(3)
   sthe = sqrt(one - cthe*cthe)
   if(sthe.ne.zero) then
      cphi = x(1)/sthe
      sphi = x(2)/sthe
   else
      cphi = one
      sphi = zero
   endif
!
!     evaluate the dirivatives of theta and phi:
!
   et(1) = cthe*cphi
   et(2) = cthe*sphi
   et(3) = -sthe
   if(sthe.ne.zero) then
      ep(1) = -sphi/sthe
      ep(2) = cphi/sthe
      ep(3) = zero
   else
      ep(1) = zero
      ep(2) = zero
      ep(3) = zero
   endif
!
!     evaluate cos(m*phi) and sin(m*phi) arrays. notice that this is
!     pointless if z = 1, as the only non vanishing terms will be the
!     ones with m=0.
!
   if(sthe.ne.zero) then
      call trgev(lmax,cphi,sphi,vcos,vsin)
   else
      do m = 1, lmax + 1
         vcos(m) = one
         vsin(m) = zero
      end do
   end if
!
!     evaluate the generalized legendre polynomials.
!
   call polleg(lmax,cthe,sthe,vplm)
!
!     now build the spherical harmonics. we will distinguish m=0,
!     m>0 and m<0:
!
   do l = 0, lmax
      ind = l**2 + l + 1
!
!     functions for m = 0
!
      fln = vfac(1,ind)
      basis(ind) = fln*vplm(ind)
!
!     gradient for m = 0
!
      if(l.gt.0) then
         dbthe = fln*vplm(ind+1)
         dbas(1,ind) = dbthe*et(1)
         dbas(2,ind) = dbthe*et(2)
         dbas(3,ind) = dbthe*et(3)
      else
         dbas(1,ind) = zero
         dbas(2,ind) = zero
         dbas(3,ind) = zero
      endif
!
      do m = 1, l
         fnorm = vfac(1,ind+m)
!
!     get p_l^m for the function, p_l^{m+1} and p_l^{m-1} for the
!     gradient
!
         plm = fnorm*vplm(ind+m)
         pp1 = zero
         if(m.lt.l) pp1 = -pt5*vplm(ind+m+1)
         pm1 = pt5*(real(l+m,t_p)*real(l-m+1,t_p)&
            &*vplm(ind+m-1))
         pp  = pp1 + pm1
!
!     m > 0
!
         dbthe = -fnorm*pp*vcos(m+1)
         dbphi = -real(m,t_p)*fnorm*vplm(ind+m)*vsin(m+1)
         basis(ind+m)  = plm*vcos(m+1)
         dbas(1,ind+m) = dbthe*et(1) + dbphi*ep(1)
         dbas(2,ind+m) = dbthe*et(2) + dbphi*ep(2)
         dbas(3,ind+m) = dbthe*et(3)
!
!     m < 0
!
         dbthe = -fnorm*pp*vsin(m+1)
         dbphi = real(m,t_p)*fnorm*vplm(ind+m)*vcos(m+1)
         basis(ind-m) = plm*vsin(m+1)
         dbas(1,ind-m) = dbthe*et(1) + dbphi*ep(1)
         dbas(2,ind-m) = dbthe*et(2) + dbphi*ep(2)
         dbas(3,ind-m) = dbthe*et(3)
      end do
   end do
   return
end
!
subroutine trgev(n,x,y,cx,sx)
   implicit none
   integer,parameter::ti_p=t_p
   integer n
   real(t_p)  x, y, cx(*), sx(*)
   integer i
   real(t_p)  zero, one, two
   save zero, one, two
   data zero/0.0_ti_p/, one/1.0_ti_p/, two/2.0_ti_p/
!
   cx(1) = one
   cx(2) = x
   sx(1) = zero
   sx(2) = y
   do i = 3, n+1
      cx(i) = two*x*cx(i-1) - cx(i-2)
      sx(i) = two*x*sx(i-1) - sx(i-2)
   end do
   return
end
!
subroutine polleg(lmax,x,y,plm)
   implicit none
!
!   computes the l,m associated legendre polynomial for -1 <= x <= 1
!   using the recurrence formula
!
!   (l-m)p(l,m) = x(2l-1)p(l-1,m) - (l+m-1)p(l-2,m)
!
   integer,parameter::ti_p=t_p
   integer lmax
   real(t_p)  x, y, plm(*)
   integer m, ind, ind2, l
   real(t_p) one, two, fact, pmm, somx2, pmm1, pmmo, pll
   save one, two
   data one/1.0_ti_p/, two/2.0_ti_p/
!
   fact  = one
   pmm   = one
   somx2 = y
!     somx2 = sqrt((one-x)*(one+x))
   do m = 0, lmax
!
!  do pmm for each m:
!
      ind  = (m+1)*(m+1)
      plm(ind) = pmm
      if(m.eq.lmax) return
      pmm1 = x*(two*real(m,t_p)+one)*pmm
      ind2 = ind + 2*m + 2
      plm(ind2) = pmm1
      pmmo = pmm
      do l = m+2, lmax
         pll   = (x*(two*real(l,t_p)-one)*pmm1 -&
            &(real(l,t_p)+real(m,t_p)-1)*pmm)/&
            &(real(l,t_p)-real(m,t_p))
         ind = l*l + l + 1
         plm(ind+m) = pll
         pmm  = pmm1
         pmm1 = pll
      end do
      pmm  = -pmmo*fact*somx2
      fact = fact + two
   end do
   return
end
!
subroutine derrot(ii,doder,i,iz,ix,iy,r,dri,driz,drix,driy)
   use atoms
   use mpole
   implicit none
!
!     given the index ii of a multipole, gives as output the indexes of
!     the atoms used to define the local frame and the derivatives of
!     the rotation matrix with respect to the position of such atoms.
!
   integer,parameter::ti_p=t_p
   integer ii, i, iz, ix, iy
   real(t_p) r(3,3)
   real(t_p) dri(3,3,3), driz(3,3,3), drix(3,3,3),driy(3,3,3)
   logical doder
   integer j, k, l
   real(t_p)  xxi, xxi2, eet, eet2, zze, zze2, uu, uu2, uu3,&
      &vez, eex, eex2, zero, one
   real(t_p)  xi(3), eta(3), zeta(3), ex(3), ey(3)
   real(t_p)  u(3), v(3), ez(3)
   real(t_p)  ezu(3,3), exv(3,3), exez(3,3), eyez(3,3),&
      &eyex(3,3),uri(3,3), uriz(3,3), urix(3,3), uriy(3,3),&
      &vri(3,3), vrix(3,3), vriy(3,3), exi(3,3), exiz(3,3),&
      &exix(3,3), exiy(3,3), eyi(3,3), eyiz(3,3), eyix(3,3),&
      &eyiy(3,3), ezi(3,3), eziz(3,3), ezix(3,3), eziy(3,3)
!
   save zero, one
   data zero/0.0_ti_p/, one/1.0_ti_p/
!
1000 format('polaxe not recognized or not implemented.')
!
!     Get the reference atoms coordinates.
!
   i  = ipole(ii)
!
   if (polaxe(i).eq.'None') then
      r    = zero
      dri  = zero
      driz = zero
      drix = zero
      driy = zero
      return
   end if
!
   iz = zaxis(i)
   ix = xaxis(i)
   iy = yaxis(i)
   if (iz.ne.0) then
      xi(1)   = x(iz) - x(i)
      xi(2)   = y(iz) - y(i)
      xi(3)   = z(iz) - z(i)
      xxi2    = xi(1)*xi(1) + xi(2)*xi(2) + xi(3)*xi(3)
      xxi     = sqrt(xxi2)
   end if
   if (ix.ne.0) then
      eta(1)  = x(ix) - x(i)
      eta(2)  = y(ix) - y(i)
      eta(3)  = z(ix) - z(i)
      eet2    = eta(1)*eta(1) + eta(2)*eta(2) + eta(3)*eta(3)
      eet     = sqrt(eet2)
   end if
   if (iy.ne.0) then
      zeta(1) = x(iy) - x(i)
      zeta(2) = y(iy) - y(i)
      zeta(3) = z(iy) - z(i)
      zze2    = zeta(1)*zeta(1) + zeta(2)*zeta(2) + zeta(3)*zeta(3)
      zze     = sqrt(zze2)
   end if
!
!     We will write ez = u/|u|, ex = (v - (v*ez)ez)/(v - (v*ez)ez) and
!     ey = ez x ex for all the possible definitions.
!     Here, we define for each method the u and v vectors:
!
   if (polaxe(i).eq.'Z-then-X') then
      u(1) = xi(1)
      u(2) = xi(2)
      u(3) = xi(3)
      v(1) = eta(1)
      v(2) = eta(2)
      v(3) = eta(3)
   else if (polaxe(i).eq.'Bisector') then
      u(1) = eet*xi(1) + xxi*eta(1)
      u(2) = eet*xi(2) + xxi*eta(2)
      u(3) = eet*xi(3) + xxi*eta(3)
      v(1) = eta(1)
      v(2) = eta(2)
      v(3) = eta(3)
   else if (polaxe(i).eq.'Z-Bisect') then
      u(1) = xi(1)
      u(2) = xi(2)
      u(3) = xi(3)
      v(1) = zze*eta(1) + eet*zeta(1)
      v(2) = zze*eta(2) + eet*zeta(2)
      v(3) = zze*eta(3) + eet*zeta(3)
   else if (polaxe(i).eq.'3-Fold') then
      u(1) = eet*zze*xi(1) + xxi*zze*eta(1) + eet*zze*zeta(1)
      u(2) = eet*zze*xi(2) + xxi*zze*eta(2) + eet*zze*zeta(2)
      u(3) = eet*zze*xi(3) + xxi*zze*eta(3) + eet*zze*zeta(3)
      v(1) = eta(1)
      v(2) = eta(2)
      v(3) = eta(3)
   else
      write(6,1000)
      call fatal
   end if
   uu2 = u(1)*u(1) + u(2)*u(2) + u(3)*u(3)
   uu  = sqrt(uu2)
!
!     Assemble the three versors:
!
   ez(1) = u(1)/uu
   ez(2) = u(2)/uu
   ez(3) = u(3)/uu
   vez   = ez(1)*v(1) + ez(2)*v(2) + ez(3)*v(3)
   ex(1) = v(1) - vez*ez(1)
   ex(2) = v(2) - vez*ez(2)
   ex(3) = v(3) - vez*ez(3)
   eex2  = ex(1)*ex(1) + ex(2)*ex(2) + ex(3)*ex(3)
   eex   = sqrt(eex2)
   ex(1) = ex(1)/eex
   ex(2) = ex(2)/eex
   ex(3) = ex(3)/eex
   ey(1) = ez(2)*ex(3) - ez(3)*ex(2)
   ey(2) = ez(3)*ex(1) - ez(1)*ex(3)
   ey(3) = ez(1)*ex(2) - ez(2)*ex(1)
   do j = 1, 3
      r(j,1) = ex(j)
      r(j,2) = ey(j)
      r(j,3) = ez(j)
   end do
   if (.not. doder) return
!
!     clear everything.
!
   do j = 1, 3
      do k = 1, 3
         ezu(j,k)  = zero
         exv(j,k)  = zero
         exez(j,k) = zero
         eyez(j,k) = zero
         eyex(j,k) = zero
         uri(j,k)  = zero
         uriz(j,k) = zero
         urix(j,k) = zero
         uriy(j,k) = zero
         vri(j,k)  = zero
         vrix(j,k) = zero
         vriy(j,k) = zero
         exi(j,k)  = zero
         exiz(j,k) = zero
         exix(j,k) = zero
         exiy(j,k) = zero
         eyi(j,k)  = zero
         eyiz(j,k) = zero
         eyix(j,k) = zero
         eyiy(j,k) = zero
         ezi(j,k)  = zero
         eziz(j,k) = zero
         ezix(j,k) = zero
         eziy(j,k) = zero
         do l = 1, 3
            dri(j,k,l)  = zero
            driz(j,k,l) = zero
            drix(j,k,l) = zero
            driy(j,k,l) = zero
         end do
      end do
   end do
!
!     We will assemble the derivatives of the rotation matrices as
!     the product of two contributions, according to the chain rule.
!     The first part is the derivative of the versors wrt the u, v
!     vectors and is independent of the model; the second part is the
!     derivatives of u and v wrt the positions of the involved atoms
!     and is specific. We will assemble here the first part. For later
!     convenience we will also compute here dex/dez, dey/dez and
!     dey/dex.
!
   uu3  = uu*uu2
   do j = 1, 3
      ezu(j,j)  = one/uu
      exv(j,j)  = one/eex
      exez(j,j) = -vez/eex
      do k = 1, 3
         ezu(j,k)  = ezu(j,k) - u(j)*u(k)/uu3
         exv(j,k)  = exv(j,k) - ez(j)*ez(k)/eex - ex(j)*ex(k)/eex
         exez(j,k) = exez(j,k) + ex(j)*vez*v(k)/eex2 - ez(j)*v(k)/eex
      end do
   end do
   eyez(1,1) = zero
   eyez(1,2) =  ex(3)
   eyez(1,3) = -ex(2)
   eyez(2,1) = -ex(3)
   eyez(2,2) = zero
   eyez(2,3) =  ex(1)
   eyez(3,1) =  ex(2)
   eyez(3,2) = -ex(1)
   eyez(3,3) = zero
   eyex(1,1) = zero
   eyex(1,2) = -ez(3)
   eyex(1,3) =  ez(2)
   eyex(2,1) =  ez(3)
   eyex(2,2) = zero
   eyex(2,3) = -ez(1)
   eyex(3,1) = -ez(2)
   eyex(3,2) =  ez(1)
   eyex(3,3) = zero
!
!     We compute now all the chain rule contributions, in particular:
!       du/dr(i), du/dr(iz), du/dr(ix), du/dr(iy)
!       dv/dr(i), dv/dr(ix), dv/dr(iy)
!     which we will use to assemble the derivatives of the rotation
!     matrix.
!
   if (polaxe(i).eq.'Z-then-X') then
      do j = 1, 3
         uri(j,j)  = -one
         uriz(j,j) = one
         vri(j,j)  = -one
         vrix(j,j) = one
      end do
   else if (polaxe(i).eq.'Bisector') then
      do j = 1, 3
         uri(j,j)  = -xxi - eet
         uriz(j,j) = eet
         urix(j,j) = xxi
         vri(j,j)  = -one
         vrix(j,j) = one
         do k = 1, 3
            uri(j,k)  = uri(j,k) - eta(j)*xi(k)/xxi - xi(j)*eta(k)/eet
            uriz(j,k) = uriz(j,k) + eta(j)*xi(k)/xxi
            urix(j,k) = urix(j,k) + xi(j)*eta(k)/eet
         end do
      end do
   else if (polaxe(i).eq.'Z-Bisect') then
      do j = 1, 3
         uri(j,j)  = -one
         uriz(j,j) = one
         vri(j,j)  = -eet - zze
         vrix(j,j) = zze
         vriy(j,j) = eet
         do k = 1, 3
            vri(j,k)  = vri(j,k)  - eta(j)*zeta(k)/zze&
               &- zeta(j)*eta(k)/eet
            vrix(j,k) = vrix(j,k) + zeta(j)*eta(k)/eet
            vriy(j,k) = vriy(j,k) + eta(j)*zeta(k)/zze
         end do
      end do
   else if (polaxe(i).eq.'3-Fold') then
      do j = 1, 3
         uri(j,j)  = -xxi*eet - xxi*zze - eet*zze
         uriz(j,j) = +eet*zze
         urix(j,j) = +xxi*zze
         uriy(j,j) = +xxi*eet
         vri(j,j)  = -one
         vrix(j,j) = one
         do k = 1, 3
            uri(j,k)  = uri(j,k)  - (zze*eta(j)+eet*zeta(j))*xi(k)/xxi&
               &- (zze*xi(j)+xxi*zeta(j))*eta(k)/eet&
               &- (eet*xi(j)+xxi*eta(j))*zeta(k)/zze
            uriz(j,k) = uriz(j,k) + (zze*eta(j)+eet*zeta(j))*xi(k)/xxi
            urix(j,k) = urix(j,k) + (zze*xi(j)+xxi*zeta(j))*eta(k)/eet
            uriy(j,k) = uriy(j,k) + (eet*xi(j)+xxi*eta(j))*zeta(k)/zze
         end do
      end do
   else
      write(6,1000)
      call fatal
   end if
!
!     We are, finally, ready to assemble the derivatives of each versor.
!     The code does not make distinctions anymore between the various
!     methods to define the local frame. This is not top efficient, but
!     the price to pay for a more efficient code would be a very messy
!     one: as the overall cost of the computation is small, order is
!     preferred.
!
!     ez derivatives:
!
   do l = 1, 3
      do j = 1, 3
         do k = 1, 3
            ezi(l,j)  = ezi(l,j)  + ezu(l,k)*uri(k,j)
            eziz(l,j) = eziz(l,j) + ezu(l,k)*uriz(k,j)
            ezix(l,j) = ezix(l,j) + ezu(l,k)*urix(k,j)
            eziy(l,j) = eziy(l,j) + ezu(l,k)*uriy(k,j)
         end do
      end do
   end do
!
!     ex derivatives:
!
   do l = 1, 3
      do j = 1, 3
         do k = 1, 3
            exi(l,j) =exi(l,j) +exv(l,k)*vri(k,j) +exez(l,k)*ezi(k,j)
            exiz(l,j)=exiz(l,j)                   +exez(l,k)*eziz(k,j)
            exix(l,j)=exix(l,j)+exv(l,k)*vrix(k,j)+exez(l,k)*ezix(k,j)
            exiy(l,j)=exiy(l,j)+exv(l,k)*vriy(k,j)+exez(l,k)*eziy(k,j)
         end do
      end do
   end do
!
!     ey derivatives:
!
   do l = 1, 3
      do j = 1, 3
         do k = 1, 3
            eyi(l,j) =eyi(l,j) +eyex(l,k)*exi(k,j) +eyez(l,k)*ezi(k,j)
            eyiz(l,j)=eyiz(l,j)+eyex(l,k)*exiz(k,j)+eyez(l,k)*eziz(k,j)
            eyix(l,j)=eyix(l,j)+eyex(l,k)*exix(k,j)+eyez(l,k)*ezix(k,j)
            eyiy(l,j)=eyiy(l,j)+eyex(l,k)*exiy(k,j)+eyez(l,k)*eziy(k,j)
         end do
      end do
   end do
!
!     Finally, assemble the derivatives of the rotation matrix.
!
   do j = 1, 3
      do k = 1, 3
         dri(k,j,1)  = exi(j,k)
         dri(k,j,2)  = eyi(j,k)
         dri(k,j,3)  = ezi(j,k)
         driz(k,j,1) = exiz(j,k)
         driz(k,j,2) = eyiz(j,k)
         driz(k,j,3) = eziz(j,k)
         drix(k,j,1) = exix(j,k)
         drix(k,j,2) = eyix(j,k)
         drix(k,j,3) = ezix(j,k)
         driy(k,j,1) = exiy(j,k)
         driy(k,j,2) = eyiy(j,k)
         driy(k,j,3) = eziy(j,k)
      end do
   end do
   return
end
!
logical function tinker_isnan(x)
   use ieee_arithmetic
   real(t_p) x
   tinker_isnan = ieee_is_nan(x)
end

logical function tinker_isnan_m(x)
   use ieee_arithmetic
   real(r_p) x
   tinker_isnan_m = ieee_is_nan(x)
end
