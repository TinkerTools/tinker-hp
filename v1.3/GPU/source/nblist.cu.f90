...
#include "midpointimage.inc.f90"
   ...
   attributes(global)&
      subroutine atoms_in_cell(nlocnl,n,ncell_tot,nx_cell,ny_cell,ineignl,x,y,z,xmin,ymin,zmin&
                              ,eps_cell,lenx_cell,leny_cell,lenz_cell,repartcell,cell_len,indcelltemp)
   implicit none
...
#ifdef TINKER_DEBUG
      if (xw<0.or.xw>=nx_cell) print*,'xb out',xw,xr,real(0.05*lenx_cell,4),eps_cell
      if (yw<0.or.yw>=ny_cell) print*,'yb out',xw,yr,real(0.05*leny_cell,4),eps_cell
      if (zw<0.or.zw>=nz_cell) print*,'zb out',xw,zr,real(0.05*lenz_cell,4),eps_cell
#endif
   ...
   attributes(global) subroutine filter_lst_sparse&
   ...
   attributes(global) subroutine filter_lsts_sparse&
        (cell_glob,block_scan,x,y,z,b_stat,b_rmid&
    ...
    !blst alst & nlst  block-atom list & size
   implicit none
   integer,value ,intent(in)   :: nblock,matsize,nlocnlb,nb_p
   ...
   attributes(global) subroutine filter_lst_sparse1&
...
!
   attributes(global) subroutine filter_shortlst_sparse1&
          (cell_glob,block_scan,x,y,z,&
          matb_lst,nlocnlb,nblock,matsize,nblocknlb_pair,&
   ...
   attributes(global) subroutine filter_lst_sparse2&
             (cell_glob,block_scan,x,y,z,&
             matb_lst,nlocnlb,nblock,matsize,nblocknlb_pair,&
...
!       Filter both nblist and shortnblist with the same adjacency matrix
!
   attributes(global) subroutine filter_shortlst_sparse2&
      (cell_glob,block_scan,x,y,z,&
      matb_lst,nlocnlb,nblock,matsize,nblocknlb_pair,&
      shortcutbuff2,cutbufbeg2,cutbuff2,&
...
!       pattern
   ...
   attributes(global) subroutine blocksat_kcu&
       (a_kind,a_id,s_key,x,y,z,nb,na,nab,n&
       ,cutbuf,len_xcell,len_ycell,len_zcell,eps_cell&
       ,s_kind,sgl_id,so_x,so_y,so_z,b_stat,b_rmid)
   implicit none
   integer  ,value :: nb,na,nab,n
   real(t_p),value :: cutbuf,len_xcell,len_ycell,len_zcell,eps_cell
