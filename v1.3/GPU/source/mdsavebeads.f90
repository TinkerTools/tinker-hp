...
subroutine mdsavebeads (istep,dt,polymer)
   use atmtyp
   use atomsMirror
   use atoms   ,only: pbcunwrap,pbcWrap,pbcWrapIdx
   use beads
   use bound
   use boxes
   use domdec
...
!$acc update host(polymer%eigpos(:,:,1) &
!$acc      ,polymer%pos,polymer%vel,polymer%forces) async
   if(allocated(polymer%forces_slow)) then
!$acc update host(polymer%forces_slow) async
   endif
!$acc update host(pbcWrap,pbcwrapidx) async
   if(nproctot>1) then
...
!$acc update device(polymer%pos(:,:,ibead_beg:ibead_end)) async
...
subroutine comm_for_write_pi(polymer,nproc_write&
   ...
   do iproc=1,nproc_write
      ...
      do i=1,nlocpi 
         ...
         do j=1,3
           buffers(1+j,i,iproc) =polymer%eigpos(j,ii,1)
         enddo
      enddo
      do k=1,ibead_end_s-ibead_beg_s+1
         ...
         do i=1,nlocpi; do j=1,3
               ii=glob(i-1+ilocpi_beg(rank_polymer+1))
               buffers(1,i+nlocpi*k,iproc)=real(ii,r_p)
   ...
   if(ranktot<nproc_write) then
      do iproc=1,nproctot
         ...
         do i=1,nlocr(iproc)
            ...
            do j=1,3
                polymer%eigpos(j,ii,1)=buffer(1+j,i,iproc)
            enddo
         enddo
...
subroutine comm_after_boundspi(polymer,nproc_write)
   ...
   call MPI_IBCAST(polymer%eigpos(1,1,1),3*n,MPI_RPREC,0 &
&     ,MPI_COMM_WORLD,reqr(1),ierr) 
   call MPI_IBCAST(pbcWrap,n,MPI_INT,0 &
