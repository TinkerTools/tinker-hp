...
!     ####################################################################
!     ##                                                                ##
!     ##  program pibar  --  thermodynamic values from simulation data  ##
!     ##                                                                ##
!     ####################################################################
!
!
!     "pibar" computes the free energy, enthalpy and entropy difference
!     between two states via Zwanzig free energy perturbation (FEP)
!     and Bennett acceptance ratio (BAR) methods for PI simulations
!
!     note current version takes as input the trajectory archives A
!     and B, originally generated for states 0 and 1, respectively;
!     then finds the total potential energy for all frames of each
!     trajectory under control of key files for both states 0 and 1;
!     finally the FEP and BAR algorithms are used to compute the
!     free energy for state 0 --> state 1, and similar estimators
!     provide the enthalpy and entropy for state 0 --> state 1
!
!     modifications for NPT simulations by Chengwen Liu, University
!     of Texas at Austin, October 2015; enthalpy and entropy methods
!     by Aaron Gordon, Washington University, December 2016
!
...
!
   ...
   call initmpi
!
!     find thermodynamic perturbation procedure to perform
!
   mode  = 0
   query = .true.
...
!
!     create TINKER BAR file or compute thermodynamic values
!
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine makebar  --  store trajectory potential energy  ##
!     ##                                                             ##
!     #################################################################
!
!
...
!
!
!     get trajectory A archive and setup mechanics calculation
!
!      call initial
   call init_keys
...
!
!     store filename and keywords for trajectory A and state 0
!
   fnamea  = filename
   lenga   = leng
   titlea  = title
...
!
!     get trajectory B archive and setup mechanics calculation
!
...
!
!     store filename and keywords for trajectory B and state 1
!
   fnameb  = filename
   lengb   = leng
   titleb  = title
...
!
!     reopen trajectory A using the parameters for state 0
!
   nkey = nkey0
   do i = 1, nkey
      keyline(i) = keys0(i)
...
!$acc wait
   do ibead=1,nbeads
      ...
      if (dcdio) then
         ...
         iarc    = freeunit ()
         polymer_main%dcdinfo(ibead)%idcd = iarc
...
!$acc wait
   do ibead=1,nbeads
      ...
      if (dcdio) then
         ...
         rewind (unit=polymer_main%dcdinfo(ibead)%idcd)
         call readxyz (polymer_main%dcdinfo(ibead)%idcd)
      end if
!$acc wait
!$acc parallel loop default(present)
...
!
   ...
   enddo
   !deallocate(polymer_main%dcdinfo)
!
!     reopen trajectory B using the parameters for state 0
!
   nkey = nkey1
   do i = 1, nkey
      keyline(i) = keys1(i)
...
!
   ...
   deallocate (keys1)
!
!     save potential energies and system volumes to a file
!
   if(ranktot==0) then
      ibar = freeunit ()
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine barcalc  --  get thermodynamics via FEP & BAR  ##
!     ##                                                            ##
!     ################################################################
!
!
...
160 continue
   nfrma = j
!
!     reset the file position to the beginning of trajectory B
!
   rewind (unit=ibar)
   next = 1
...
!
!     iterate the BAR equation to converge the free energy
!
   do while (.not. done)
      iter = iter + 1
      cold = cnew
