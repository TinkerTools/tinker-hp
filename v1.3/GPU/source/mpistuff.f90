...
subroutine commposrec
   ...
   use mpi
   use potent   ,only: use_pmecore,use_mlpot_only
...
!
   if (nproc.eq.1.or.use_mlpot_only) return
...
!
   ...
   subroutine allgather_pos( )
     ...
     integer i, j
     integer soff(nproc),rsiz(nproc),roff(nproc),req0,req1,off,off1,iglob
     integer tag,status(MPI_STATUS_SIZE),ierr
     integer   aglob(n)
     real(r_p) tbuff(s*n)

     if (nproc.eq.1.or.nbig_recep.eq.nproc) return
     if (deb_Path) print*, '  >>allgather_pos'

!$acc wait
!$acc data create(aglob,tbuff)

     soff(1) = 0
     do i = 2,nproc
        soff(i) = soff(i-1) + domlen(i-1)
     end do
     off  =   soff(rank+1)+1
     off1 = s*soff(rank+1)+1

!$acc parallel loop default(present)
     do i = 0,nloc-1
        iglob = glob(i+1)
        aglob( off + i     ) =   iglob
        tbuff( off1+ i*s   ) = x(iglob)
        tbuff( off1+ i*s+1 ) = y(iglob)
        tbuff( off1+ i*s+2 ) = z(iglob)
     end do

     rsiz(:) = s*domlen(1:nproc)
     roff(:) = s*  soff(1:nproc)

!$acc host_data use_device(aglob,tbuff)
     call MPI_Allgatherv(MPI_IN_PLACE,1,MPI_DATATYPE_NULL, aglob,domlen,soff,  MPI_INT,COMM_TINKER,ierr)
     call MPI_Allgatherv(MPI_IN_PLACE,1,MPI_DATATYPE_NULL, tbuff,  rsiz,roff,MPI_RPREC,COMM_TINKER,ierr)
!$acc end host_data
...
!
!     subroutine reassign: deal with particles changing of domain between two time steps
!
#ifdef _OPENACC
subroutine reassign
   implicit none
...
!
!     Assign all atoms to their real space domain
!
subroutine AllDirAssign
   ...
   use domdec
   use inform
   use mpistuff_inl,only: image_orth_inl,ctf_inl
   use potent
   use tinheader,only: prec_eps
   use timestat ,only: timer_eneig,quiet_timers,timer_enter,timer_exit
   implicit none
   integer i,j,k,iproc,nn
   integer nprocloc,rankloc,n_ar,n_br,n_cr
   real(t_p) xr,yr,zr,lena_box,lenb_box,lenc_box
...
!$acc parallel loop async default(present) copy(domlen)
   do i = 1, n
      ...
      if ((ycell2-abs(yr)).lt.eps_cell) yr= yr-sign(4*eps_cell,yr)
      if ((zcell2-abs(zr)).lt.eps_cell) zr= zr-sign(4*eps_cell,zr)
      n_ar  = int((xr+xcell2)/lena_box)
      n_br  = int((yr+ycell2)/lenb_box)
      n_cr  = int((zr+zcell2)/lenc_box)
...
!$acc wait
   ...
   ! Check Assignment
   nn=0;  do i= 1, nproc; nn= nn+ domlen(i); end do
   if (nn.ne.n) then
46    format(' ERROR AllDirAssign: rank ',I0,' Total atoms assigned ',I0)
      write(0,46) rank, nn
      __TINKER_FATAL__
   end if
...
!
!     Reciproqual space domain assignment of all atoms
!
...
subroutine reassignrespa(ialt,nalt)
   ...
   use boxes
   use cell
   use domdec,only : nloc,nproc,glob,comm_dir,rank,rank_bis,ndir,nxdd,nydd,nzdd&
             ,xbegproc,ybegproc,zbegproc,xendproc,yendproc,zendproc&
             ,repart,repartrec,loc,locrec,domlen&
             ,COMM_TINKER, nneig_send,nneig_recep,pneig_recep,pneig_send
   ...
   real(t_p) lena_box, lenb_box, lenc_box
...
!$acc parallel loop async
   do i = 1, nloc
      ...
      xtemp = xr
      ifind = .false.
      if ((xbox2-abs(xr)).lt.eps_cell) xr = xr-sign(4*eps_cell,xr)
      if ((ybox2-abs(yr)).lt.eps_cell) yr = yr-sign(4*eps_cell,yr)
      if ((zbox2-abs(zr)).lt.eps_cell) zr = zr-sign(4*eps_cell,zr)
      n_ar  = int((xr+xcell2)/lena_box)
      n_br  = int((yr+ycell2)/lenb_box)
      n_cr  = int((zr+zcell2)/lenc_box)
      iproc = n_ar + n_br*nxdd + n_cr*nydd*nxdd
      if (iproc.eq.rankloc) then
!$acc atomic capture
...
!$acc end atomic
         ...
         do ineighbor = 1, nneig_send
            if (iproc.eq.pneig_send(ineighbor)) then
!$acc atomic capture
...
!      end if ! (Find atom domain)
      ...
      if (.not.ifind) then
         print*,'Issue reassign',rank,iglob,iproc,real(xr,4),real(yr,4),real(zr,4)
      end if
   end do
...
!$acc loop vector
   ...
   !! send also aalt and aalt2 to have complete forces (including slow, int and fast) for ABF
      ...
      ! Recast workspace
      void_p = c_loc(data_send)
      call c_f_pointer(void_p,aalt_send,[max_atoms_send,nneig_send])
      void_p = c_loc(data_recv)
      call c_f_pointer(void_p,aalt_recv,[max_atoms_recv,nneig_recep])
...
!$acc host_data use_device(aalt_send, aalt_recv)
      do ineighbor = 1, nneig_send 
        call MPI_Isend(aalt_send(1,ineighbor),6*n_data_send (ineighbor) &
            &,MPI_TPREC, pneig_send(ineighbor), 0, COMM_TINKER, &
            & req_data_send(ineighbor), ierr)
      end do
      do ineighbor = 1, nneig_recep
        call MPI_Irecv(aalt_recv(1,ineighbor),6*n_data_recv (ineighbor)&
              & ,MPI_TPREC, pneig_recep(ineighbor), 0, COMM_TINKER,&
              & req_data_recv(ineighbor), ierr)
      end do
!$acc end host_data
       ...
       ! Wait all communications
      call MPI_Waitall(nneig_recep,req_data_recv ,MPI_STATUSES_IGNORE,ierr)
      call MPI_Waitall(nneig_send ,req_data_send ,MPI_STATUSES_IGNORE,ierr)
...
!
!     subroutine reassignrespa: deal with particles changing of domain between two time steps
!
...
!
   ...
   write(0,*) " ERROR! reassignrespa_old Routine Is Not Supposed To Be Called !"
    __TINKER_FATAL__
!
   call timer_enter( timer_eneig )
   if (use_pmecore) then
...
!
!     Reallocate some buffers if necessary
!
   call prmem_requestm(buffMpi_p1,3*nbloc)
   call prmem_requestm(buffMpi_p2,3*nloc)
   buffer (1:3,1:nbloc)=>buffMpi_p1(1:3*nbloc)
...
!
!
!     subroutine commforce_one : deal with communications of forces by bloc
!     between two time steps
!
...
! FIXME  temprec(1:3,1:max(1,nlocrec2))           => buffMpi_p3(1:sz3)
! FIXME  call sum_forces_rec1( temprec )
!        call comm_forces_rec( temprec )
!        call timer_enter( timer_dirreccomm )
!        if (ftot_l) then
!           call add_forces_rec_1d1( de_tot,temprec )
! FIXME  else
! FIXME     call add_forces_rec1_1d1( derivs,temprec )
!        end if
...
!
!     Second version of commforcesrec
!
...
!
!     subroutine Icommforceststgrad : deal with communications of
!                                     one force (de) for testgrad
!
...
!
   ...
   nbloc = count
!     nvdwblocloop is nvdwbloc if nvdwbloc is a multiple of 16, or the first one greater
...
!
!     Orderbuffer with openacc directives
!
...
!     call MPI_BARRIER(hostcomm,ierr)
!46   format(I3,'domlen',4I8,'bufbeg',2I8)
!     print 46, rank,domlen,domlenrec,bufbegrec
...
!        if (btest(tinkerdebug,0).and.nproc.eq.4) then
!22         format('___',A6,I5,3x,9I5)
!           write(0,22)
!    &      'domlenpole',rank,npoleloc,p_send1(1:4),p_recep1(1:4)
!        end if
!
!     MPI : begin reception
!
...
!        if (btest(tinkerdebug,0).and.nproc.eq.4) then
!22         format('___',A6,I5,3x,10I7)
!           write(0,22)
!    &      'domlen',rank,npoleloc,domlen(1:4),npolebloc,bufbegpole(1:4)
!        end if
!
!     MPI : begin reception
!
...
!      real(t_p) buffermpimu1(3,nrhs,max(npoleloc,1))
!      real(t_p) buffermpimu2(3,nrhs,max(1,npolerecloc))
...
!        if (btest(tinkerdebug,0).and.nproc.eq.4) then
!22         format('_domlenpole ',I5,I7,3x,5I7,' _')
!           write(0,22) rank,npoleloc,domlenpole(1:4)
!        end if
...
!        if (btest(tinkerdebug,0).and.nproc.eq.4) then
!22         format('_',A6,I5,3x,5I7,'_ ')
!           write(0,22) 'npoleloc',rank,npoleloc
!        end if
...
!
!     Subroutine commGridFront: Exchange FFtgrid between reciprocal process
!                 before call to fft_frontmpi
!
...
!        if (nrec_send.ge.1) then
!        do i = 1,n3mpimax
!           if (i.gt.3.and.i.le.n3mpimax-3) cycle
!           write(ich,'(I3)') i
!           call minmaxone(qgrid(1,1,1,i,2)
!    &          ,2*n1mpimax*n2mpimax,'qg'//ich)
!        end do
!        end if
...
!
!     Subroutine commGridBack: Exchange FFtgrid between reciprocal process
!                 after call to fft_frontmpi
!
