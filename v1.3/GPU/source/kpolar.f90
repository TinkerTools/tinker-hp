      ...
      if ((use_polar .or. use_repuls)) then
         ...
         do i = 1, n
            if (polsiz(i).ne.0 .or. polarity(i).ne.0.0_ti_p) then
               ...
               if (tholed(i) .ne. 0.0_ti_p)  use_tholed = .true.
...
!
!       Display if some poles does not have polarity
!
...
!
!       copy original polarizability values that won't change during mutation
!
...
!
!       initialization for TCG and omega fit
!
...
!
!  allocate predictor arrays
!
      if (use_polar.and.use_pred) then
         if (maxualt.eq.-1) then
            if (polpred.eq.'SVLS') then
               maxualt= merge(2, 5, index(integrate,'RESPA1').gt.0)
            else
               maxualt= merge(2, 6, index(integrate,'RESPA1').gt.0)
            end if
         end if
         ...
         d_altbuf => d_altbuf
#else
         call prmem_request(udpalt,6*n*maxualt)
#endif
...
#else
            call prmem_request(udpalt1,6*n*maxualt)
#endif
         end if
         if (deb_Path) then
            call mem_get(m1,m3)
...
!
!          initialize prior values of induced dipole moments
!
#ifdef USE_NVSHMEM_CUDA
...
#else
         call set_to_zero1(udpalt,6*n*maxualt,rec_queue)
#endif

         if ((integrate.eq.'RESPA1').or.(integrate.eq.'BAOABRESPA1')&
         .or.centroid_longrange&
         .and.(.not.use_pmecore.or.use_pmecore.and.rank.lt.ndir)) then
#ifdef USE_NVSHMEM_CUDA
...
#else
            call set_to_zero1(udpalt1,6*n*maxualt,rec_queue)
#endif
         end if
      end if
!
...
!$acc  polsiz,poleglobnl,polelocnl)
   do i = 1, nlocnl
      iglob  = ineignl(i)
      iipole = pollist(iglob)
!
...
!
   npolelocloop = merge( npoleloc,&
      ...
      &(mod(npoleloc,16).eq.0))
!
!     comput nZ_Onlyloc if necessary
!
...
!
!     npolelocnlloop is npolelocnl if npolelocnl is a multiple of 16
!                    or the first one greater
!     npolelocnlloop = merge( npolelocnl,
!    &                     (int(npolelocnl/16)+1)*16,
!    &                     (mod(npolelocnl,16).eq.0))

!     npoleblocloop = merge( npolebloc,
!    &                    (int(npolebloc/16)+1)*16,
!    &                    (mod(npolebloc,16).eq.0))
end
...
!$acc end host_data
!
!     Wait for poleglob communication request to end
!     And Fill the missing part of it
!
...
!     call upload_device_polar(0)
end

!
!     Update polar's data on device
!
...
!!$acc update device(ip11(:,:),np11(:),ip12(:,:),np12(:),
!!$acc              ip13(:,:),np13(:),ip14(:,:),np14(:))
   else
!$acc update device(thole,pdamp,tholed,polarity)
      if (use_lambdadyn) then
...
12 format(2x,'dealloc_shared_polar')
   ...
   call shmem_request(tholed, wintholed, [0],config=mhostacc)

end subroutine
!
!     subroutine alloc_shared_polar : allocate shared memory pointers for polar
!     parameter arrays
!
...
!
   ...
   call shmem_request(tholed, wintholed, [n],config=mhostacc)
end
