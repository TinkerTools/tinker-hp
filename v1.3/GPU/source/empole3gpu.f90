...
!     ########################################################################
!     ##                                                                    ##
!     ##  subroutine empole3cgpu     --  Ewald multipole analysis via list  ##
!     ##                                                                    ##
!     ########################################################################
!
!
...
!     ########################################################################
!     ##                                                                    ##
!     ##  subroutine elambdampole3cgpu     --  Ewald multipole analysis via list  ##
!     ##                                                                    ##
!     ########################################################################
!
!
!     "empole3d" calculates the atomic multipole interaction energy
!     using particle mesh Ewald summation and a neighbor list, and
!     partitions the energy among the atoms, when lambdadyn is activated
!
!
...
!
   ...
   if ((.not.(use_pmecore)).or.(use_pmecore).and.(rank.gt.ndir-1))&
      ...
      call timer_enter( timer_rec )
!
!        the reciprocal part is interpolated between 0 and 1
!
      elambda = 0.0
      call altelec(altopt)
      call rotpolegpu
...
!     ######################################################################
!     ##                                                                  ##
!     ##  subroutine emreal3dgpu  --  real space mpole analysis via list  ##
!     ##                                                                  ##
!     ######################################################################
!
!
...
!        if (use_lambdadyn) delamdbae=delamdbae+delambdae_
   end do
!
!     compute the real space portion of the Ewald summation
!
...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine emrecipgpu  --  PME recip space multipole energy  ##
!     ##                                                               ##
!     ###################################################################
!
!
!     "emrecipgpu" evaluates on device the reciprocal space portion of the particle
...
!
   ...
   f = electric / dielec
!
!     dynamic allocation of global arrays
!
   j = max(npolerecloc,1)
   call mallocMpiGrid
