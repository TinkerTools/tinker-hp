...
subroutine nblist(istep)
   use atoms ,only: n
   use mlpot ,only: mlpot_rfield, amloc, namloc
   use boxes ,only: xbox2,ybox2,zbox2
   use cutoff,only: use_clist,use_mlist,use_vlist,use_dlist&
      &,use_shortvlist,use_shortclist,use_shortmlist&
      &,use_shortdlist&
      &,vdwcut,chgcut,mpolecut,dispcut
   use domdec
   ...
   use neigh ,only: mbuf2,cbuf2,vbuf2,dbuf2,lbuffer&
      &, build_pairwise_list,list_mlpot,ineignl
   use potent
...
!
!     check the consistency of the cutoffs+buffer compared to the size
!     of the box (minimum image convention)
!
1000 format('Error in ',A,' neigbor list: max cutoff + buffer should'&
      &,' be less than half one edge of the box')
1010 format(A12,' cutoff = ',F14.3)
1020 format(A8,'List buffer = ',F14.3)
   boxedge2 = min(xbox2,ybox2,zbox2)
   if (use_clist.and.cbuf2.gt.boxedge2*boxedge2) then
      if (rank.eq.0) then
         write(iout,1000) 'charge'
         write(iout,1010) 'charge',chgcut
         write(iout,1020) lbuffer
      end if
      call fatal
   end if
   if (use_mlist.and.mbuf2.gt.boxedge2*boxedge2) then
      if (rank.eq.0) then
         write(iout,1000) 'electrostatic'
         write(iout,1010) 'Multipole',mpolecut
         write(iout,1020) lbuffer
      end if
      call fatal
   end if
   if (use_vlist.and.vbuf2.gt.boxedge2*boxedge2) then
      if (rank.eq.0) then
         write(iout,1000) 'VDW'
         write(iout,1010) 'VDW',vdwcut
         write(iout,1020) lbuffer
      end if
      call fatal
   end if
   if (use_dlist.and.dbuf2.gt.boxedge2*boxedge2) then
      if (rank.eq.0) then
         write(iout,1000) 'dispersion'
         write(iout,1010) 'dispersion',dispcut
         write(iout,1020) lbuffer
      end if
      call fatal
   end if
...
!
   ...
   if (use_shortdlist) then
      if (vlst_en) call dlistcell2
   else if (use_dlist) then
      if (vlst_en) call dlistcell
   end if
   ...
   if (use_mlpot) then
      call build_pairwise_list(amloc, namloc&
         &,(mlpot_rfield+lbuffer)**2 ,list_mlpot)
   end if

   !call save_atoms_nl

   ! Disable Extra allocation
!     if (extra_alloc.and.istep.gt.1.and.mod(istep,2*ineigup).eq.0) then
!        if (debMem.ne.0) print*, 'Disable extra allocation',rank
!        extra_alloc=.false.
!     end if
...
subroutine check_nl_rebuild()
   ...
   if (nproc.lt.4) return

   cap = 0
!$acc wait(rec_queue)
!$acc parallel loop async(rec_queue) copy(cap)
   do ii = 1,nlocnl
      i = loc(ineignl(ii))
      if (i.lt.1.or.i.gt.nbloc) then
!$acc atomic update
         cap = cap+1
!$acc end atomic
      end if
   ...
   if (cap.gt.0) then
 23   format(' I rank',I0,' detected', I0,' out-of-scope atoms from Neighbor List Atoms')
      write(*, 23) rank,cap
   end if
...
subroutine reinitnl(istep)
   ...
   use mlpot   ,only: mlpot_rfield
   use neigh
   use potent  ,only: use_mlpot
   use utilgpu ,only: rec_queue
...
!
   ...
   !vbuf      = sqrt(vbuf2)
   !dbuf      = sqrt(dbuf2)
   bigbuf    = max( vdwcut,mpolecut,chgcut,dispcut )
...
!
   ...
   if (.not.allocated(ineignl)) then
      if(use_mlpot) call init_aboxPart(list_mlpot%bPar)
      call prmem_request(ineignl,n)
!$acc wait
   end if

   !call check_nl_rebuild

   if (no_commdir) then
      distcut = bigbuf + lbuffer
   else
      distcut = bigbuf/2 + lbuffer
   end if
   if (use_mlpot) then
      distcut = max(distcut, mlpot_rfield+lbuffer)
   end if

   call ddbuild_gpu(distcut)
   goto 66
...
14 format(' reinitnl  -rank ',I0,' -nloc ',I0,' -nlocnl ',I0)
   write(*,14) rank,nloc,nlocnl
