...
30             format (1x,4i4,1x,6(f6.2,i4))
               ...
               if (rank.eq.0) write (iout,40)  ia,ib,ic,id,&
...
!
!     use special torsional parameter assignment method for MMFF
!
...
120         format (/,' Undefined Torsional Parameters :',&
         ...
         if (iring .eq. 5)  label = '5-Ring '
         if (iring .eq. 4)  label = '4-Ring '
         if (rank.eq.0) write (iout,130)  label,ia,name(ia),ib,&
            &name(ib),ic,&
...
!
   ...
   if (.not.use_tors) return

   call upload_device_ktors
...
!     ###############################################################
!     ##                                                           ##
!     ##  subroutine ktorsm  --  assign MMFF torsional parameters  ##
!     ##                                                           ##
!     ###############################################################
!
!
...
!     Scope, Parametrization, and Performance of MMFF94", Journal of
!     Computational Chemistry, 17, 490-519 (1995)
!
!     T. A. Halgren, "Merck Molecular Force Field. V. Extension of
!     MMFF94 Using Experimental Data, Additional Computational Data,
!     and Empirical Rules", Journal of Computational Chemistry, 17,
!     616-641 (1995)
!
!
subroutine ktorsm
...
!
!     assign MMFF torsional parameters for each torsional angle
!
   do i = 1, ntors
      ia = itors(1,i)
      ib = itors(2,i)
...
10    continue
!
!     determine the atom class equivalency assignments
!
      mclass = mclass + 1
      if (mclass .eq. 1) then
...
!
!     construct search string and zero out parameters
!
      size = 4
      call numeral (ita,pa,size)
      call numeral (itb,pb,size)
...
!
!     set the MMFF torsion type attribution
!
      ab = 0
      if (ia .le. ib) then
         do j = 1, nlignes
...
!
!     condition below deduced from validation suite comparison
!
      if ((ab.eq.1 .and. (mltb(class(ic)).eq.0.or.&
         ...
         if (tors1(1,i) .eq. 1000.0_ti_p)  done = .false.
         if (tors1(2,i) .eq. 1000.0_ti_p)  done = .false.
         if (tors2(1,i) .eq. 1000.0_ti_p)  done = .false.
         if (tors2(2,i) .eq. 1000.0_ti_p)  done = .false.
         if (tors3(1,i) .eq. 1000.0_ti_p)  done = .false.
         if (tors3(2,i) .eq. 1000.0_ti_p)  done = .false.
         goto 20
...
20    continue
!
!     use the empirical rules for parameter not located
!
      if (.not. done) then
         ita = class(ia)
         itb = class(ib)
...
!
!     upload ktors's data on device
!
 ! absence of test on use_tors is due to facte that those
...
12 format(2x,'upload_device_ktors')
   ...
   c_sz  = size( c_tors1(mype)%pel )
   c_szt = 4*ntors

   call shmem_update_device(tors1,c_szt,&
      &dst=c_tors1(mype)%pel,nd=c_sz,config=mhostnvsh)
   call shmem_update_device(tors2,c_szt,&
      &dst=c_tors2(mype)%pel,nd=c_sz,config=mhostnvsh)
   call shmem_update_device(tors3,c_szt,&
      &dst=c_tors3(mype)%pel,nd=c_sz,config=mhostnvsh)
   call shmem_update_device(tors4,c_szt,&
      &dst=c_tors4(mype)%pel,nd=c_sz,config=mhostnvsh)
   call shmem_update_device(tors5,c_szt,&
      &dst=c_tors5(mype)%pel,nd=c_sz,config=mhostnvsh)
   call shmem_update_device(tors6,c_szt,&
      &dst=c_tors6(mype)%pel,nd=c_sz,config=mhostnvsh)
#else
!$acc update device(tors1,tors2,tors3,tors4,tors5,tors6)
#endif
...
12 format(2x,'delete_data_ktors')
   ...
   call shmem_request(tors1,wintors1,[0,0],c_tors1,d_tors1,&
      &config=mhostnvsh)
   call shmem_request(tors2,wintors2,[0,0],c_tors2,d_tors2,&
      &config=mhostnvsh)
   call shmem_request(tors3,wintors3,[0,0],c_tors3,d_tors3,&
      &config=mhostnvsh)
   call shmem_request(tors4,wintors4,[0,0],c_tors4,d_tors4,&
      &config=mhostnvsh)
   call shmem_request(tors5,wintors5,[0,0],c_tors5,d_tors5,&
      &config=mhostnvsh)
   call shmem_request(tors6,wintors6,[0,0],c_tors6,d_tors6,&
      &config=mhostnvsh)
#else
   call shmem_request(tors1,wintors1,[0,0],config=mhostacc)
   call shmem_request(tors2,wintors2,[0,0],config=mhostacc)
   call shmem_request(tors3,wintors3,[0,0],config=mhostacc)
   call shmem_request(tors4,wintors4,[0,0],config=mhostacc)
   call shmem_request(tors5,wintors5,[0,0],config=mhostacc)
   call shmem_request(tors6,wintors6,[0,0],config=mhostacc)
#endif
end subroutine
