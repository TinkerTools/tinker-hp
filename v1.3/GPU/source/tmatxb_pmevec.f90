...
!
   ...
   integer iploc
   real(t_p) cutoff2
   real(t_p) pdi, pti ,pol
   real(t_p) alsq2, alsq2n,half,one
   real(t_p) xi,yi,zi
   real(t_p) time0, time1
! DIR$ ATTRIBUTES ALIGN:64::mu,efi
   real(t_p)  mu(3,nrhs,npolebloc), efi(3,nrhs,npolebloc)

!DIR$ ATTRIBUTES ALIGN:64::mu11,efi11
   real(t_p)  mu11(nblocloop), efi11(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64::mu21,efi21
   real(t_p)  mu21(nblocloop), efi21(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64::mu31,efi31
   real(t_p)  mu31(nblocloop), efi31(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64::mu12,efi12
   real(t_p)  mu12(nblocloop), efi12(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64::mu22,efi22
   real(t_p)  mu22(nblocloop), efi22(nblocloop)
!DIR$ ATTRIBUTES ALIGN:64::mu32,efi32
   real(t_p)  mu32(nblocloop), efi32(nblocloop)
   ...
   if(rank.eq.0.and.tinkerdebug)&
      &write(*,*) 'tmatxb_pmevec', npolebloc,npoleloc,&
      &npoleblocloop,npolelocloop

   call timer_enter( timer_tmatxb_pmevec )

!
!     initialize the mu and the result vectors
!
...
!
      if (polarity(iipole).eq.0.0_ti_p) cycle MAINLOOP
      iglob = ipole  (iipole)
      i     = loc    (iglob)
...
#ifdef SINGLE
!DIR$ ASSUME (mod(neloop8,8).eq.0)
      call vserfc(neloop8,ralphavec,bn0vec)
#elif defined(MIXED)
      call vserfc(neloop8,ralphavec,bn0vec)
#else
...
!
         rr3vec(k) =         (1.0_ti_p -&
            &(1.0_ti_p - expdampvec1(k))&
            &* uscalevec(k))&
            &* invd1vec(k) * invd2vec(k)
         rr5vec(k) = 3.0_ti_p * (1.0_ti_p -&
            &(1.0_ti_p - expdampvec1(k)&
            &* (1.0_ti_p - dampvec1(k)))&
            &* uscalevec(k))&
...
!
            if (polarity(iipole).eq.0.0_ti_p) then
               pol = tinypol ** (-1)
            else
...
!!DIR$ VECTOR ALIGNED
   ...
   call timer_exit( timer_tmatxb_pmevec )
end
