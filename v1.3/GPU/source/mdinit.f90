...
#endif
!
!      set default for neighbor list update,
!
   if (.not.isothermal.and. .not.isobaric) then  !Microcanonical (NVE)
!        reference = 2\AA buffer allows 30fs integration in double precision
!        reference = 0.7\AA buffer allows 10.5fs integration in mixed or single precision
      ineigup= max( 1,int(((real(lbuffer,r_p)*0.015_re_p)/dt)+1d-3) )
      call fetchkey('POLAR-EPS',poleps,oe6)
   else
!        reference = 2\AA buffer allows 40fs integration in double precision
!        reference = 0.7\AA buffer allows 14fs integration in mixed or single precision
...
!
   ...
   adaptive_qtb=.false.
!
!     set default values for lambda dynamic
!
!      use_lambdadyn = .false.
   use_OSRW = .false.
...
!          call upcase (volscale)
   ...
   if (isobaric .and. any(freeze_axis)) then
      anisotrop = .true.
      if (rank == 0) then
         write(*,*) "SEMI-ISOTROPIC freeze a,b,c=",freeze_axis
      endif
      if (all(freeze_axis)) then
         if (rank == 0) then
            write(0,*) "Error: all axes frozen"
         endif
         call fatal
      endif
   end if

   if (isobaric .and. anisotrop .and. barostat == "MONTECARLO") then
      if (rank == 0) then
         write(*,*) "Error: Monte Carlo barostat not compatible with anisotropic pressure"
      endif
      call fatal
   endif
!
!     enforce the use of monte-carlo barostat with the TCG family of solvers
!
...
!
!     default time steps for respa and respa1 integrators
!
...
!
!     keywords for respa and respa1 integrators
!
   do i = 1, nkey
      next = 1
      record = keyline(i)
...
!
!     check for use of Monte Carlo barostat with constraints
!
...
!
!  semi-isotropic barostat not compatible with monte-carlo barostat
!
   if (barostat.eq.'MONTECARLO' .and. any(freeze_axis)) then
         if (rank == 0) then
            write(0,*) "Monte Carlo barostat not compatible with semi-isotropic pressure control"
         endif
         __TINKER_FATAL__
   end if
!
!     Plumed feature initialisation
!
   if (lplumed) call plumed_init(dt)
!
!     nblist initialization for respa-n integrators
!
...
!
!     initialization for Langevin dynamics
!
...
!
!     Initiate Track of MD_state
!     ! Do not move that call upstream
!
   if (app_id.eq.dynamic_a) call mds_init
!
!     Colvars Feature Initialization
!
   if (use_reps.and.use_lambdadyn) call lambda_init_reps
   call colvars_init(dt)
!
!     lambda dynamic initialization
!
...
!
!     check wether or not to compute virial
!
...
#endif
!
!     decide whether to remove center of mass motion
!
...
!
!     Reinterpret force buffer shape (3,nloc) --> (nloc,3)
!
...
#endif
!
!     set velocities and fast/slow accelerations for RESPA method
!
...
!
!     set velocities and fast/inter/slow accelerations for RESPA-n method
!
   else if ((integrate.eq.'RESPA1').or.(integrate.eq.'BAOABRESPA1'))&
      &then
...
!
!     Re-Enable COLVARS if necessary
!
