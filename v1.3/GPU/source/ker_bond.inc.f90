...
&,ded&
   ...
   &,ver,fea)
!$acc routine
use bndpot    ,only: BND_HARMONIC,BND_MORSE,BND_MORSE4,BND_FLATBOTTOM
...
!     with alpha = sqrt(ForceConst/BDE)
...
!
   ...
   if (dt > 0.0_ti_p) then
      ...
      IF (IAND(ver,ene).NE.0)  e = 0.0_ti_p
      IF (IAND(ver,grd+vir).NE.0) deddt = 0.0_ti_p
   endif
end if
!
