...
inline static void gpuAssert(cublasStatus_t code, const char* file, int line){
   if (code!=CUBLAS_STATUS_SUCCESS){
      fprintf(stderr,"GPUAssert: cuBlas Error : %d %s %d\n", code, file, line);
      exit(code);
   }
}
...
#define gpuErrchkBlas(ans) { gpuAssert((ans), __FILE__, __LINE__); }
