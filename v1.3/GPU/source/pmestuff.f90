...
!
            ...
            fr = real(nfft3,t_p) * (wbeg-anint(wbeg)+0.5_ti_p)
            ifr = int(fr-pme_eps)
            !if (fr-ifr.eq.0.0.and.ifr>0) ifr = ifr-1
            kbeg = ifr - bsorder + grdoff
            wend = (zendproc(iproc1+1))/zbox
            fr = real(nfft3,t_p) * (wend-anint(wend)+0.5_ti_p)
            ifr = int(fr-pme_eps)
            !if (fr-ifr.eq.0.0.and.ifr>0) ifr = ifr-1
...
!
            wbeg = (ybegproc(iproc1+1))/real(ybox,t_p)
            fr = real(nfft2,t_p) * (wbeg-anint(wbeg)+0.5_ti_p)
            ifr = int(fr-pme_eps)
            !if (fr-ifr.eq.0.0.and.ifr>0) ifr = ifr-1
            jbeg = ifr - bsorder + grdoff
            wend = (yendproc(iproc1+1))/real(ybox,t_p)
            fr = real(nfft2,t_p) * (wend-anint(wend)+0.5_ti_p)
            ifr = int(fr-pme_eps)
            !if (fr-ifr.eq.0.0.and.ifr>0) ifr = ifr-1
...
#endif
!
!     Associate 1D grid pointer with his 5D grid target
!
...
!
!     send and receive repartrectemp
!
...
! 43  format( "I rank",I2," nc(",I2,") bl(",I7,") stri("
!    &      ,I8,") GrSize (",I9,") MPI_TYPE:",I3)
!     write(*,43) rank,ncount,block,stride,gridin_size
!    &           ,MPI_GRID1D
end subroutine
!
