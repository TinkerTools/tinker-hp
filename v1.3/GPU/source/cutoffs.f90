...
subroutine cutoffs
   use atoms  ,only: n
   use mlpot    ,only: mlpotcut,mlpot_rfield
   use bound
   use cutoff
   use domdec
   use keys
...
!
      ...
      else if (keyword(1:13) .eq. 'MLPOT-RFIELD ') then
         read (string,*,err=10,end=10)  mlpot_rfield
!
...
!
!     keyword to include another (bigger than the non bonded ones) cuto.f90 in the
...
100   continue
   ...
   mlpot_rfield = max(mlpot_rfield,mlpotcut)
