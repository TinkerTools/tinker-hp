...
!
               call front_convert_base(0,ita,0,0,itd,pt6)
               call front_convert_base(0,ita,0,0,itc,pt5)
               call front_convert_base(0,ita,0,0,itb,pt4)
               call front_convert_base(0,ita,itd,0,0,pt3)
               call front_convert_base(0,ita,itc,0,0,pt2)
               call front_convert_base(0,ita,itb,0,0,pt1)
               ...
               if (.not. done) then
                  do j = 1, ndi
                     if (kdi(j).eq.pt1 .or. kdi(j).eq.pt4) then
                        ...
                        vprop(niprop) = tdi(j)
                        ! Build system kdi
                        if (.not.is_find8(kdi_sys(1),ndis,kdi(j))) then
                           ndis = ndis + 1
                           kdi_sys(ndis) = kdi(j)
                        end if
...
!$acc end atomic
  ...
  61        continue
            if (.not.done) then
               do j = 1, ndis
                  if (kdi_sys(j).eq.pt1 .or. kdi_sys(j).eq.pt4) then
!$acc atomic capture
                     niproploc  = niproploc + 1
                     capt       = niproploc
!$acc end atomic
                     niproploc1 = niproploc1 + 1
                     impropglob(capt)=impropcount +niproploc1
                  else if (kdi_sys(j).eq.pt2 .or. kdi_sys(j).eq.pt5) then
!$acc atomic capture
                     niproploc  = niproploc + 1
                     capt       = niproploc
!$acc end atomic
                     niproploc1 = niproploc1 + 1
                     impropglob(capt)=impropcount +niproploc1
                  else if (kdi_sys(j).eq.pt3 .or. kdi_sys(j).eq.pt6) then
!$acc atomic capture
                     niproploc  = niproploc + 1
                     capt       = niproploc
!$acc end atomic
