...
!      write(*,*) walpha.ne.1.0_ti_p, wbeta.ne.1.0_ti_p
!     & , wgamma.ne.1.0_ti_p, compute_group
   if(first_in) then
...
!
!     add scaled energies and forces
!
...
!$acc end serial
      ...
      if(dograd) then
!!$acc update host(demgroup,locgroup,loclocgroup,emgroup) async
!!$acc wait
!          i=locgroup(loclocgroup(1))
!          j=locgroup(loclocgroup(12))
!          write(*,'(f15.6,A,2f15.6)') emgroup,"  demgroup"
!     &       ,demgroup(1,i),demgroup(3,j)
