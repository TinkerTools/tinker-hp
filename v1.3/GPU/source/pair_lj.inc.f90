...
!
!     ------------------------------------------------------
!     Lennard-Jones pairwise interaction computation routine
!     ------------------------------------------------------
!
M_subroutine &
    duo_lj_(rik2,rik,rik_i,rv,eps,cut2,scut &
...
real(t_p),intent(in ):: rik2,rik,rik_i,rv,eps,cut2,scut&
...
IF (IAND(fea,sfc).NE.0.and.mutik.eq.one1) then
   ...
   dgsclambda = -(scs/sck)*rvk*galpha*(glamb**(scs-1.0))*gsc**(1.0-sck)
!
...
IF (iand(fea,shr+lgr).ne.0) &
    call switch_respa_inl(rik,scut,sheal,s,ds)
...
M_subroutine &
    duo_lj(rik2,xr,yr,zr,rv,eps,cut2,scut &
    ,rinv,off,sheal,ugrp,fgrp,mutik &
    ,sck,sct,scs,scalpha,galpha,ulamdyn &
    ,lambda,vlambda,glamb,lambdavt,lambdavt1 &
...
integer  ,intent(in ):: ver,fea
integer(1),intent(in ):: mutik
real(t_p),intent(in ):: rik2,xr,yr,zr,rv,eps,cut2,scut &
    ,rinv,off,sheal,fgrp,sct,sck,scs &
    ,scalpha,lambda,vlambda,lambdavt,lambdavt1 &
    ,galpha,glamb
...
call duo_lj_(rik2,rik,rik_i,rv,eps,cut2,scut &
    ,rinv,off,sheal,ugrp,fgrp,mutik &
    ,sck,sct,scs,scalpha,galpha,ulamdyn &
    ,lambda,vlambda,glamb,lambdavt,lambdavt1 &
    ,delambdav,e,de,ver,fea)
