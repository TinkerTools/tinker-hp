...
!     loop over the k blocks and perform a cholesky factorization
   do k = 1, km
      mdim = npergrp(k)*3
      if(mdim .gt. 0)then
         knd = kofst(k) + mdim*(mdim+1)/2
...
!
      ...
      do kkk = 1, nnelst
         ...
         kglob = ipole(kkpole)
!
!     only build induced field for interactions outside of a block
!     check if the atoms are in the same block
!
...
!      write(*,*) 'n_recep1=',n_recep1,'p_recep1=',p_recep1
!      write(*,*) 'n_send1=',n_send1,'p_send1=',p_send1
!
   do i = 1, n_send1
