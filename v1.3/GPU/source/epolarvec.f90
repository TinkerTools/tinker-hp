...
!     ######################################################################
!     ##                                                                  ##
!     ##  subroutine epolar0cvec  --  Ewald polarization derivs via list  ##
!     ##     o                                                            ##
!     ######################################################################
!
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine epreal0cvec  --  real space polar energy via list  ##
!     ##                                                                ##
!     ####################################################################
!
!
...
!
   ...
   call switch (mode)
!
!     compute the dipole polarization energy component
!
...
!old     ci = rpole(1,iipole)
      ci = rpole(1,iipole) !
!old     dix = rpole(2,iipole)
!old     diy = rpole(3,iipole)
!old     diz = rpole(4,iipole)
!old     qixx = rpole(5,iipole)
!old     qixy = rpole(6,iipole)
!old     qixz = rpole(7,iipole)
!old     qiyy = rpole(9,iipole)
!old     qiyz = rpole(10,iipole)
!old     qizz = rpole(13,iipole)
!        divec is dix, diy, diz
      divec = rpole(2:4,iipole)
!
!        qivec  is qixx, qixy, qixz
!               qixy, qiyy, qiyz
!               qixz, qiyz, qizz
      ...
      qivec= reshape ( rpole( (/ 5,6,7,6,9,10,7,10,13 /), iipole),&
         &(/3,3/)&
         &)
!old     uix = uind(1,iipole)
!old     uiy = uind(2,iipole)
!old     uiz = uind(3,iipole)
!        ui is  uix, uiy, uiz
      uivec = uind(1:3,iipole)


!old     do j = 1, n12(iglob)
!old        pscale(i12(j,iglob)) = p2scale
!old     end do
!old     do j = 1, n13(iglob)
!old        pscale(i13(j,iglob)) = p3scale
!old     end do
!old     do j = 1, n14(iglob)
!old        pscale(i14(j,iglob)) = p4scale
!old        do k = 1, np11(iglob)
!old            if (i14(j,iglob) .eq. ip11(k,iglob))
!old &            pscale(i14(j,iglob)) = p4scale * p41scale
!old        end do
!old     end do
!old     do j = 1, n15(iglob)
!old        pscale(i15(j,iglob)) = p5scale
!old     end do
      nnp11 = np11(iglob)
      nn12  = n12(iglob)
      nn13  = n13(iglob)
...
!
!old    do kkk = 1, nelst(ii)
!old       kkpole = elst(kkk,ii)
!old       kglob = ipole(kkpole)
!old       xr = x(kglob) - xi
!old       yr = y(kglob) - yi
!old       zr = z(kglob) - zi
!old       if (use_bounds)  call image (xr,yr,zr)
!old       r2 = xr*xr + yr*yr + zr*zr
      nnelst =nelst(ii)
...
!old           r = sqrt(r2)
!old           ck = rpole(1,kkpole)
!old           dkx = rpole(2,kkpole)
!old           dky = rpole(3,kkpole)
!old           dkz = rpole(4,kkpole)
!old           qkxx = rpole(5,kkpole)
!old           qkxy = rpole(6,kkpole)
!old           qkxz = rpole(7,kkpole)
!old           qkyy = rpole(9,kkpole)
!old           qkyz = rpole(10,kkpole)
!old           qkzz = rpole(13,kkpole)
!old           ukx = uind(1,kkpole)
!old           uky = uind(2,kkpole)
!old           ukz = uind(3,kkpole)
      rvec(1:nnelst1)      = sqrt(r2vec1(1:nnelst1))
      ckvec(1:nnelst1)     = rpole(1,kkpolevec1(1:nnelst1)) !
!
!        dkvec is  dkx, dky, dkz
!
...
!
!            qkvec is  qkxx, qkxy, qkxz
!                      qkxy, qkyy, qkyz
!                      qkxz, qkyz, qkzz
!new     !DIR$ ASSUME_ALIGNED qkvec:64
!new     !DIR$ ASSUME_ALIGNED kkpolevec1:64
...
!old           rr1 = f / r
!old           rr3 = rr1 / r2
!old           rr5 = 3.0_ti_p * rr3 / r2
!old           rr7 = 5.0_ti_p * rr5 / r2
...
!
!old           ralpha = aewald * r
!old           bn(0) = erfc(ralpha) / r
!old           alsq2 = 2.0_ti_p * aewald**2
!old           alsq2n = 0.0_ti_p
!old           if (aewald .gt. 0.0_ti_p)  alsq2n = 1.0_ti_p / (sqrtpi*aewald)
!old           exp2a = exp(-ralpha**2)
!old           do j = 1, 3
!old              bfac = real(j+j-1,t_p)
!old              alsq2n = alsq2 * alsq2n
!old              bn(j) = (bfac*bn(j-1)+alsq2n*exp2a) / r2
!old           end do
!old           do j = 0, 3
!old              bn(j) = f * bn(j)
!old           end do
...
!
!     intermediates involving Thole damping and scale factors
!
...
!
!     calculate intermediate terms for polarization interaction
!
...
!
!     increment the overall polarization energy components
!
      ep = ep + e
!
...
!     ######################################################################
!     ##                                                                  ##
!     ##  subroutine eprecipvec  --  PME recip space polarization energy  ##
!     ##                                                                  ##
!     ######################################################################
!
!
!     "eprecipvec" evaluates the reciprocal space portion of particle
