...
!     J. Chem. Theory Comput., 2015, 11 (6), pp 2589�~@~S2599
!
...
!
   if (polalg.eq.1) then
      if (rank.le.ndir-1) then
         call inducepcg_pmevec(tmatxb_pmevec,nrhs,.true.,ef,mu,xx)
      else
         call inducepcg_pmevec(tmatxb_pmevec,nrhs,.true.,xx,xx,murec)
      end if
   else if (polalg.eq.2) then
      if (rank.le.ndir-1) then
         call inducejac_pmevec(tmatxb_pmevec,nrhs,.true.,ef,mu,xx)
      else
         call inducejac_pmevec(tmatxb_pmevec,nrhs,.true.,xx,xx,murec)
      end if
   else
...
!
subroutine inducepcg_pmevec(matvec,nrhs,precnd,ef,mu,murec)
   use atmlst
   use domdec
   use ewald
   use iounit
...
!old  real(t_p)  ef(max(1,npolebloc),3,nrhs), mu(3,nrhs,*), murec(3,nrhs,*)
   logical precnd
...
!
subroutine inducejac_pmevec(matvec,nrhs,dodiis,ef,mu,murec)
   use atmlst
   use domdec
   use ewald
   use iounit
...
subroutine efld0_recipvec(cphi)
!
...
!
!
subroutine tmatxbrecipvec(mu,murec,nrhs,dipfield,dipfieldbis)
!
