...
!
!     "empole1" : driver for calculation of the multipole and dipole polarization
...
!
!     "elec_calc_reset" reset global data for electrostatics calculations
!
...
#ifdef _OPENACC
!
!     Reorder Atom-Atom neigbhor list
!
...
!
   aewald = aeewald
!
!     zero out the atomic multipole energy and derivatives
!     reset lambda elec forces for lambda dynamics
!
...
!
!     save delambdae for short range computation
!
...
!
!     modify gradient and virial for charge flux self-energy
!
         if (use_chgflx) then
...
#endif
!
!     Add both contribution to the energy
!
...
!     #################################################################
!     ##                                                             ##
!     ##  subroutine emreal1c  --  Ewald real space derivs via list  ##
!     ##                                                             ##
!     #################################################################
!
!
...
subroutine emreal1c_
   ...
   use utilgpu    ,only: dir_queue,def_queue,rec_queue,mem_set&
      ...
      &,pot=>ug_workS_r
#ifdef _OPENACC
   use utilgpu    ,only: rec_event,stream_wait_async
#endif
...
#endif
!
!     calculate multipole potentials
!
...
subroutine emreal1ca_ac(tem,vxx,vxy,vxz,vyy,vyz,vzz)
   ...
   use empole1gpu_inl ,only: tp2enr
#endif
...
!
!     call C wrapper to compute the real space portion of the Ewald summation
!
   def_stream = dir_stream
...
!     #######################################################################
!     ##                                                                   ##
!     ##  subroutine emrecip1gpu  --  PME recip multipole energy & derivs  ##
!     ##                                                                   ##
!     #######################################################################
!
!
...
!
!     assign permanent multipoles to PME grid and perform
!     the 3-D FFT forward transformation
!
...
!
!     save the virial for use in polarization computation
!
...
!     nff   = 2*isize2(rankloc+1)*jsize2(rankloc+1)*ksize2(rankloc+1)
!     time0 = comput_norm(qgridout_2d,nff,0)
!     time1 = comput_norm(qgridout_2d,nff,1)
!     call MPI_ALLREDUCE(MPI_in_place,time0,1,MPI_real8,MPI_MAX,
!    &     COMM_TINKER,i)
!     call MPI_ALLREDUCE(MPI_in_place,time1,1,MPI_real8,MPI_SUM,
!    &     COMM_TINKER,i)
!     if (rank.eq.0) print*,' gridout Li ',time0
!     if (rank.eq.0) print*,' gridout L1 ',time1
!
...
!
!     permanent multipole contribution to the internal virial
!
!$acc parallel loop async(rec_queue) default(present) &
