!     ##################################################################
!     ##                                                              ##
!     ##  subroutine torquevec  --  convert single site torque to force  ##
!     ##                                                              ##
!     ##################################################################
!
!
!     "torquevec" takes the torque values on multiple sites defined by
!     a local coordinate frame and converts to Cartesian forces on
!     the original site and sites specifying the local frame
!
!     INPUT  : nploc is number of sites
!              ivec is indices of sites
!              trqvec are trqs of sites
!              devec are deps coming from epolar
!
!     OUTPUT : frcxvec, frcyvec and frczvec are forces
!              devec are deps sent back to epolar
...
!deb     if(rank.eq.0.and.tinkerdebug) then
!deb        write(*,*) 'TORQUEVEC IN nploc, nploc1, nploop,trqvec'
!deb        write(*,*) nploc,nploc1,nploop
!deb        write(*,*)
!deb &      'trq1 ',trqvec(1:nploc,1),
!deb &      'trq2 ',trqvec(1:nploc,2),
!deb &      'trq3 ',trqvec(1:nploc,3)
!deb        write(*,*) 'dep',
!deb &      'devec1 ',devec(1,1:nploc),
!deb &      'devec2 ',devec(2,1:nploc),
!deb &      'devec3 ',devec(3,1:nploc)
!deb        write(*,*) 'ivec' ,ivec(1:nploc)
!deb        write(*,*) 'axetypvec',axetypvec(1:nploc)
!deb     endif
...
!new
!new  send the results back in the original arrays
   do i=1,3
      frcxvec(1:nploc,i) =&
   ...
   enddo
!deb  if(rank.eq.0.and.tinkerdebug) write(*,*) 'torquevec'
!deb     if(rank.eq.0.and.tinkerdebug) then
!deb        write(*,*) 'TORQUEVEC OUT nploc, trqvec'
!deb        write(*,*) nploc,nploc1,nploop
!deb        write(*,*)
!deb &      'fixvec1 ',frcxvec(1:nploc,1),
!deb &      'fixvec2 ',frcxvec(1:nploc,2),
!deb &      'fixvec3 ',frcxvec(1:nploc,3),
!deb &      'fiyvec1 ',frcyvec(1:nploc,1),
!deb &      'fiyvec2 ',frcyvec(1:nploc,2),
!deb &      'fiyvec3 ',frcyvec(1:nploc,3),
!deb &      'fizvec1 ',frczvec(1:nploc,1),
!deb &      'fizvec2 ',frczvec(1:nploc,2),
!deb &      'fizvec3 ',frczvec(1:nploc,3)
!deb     endif
