...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine empole1c  --  Ewald multipole derivs via list  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!DIR$ ASSUME (mod(nblocloop,16).eq.0)
   ...
   enddo
!
!     initialize global mask for selection loop
!
...
!        qivec  is qixx, qixy, qixz, qixy, qiyy, qiyz,qixz, qiyz, qizz
      ...
      kk = 0
!
!       select all sites under the cutoff
!DIR$ ASSUME (mod(neloop16,16).eq.0)
      do k = 1, neloop16
         if(      kbisvec(k).ne.0&
...
!
!     and compute the energy for this interaction
!
!DIR$ ASSUME (mod(neloop8,8).eq.0)
!DIR$ VECTOR ALIGNED
!DIR$ SIMD
