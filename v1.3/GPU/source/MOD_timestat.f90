...
!
!   module timestat : Timers to measure time spent in regions of the code
!
!     tinkertime     logical flag to enable timers verbosity
...
!           write(*,'(A20,2F14.4)') timer_name(comm_timers(i)),
!    &            buffer(comm_timers(i),3),t_comm
      end do
...
!           if (rank.eq.0)
!    &      write(*,'(A20,2F14.4)') timer_name(comp_timers(i)),
!    &            buffer(comp_timers(i),3),t_comp
      end do
