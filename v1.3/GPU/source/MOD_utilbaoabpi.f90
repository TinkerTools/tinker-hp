...
!     #######################################################################################
!     ##                                                                                   ##
!     ##  module baoabopi  --  BAOAB Path Integral Langevin molecular dynamics step    ##
!     ##                                                                                   ##
!     #######################################################################################
!
!
!     "baoabpi" performs a single molecular dynamics time step
...
!$acc parallel loop collapse(3) default(present) async
      do k=1,nu; do iloc=1,nlocpi; do j=1,3
               ...
               !! BLOCK B !!
!          polymer%eigvel(j,i,k)=polymer%eigvel(j,i,k)
!     &       +dt2*convert*polymer%eigforces(j,i,k)/mass(i)
               if(k==1) then
                  !! BLOCK A centroid !!
...
!       GRADIENT SUBROUTINES
   ...
   &,only_long,remove_polarshort)
      ...
      logical save_colvars, save_plumed
...
!     FIRST PASS TO SUBSTRACT POLAR SHORTRANGE
...
!$acc end serial
         ...
         if (use_lambdadyn) then
            !$acc serial async present(delambdav,delambdae,delambdavpi,delambdaepi)
...
!          if(.not.do_short) then
!          call minmaxone(derivs_(:,:,k),3*nbloc
!     &       ,"d"//int_to_str(k+ibeadbeg-1))
!          call minmaxone(dev,3*nbloc
!     &       ,"dev"//int_to_str(k+ibeadbeg-1))
!          call minmaxone(dem,3*nbloc
!     &       ,"dem"//int_to_str(k+ibeadbeg-1))
!
!          endif
      enddo
...
!
 ...
 15   format(/,&
   ...
   subroutine prepare_colvars_pi(polymer)
      ...
      implicit none
      integer i,j,k,idx,iglob,iloc,ilocrec,ierr,offr,iproc
      real(r_p) :: nuinv
      ...
      nuinv = 1.0_re_p/real(polymer%nbeads,r_p)
  
!$acc parallel loop gang vector async copyin(cvatoms_ids) &
!$acc         copy(decv_tot,cv_pos) &
!$acc present(x,y,z,de_tot,derivs,derivsRec,de_buffr &
!$acc        ,repart,repartrec,loc,locrec,polymer%eigpos & 
!$acc        ,pbcWrapIdx)
      do i = 1, ncvatoms
        iglob = cvatoms_ids(i)
        iloc = loc(iglob)
     ...
     &       + pbcWrapIdx(4*(iglob-1)+1)*xbox
        cv_pos(2,i) = polymer%eigpos(2,iglob,1) &
     &       + pbcWrapIdx(4*(iglob-1)+2)*ybox
        cv_pos(3,i) = polymer%eigpos(3,iglob,1) &
     &       + pbcWrapIdx(4*(iglob-1)+3)*zbox

!$acc loop seq
        do k=1,nbeads
          decv_tot(1,i) = decv_tot(1,i) &
     &       - (polymer%forces(1,iglob,k) &
     &           + polymer%forces_slow(1,iglob,k))*nuinv
          decv_tot(2,i) = decv_tot(2,i) &
     &       - (polymer%forces(2,iglob,k) &
     &           + polymer%forces_slow(2,iglob,k))*nuinv
          decv_tot(3,i) = decv_tot(3,i) &
     &       - (polymer%forces(3,iglob,k)  &
     &           + polymer%forces_slow(3,iglob,k))*nuinv
        enddo

      end do

      if (use_lambdadyn) then
!$acc serial async present(delambdav,delambdae &
!$acc                     ,delambdavpi,delambdaepi)
        delambdav = delambdavpi
        delambdae = delambdaepi
        delambdavpi = 0_re_p
        delambdaepi = 0_re_p
!$acc end serial
!$acc update host(delambdav,delambdae,delambdavpi,delambdaepi) async
      endif
...
!$acc wait
   ...
   subroutine distrib_colvars_pi(polymer)
      ...
      if (nproctot > 1) then
        call MPI_BCAST(decv,3*ncvatoms,MPI_RPREC,0,MPI_COMM_WORLD,ierr)
      endif
      if (deb_Path) print*, 'distrib_colvars'

      if (use_lambdadyn) then
          if (nproctot > 1) then
            call MPI_BCAST(lambda,1,MPI_TPREC,0,MPI_COMM_WORLD,ierr)
          endif
          call def_lambdadyn
          if (use_osrw) then
            if (nproctot > 1) then
              call MPI_BCAST(flambdabias,1,MPI_TPREC,0 &
     &           ,MPI_COMM_WORLD,ierr)
            endif
            do i = 1, nloc
...
!$acc            ,polymer%eigpos,polymer%eigforces)
      do i = 1, ncvatoms
        iglob = cvatoms_ids(i)
        iloc = loc(iglob)
        if (repart(iglob) /= rank) cycle
        ...
        polymer%eigforces(1,iglob,1) = polymer%eigforces(1,iglob,1) &
     &      - decv(1,i)
        polymer%eigforces(2,iglob,1) = polymer%eigforces(2,iglob,1) & 
     &      - decv(2,i)
        polymer%eigforces(3,iglob,1) = polymer%eigforces(3,iglob,1) &
     &      - decv(3,i)
!
