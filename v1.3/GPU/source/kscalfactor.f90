...
!
!     workS[-p]  workspace required to build polar scaling with
!                low amount of memory
!     maxgangs   maximum gangs number used to process polar scaling
!                kernel
!     vecl       length of each gang
...
!
!     Gather in a structure array all the n scaling factor along with
!     their scaling type (1-2, 1-3, 1-4, 1-5)
!
...
!!$acc parallel loop present(n12,n13,n14,n15,i12,i13,i14,i15,
!!$acc   numscal_n)
   do i = 1, n
...
!#ifdef _OPENACC
!!$acc host_data use_device(numscal_n,scalbeg_n)
!      call thrust_exclusive_scan(numscal_n, n, scalbeg_n)
!!$acc end host_data
!#else
   scalbeg_n(1) = 0
   do i = 2, n
...
!!$acc parallel loop
!!$acc         present(numscal_n,scalbeg_n,typscal_n,allscal_n,
!!$acc   i12,i13,i14,i15,n12,n13,n14,n15)
   do i = 1, n
      nn12  = n12(i)
      nn13  = n13(i) + nn12
      nn14  = n14(i) + nn13
...
!
!     Gather in a structure array all the p scaling factor along with
!     their scaling type (1-1, 1-2, 1-3, 1-4)
!
...
!!$acc parallel loop present(np11,np12,np13,np14,ip11,ip12,
!!$acc  ip13,ip14,numscal_p)
   do i = 1, n
...
!#ifdef _OPENACC
!!$acc host_data use_device(numscal_p,scalbeg_p)
!      call thrust_exclusive_scan(numscal_p, n, scalbeg_p)
!!$acc end host_data
!#else
   scalbeg_p(1) = 0
   do i = 2, n
...
!!$acc   ip11,ip12,ip13,ip14,np11,np12,np13,np14)
   do i = 1, n
      nnp11    = np11(i)
      nnp12    = np12(i) + nnp11
      nnp13    = np13(i) + nnp12
...
!
!     Filter scaling factor for vscale
!
...
!        if (v2scale.ne.1) then
!           do j = 1,nn12
!              kglob = i12(j,iglob)
!              if (iglob.lt.kglob) then
!                 k = k + 1
!                 pair_factorn(k,ii) = kglob
!                 scal_factorn(k,ii) = 1-v2scale
!              end if
!           end do
!        end if
!        if (v3scale.ne.1) then
!           do j = 1,nn13
!              kglob = i13(j,iglob)
!              if (iglob.lt.kglob) then
!                 k = k + 1
!                 pair_factorn(k,ii) = kglob
!                 scal_factorn(k,ii) = 1-v3scale
!              end if
!           end do
!        end if
!        ! 1-4 interaction need absolutely to be take into account
!           do j = 1,nn14
!              kglob = i14(j,iglob)
!              if (iglob.lt.kglob) then
!                 k = k + 1
...
!              end if
!           end do
!        if (v5scale.ne.1) then
!           do j = 1,nn15
!              kglob = i15(j,iglob)
!              if (iglob.lt.kglob) then
!                 k = k + 1
!                 pair_factorn(k,ii) = kglob
!                 scal_factorn(k,ii) = 1-v5scale
!              end if
!           end do
!        end if
      n_factorn(ii) = k
...
!
!     Fill scaling the scaling_factor intercations along with their
!     values
!
...
!
!     Construct vdw scaling factor interation to postprocess
!
...
#endif
   if (tinkerdebug.gt.0.and.tinkerdebug.lt.16) then
...
12          format(' MPI',I9,' vscale pair found against',2I9,I3)
            if(rank.lt.3) print 12,n_vscalei,n_vscale1,n_vscale,rank
         else
...
!
!     Construct mpole scaling factor to postprocess
!
...
!$acc end data
   ...
   if (tinkerdebug.gt.0.and.tinkerdebug.lt.16.and.rank.eq.0) then
...
!$acc     async
   ...
   kglob_s=0
!
!     Construct induced dipoles scalar product scaling interactions
!
!$acc serial async
   n_uscale     = 0
   !n_dscale     = 0
...
!
!     Analyze polar scaling interactions
!
...
!$acc         async
   do ii = 1, nionlocnl
      ...
      do j = 1, n_factorn(ii)
         ...
         ccorrect_scale(2*(iscan+j)+2) = pchg(iichg)*merge(0.0_ti_p,pchg(kkchg),kkchg.eq.0)
      end do
   end do
...
#endif
   if (tinkerdebug.gt.0.and.tinkerdebug.lt.16.and.rank.lt.4) then
      print*,'n_scale',n_scale,rank
   end if
...
10 continue
   ...
   if (deb_Path) print*,"build_scaling_factor -step", istep
