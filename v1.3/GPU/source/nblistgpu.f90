...
!
!     Subroutine : Reorder the neigbhor list to obtain atoms under cutoff
...
!$acc         vector_length(32) async(dir_queue)
   do i = 1,nloc
      ...
      do while (restart)
         read_index = 0
!
!           Look for interaction over the cutoff
!           And store them
!
!$acc loop vector
         do k = 1,nnneig
...
!$acc end atomic
         ...
         !end if
!
!           Replace all interactions over index cutoff (read_index_s)
!
!$acc loop vector
...
!
!           Retrive all missing interactions under the cutoff index
!
         read_index = 0
!$acc loop vector
         do k = 1,read_index_s
...
subroutine initmpipmegpu
   ...
   use mpole
   use pme
   use potent  ,only: use_mlpot_only
   use mpi
   use tinMemory
...
subroutine ddbuild_gpu(dcut)
   ...
   integer abeg_c,aend_c,bbeg_c,bend_c,cbeg_c,cend_c,d_a,d_b,d_c,cap
   logical accept
   interface
     subroutine best_rational_approx ( x, max_den, num, den ) bind(C)
        import c_double,c_long_long
        real(c_double)      ,value:: x
        integer(c_long_long),value:: max_den
        integer(c_long_long)      :: num,den
     end subroutine
   end interface
!$acc routine(distprocpart1)
!
   if (deb_Path) write(*,'(2x,a)') 'ddbuild_gpu'

   if (.true.) then
      call best_rational_approx(xbox/(dcut/scala),int(65536,8),numa,dena) 
      call best_rational_approx(ybox/(dcut/scalb),int(65536,8),numb,denb) 
      call best_rational_approx(zbox/(dcut/scalc),int(65536,8),numc,denc) 
      ...
      ncell2buffa = 1*int(dcut)
      ncell2buffb = 1*int(dcut)
      ncell2buffc = 1*int(dcut)
      bigbufx     = (dcut)/ncell2buffa
      bigbufy     = (dcut)/ncell2buffb
      bigbufz     = (dcut)/ncell2buffc

      nx_cell     = max(2*ncell2buffa+1,int(xcell*scala/bigbufx))
      ny_cell     = max(2*ncell2buffb+1,int(ycell*scalb/bigbufy))
      nz_cell     = max(2*ncell2buffc+1,int(zcell*scalc/bigbufz))
   end if
   lenx_cell  = xcell/nx_cell
   leny_cell  = ycell/ny_cell
   lenz_cell  = zcell/nz_cell

   abeg_c = int((xbegproc(rank+1)+xcell2)/lenx_cell)
   aend_c = int((xendproc(rank+1)+xcell2)/lenx_cell)
   bbeg_c = int((ybegproc(rank+1)+ycell2)/leny_cell)
   bend_c = int((yendproc(rank+1)+ycell2)/leny_cell)
   cbeg_c = int((zbegproc(rank+1)+zcell2)/lenz_cell)
   cend_c = int((zendproc(rank+1)+zcell2)/lenz_cell)
   if (aend_c.eq.nx_cell) aend_c= aend_c -1
   if (bend_c.eq.ny_cell) bend_c= bend_c -1
   if (cend_c.eq.nz_cell) cend_c= cend_c -1

   nlocnl = nloc

 36 format(A,3I7,A,F7.2,A,3I8)
   if (ranktot.eq.0.and.tinkerdebug.gt.0)&
      write(*,36) ' -ddbuild_gpu Cells x y z',nx_cell,ny_cell,nz_cell,' -dcut',dcut,' -ncell2buff ',ncell2buffa,ncell2buffb,ncell2buffc
   !write(*,'(A,7I7)') ' -ddbuild_gpu edge',rank,abeg_c,aend_c,bbeg_c,bend_c,cbeg_c,cend_c
...
!$acc parallel loop copy(nlocnl) async(rec_queue)
   do i = nloc+1,nbloc
      iglob = glob(i)
      xr    = x(iglob)
      yr    = y(iglob)
      zr    = z(iglob)
      if (monoclinic.or.triclinic) call ctf_inl(xr,yr,zr)
      ...
      xw    = int( (xr+xcell2)/lenx_cell )
      yw    = int( (yr+ycell2)/leny_cell )
      zw    = int( (zr+zcell2)/lenz_cell )

      d_a   = min( abs(xw-abeg_c), abs(xw-aend_c) )
      d_b   = min( abs(yw-bbeg_c), abs(yw-bend_c) )
      d_c   = min( abs(zw-cbeg_c), abs(zw-cend_c) )

           if (xw.lt.abeg_c) then; d_a = min( abs(xw+nx_cell-abeg_c), abs(xw+nx_cell-aend_c), d_a )
      else if (xw.gt.aend_c) then; d_a = min( abs(xw-nx_cell-abeg_c), abs(xw-nx_cell-aend_c), d_a ) 
      end  if
           if (yw.lt.bbeg_c) then; d_b = min( abs(yw+ny_cell-bbeg_c), abs(yw+ny_cell-bend_c), d_b )
      else if (yw.gt.bend_c) then; d_b = min( abs(yw-ny_cell-bbeg_c), abs(yw-ny_cell-bend_c), d_b ) 
      end  if
           if (zw.lt.cbeg_c) then; d_c = min( abs(zw+nz_cell-cbeg_c), abs(zw+nz_cell-cend_c), d_c )
      else if (zw.gt.cend_c) then; d_c = min( abs(zw-nz_cell-cbeg_c), abs(zw-nz_cell-cend_c), d_c ) 
      end  if

      accept=.false.
      if (xw.le.abeg_c .or. aend_c.le.xw) accept= accept.or. d_a.le.ncell2buffa
      if (yw.le.bbeg_c .or. bend_c.le.yw) accept= accept.or. d_b.le.ncell2buffb
      if (zw.le.cbeg_c .or. cend_c.le.zw) accept= accept.or. d_c.le.ncell2buffc
      !accept=   .or.  d_b.le.ncell2buffb .or. d_c.le.ncell2buffc

      if (accept) then
      !  call distprocpart1(iglob,rank,d,.true.,x,y,z)
      !  if (rank.eq.0.and.d.gt.dcut) then
      !     print*, iglob,real(d,4),d_a,d_b,d_c
      !  end if
...
14 format(' ddbuild_gpu  -rank ',I0,' -nloc ',I0,' -nlocnl ',I0)
   if (tinkerdebug.gt.0) write(*,14) rank,nloc,nlocnl
...
subroutine build_cell_listgpu(istep)
   ...
   use utilgpu   ,only: nSMP,cores_SMP&
                 ,openacc_abort,rec_queue,rec_stream
#ifdef _CUDA
   use nblistcu  ,only: atoms_in_cell
   use utilcu    ,only: check_launch_kernel
...
!
   lenx       = abs(xmax-xmin)
   nx_cell    = max(2*ncell2buffb+1,int(scala*lenx/(bigbuf)))
   lenx_cell  = lenx/nx_cell
   leny       = abs(ymax-ymin)
   ny_cell    = max(2*ncell2buffb+1,int(scalb*leny/(bigbuf)))
   leny_cell  = leny/ny_cell
   lenz       = abs(zmax-zmin)
   nz_cell    = max(2*ncell2buffb+1,int(scalc*lenz/(bigbuf)))
...
24       format ('build_cell_listgpu -- Improper cell number',/,&
   ...
   call prmem_request(neigcell,num_neigcell,ncell_tot,async=.true.)
!
...
#ifdef _CUDA
!$acc host_data use_device(ineignl,x,y,z,repartcell,cell_len,indcelltemp)
   call atoms_in_cell<<<(nlocnl/128)+1,128,0,rec_stream>>>&
   (nlocnl,n,ncell_tot,nx_cell,ny_cell,ineignl&
   ,x,y,z,xmin,ymin,zmin,eps_cell,lenx_cell,leny_cell,lenz_cell&
   ,repartcell,cell_len,indcelltemp&
   )
   call check_launch_kernel("atoms_in_cell")
!$acc end host_data
#else
   ! FIXME  Atomic Capture on Integer Bugged from 24.XX SDK 
   ! FIXME  Report To Nvidia
!$acc parallel loop default(present) async(rec_queue)
   do i = 1,nlocnl
      iglob = ineignl(i)
      xr    = x(iglob)
...
#ifdef TINKER_DEBUG
      if (monoclinic.or.triclinic) call ftc_vec(xr,yr,zr)
...
!
!     Find neighbor cells for octahedron
!
   if (octahedron) then

!
!     Build Octahedron Adjacency matrix on cells
!
...
!
!     From Adjacency matrix to neighbor cells list
!
...
!     print*, 'num neighbor box',nx_cell,ny_cell,nz_cell,
!    &        num_neigcell
!     do i = 1,ncell_tot
!        if (cell_len(i).eq.0) cycle
!        print*, i, cell_len(i), numneigcell(i)
!     end do
!     print*, '---------------------------------'
...
!
!     Find neighbor cells for cubic box
!
!$acc parallel loop async(rec_queue) private(locat) &
!$acc         vector_length(32) &
            ...
            do q = -ncell2buff,ncell2buff
               do r = -ncell2buff,ncell2buff
                  ...
                  tempcell = (temp_z*ny_cell*nx_cell+temp_y*nx_cell+&
                     &temp_x) + 1
!                 locat =((r+ncell2buff)
!    &                   +(q+ncell2buff)*(2*ncell2buff+1)
!    &                   +(p+ncell2buff)*(2*ncell2buff+1)**2) + 1
!$acc atomic capture
                  locat = locat +1
                  count = locat
!$acc end atomic
...
!
!     Scan cell_len and build index for repartcell
!
...
!
!     subroutine build_cell_list2 : build the cells small enough in order to build
!     the non bonded neighbor lists with the cell-list method
!
subroutine build_cell_list2(ineignl,cell_order,&
   &nlocnl,buf,bP)
   use atoms ,only: x,y,z,n
   use bound ,only: use_bounds
   use boxes ,only: monoclinic,triclinic,orthogonal,octahedron,xbox,ybox,zbox
   use cell  ,only: xcell2,ycell2,zcell2,eps_cell
   use domdec,only: nproc,rank,COMM_TINKER,comm_dir,comm_rec,ndir,scala,scalb,scalc
   use inform,only: deb_Path,tindPath
   use nblistgpu_inl
...
!           nx_cell = nx_cell + 1
      ...
      if (xb) then
         nx_cell = max(1,int(bx_cell*scala*lenx/buf))
      else
         bx_cell = ceiling(buf*nx_cell/(scala*lenx))
      endif
      if (yb) then
         ny_cell = max(1,int(by_cell*scalb*leny/buf))
      else
         by_cell = ceiling(buf*ny_cell/(scalb*leny))
      endif
      if (zb) then
         nz_cell = max(1,int(bz_cell*scalc*lenz/buf))
      else
         bz_cell = ceiling(buf*nz_cell/(scalc*lenz))
      endif
...
!$acc   loc)
      do i = 1, nionlocnlb
         if (i.le.nionlocnl) then
            iglob        = celle_glob(i)
            celle_loc(i) = loc(iglob)
!              celle_ploc(i)= chgloc(celle_chg(i))
            celle_x  (i) = x(iglob)
            celle_y  (i) = y(iglob)
            celle_z  (i) = z(iglob)
         else
            celle_loc(i) = nbloc
!              celle_ploc(i)= nionbloc
            celle_x  (i) = inf
            celle_y  (i) = inf
            celle_z  (i) = inf
         end if
...
!                 xbk    = xmin + (kcell_x+0.5)*lenx_cell
!                 ybk    = ymin + (kcell_y+0.5)*leny_cell
!                 zbk    = zmin + (kcell_z+0.5)*lenz_cell
!                 posx   =  xbi - xbk
!                 posy   =  ybi - ybk
!                 posz   =  zbi - zbk
...
!                 call midpointimage_inl(xbk,ybk,zbk,posx,posy,posz)
!                 if   ((zbk.ge.zbeg).and.(zbk.lt.zend)
!    &             .and.(ybk.ge.ybeg).and.(ybk.lt.yend)
!    &             .and.(xbk.ge.xbeg).and.(xbk.lt.xend)) then
...
!
!     Preprocess adjacency matrix to find optimise memory allocation
!     Count every neighors blocks interaction
!
...
#endif
!  Wait for nlocnlb_pair to be updated on host memory
!  Implicit Wait seems to be removed with PGI-20
...
!
!     Build nblist with optimal space according to matb_lst
!     From full storage to adjacency list
!
...
!
!     Re use cell_len for blocks
!     initiate to block_size
!
...
!
!     finalize the pair block-atoms list
!     complete lst to fit a multiple of BLOCK_SIZE
!     building his pair block vector
!
...
! Wait for nlocnl_pair
! Implicit wait removed with PGI-20.4
!$acc wait(rec_queue)
...
!
!     "build_pairwise_list" constructs a pairwise atom nblist
!          inside 'a_plist' structure
!
...
#ifdef _OPENACC
   use interfaces,only:cu_filter_lst_sparse
...
!
!     Build Adjacency list piece by piece
!
...
16    format(1x,'natmnl',I8,';',3x,'mlpot list mem space (Mo)',F12.3)
      !nbMatb = max(1,min(int(buffMatb*1d9/(4*szMatb)),nblock))
...
!
!     From Block-Block to Block-Atoms list
!
#ifdef _CUDA
   c_glob=> a_plist%c_glob
   cell_x=> a_plist%cell_x
