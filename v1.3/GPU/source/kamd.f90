...
!
!
!     "kamd" checks the aMD activation and assign the necessary
...
!######################################################################
!
!     Check the aMD key activation for:
!               - a dihedral potential boost
!               - a potential energy boost
!               - both dihedral and potential boost
!               - a bond/angle potential boost on water
!               - a potential energy boost on water
!               - use or not of the GaMD boost potential
!
      do i = 1, nkey
         next = 1
         record = keyline(i)
...
!######################################################################
!
!     Assign the desired parameters for the aMD (LOOP2)
!
...
!
!     Rescaling of the aMDoutputFreq_dih for dihedrals
!
...
!
!     Reading of the number of water molecules
!
...
!
!     Reading of the type of water molecules
!
         if (use_amd_wat1) then
            do i = 1, nkey
               next = 1
...
!
!########################################
!#### Specific of the GaMD procedure ####
!########################################
!
!     Threshold energy for adding boost potential
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     1/ Preparation steps for GaMD procedure
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     3/ Preparatory equilibration steps
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     Upper limit of SD of the potential energy
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     Upper limit of SD of the dihedral potential boost
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!
!     Upper limit of SD of the bond/angle water potential boost
!
         do i = 1, nkey
            next = 1
            record = keyline(i)
...
!######################################################################
!
!    Check the validity of the aMD parameters
!
         quit = .false.
10       continue
...
499         format(3x,44('*'),/,&
               ...
               &3x,44('*'),/)
!
!    Type of aMD used (aMDdihE / aMDePE / dual / water / GaMD)
!
            write (iamdout,500)
500         format(3x,"#####################")
