...
ifeq ($(COLVARS_SUPPORT),1)
   pp_flags_common_ += -DCOLVARS -DCOLVARS_MPI
   ifeq ($(TCL_SUPPORT),1)
...
legacy_flags        := off
cxx_std             := 14
compute_capability  := 70,80
cuda_version        := 12.5
device_c_comp       := -ccbin $(RunCUHC)
...
ifneq ($(add_host),$(empty__))
   ...
   host_flags_cuda_c_ += --compiler-options '$(add_host)'
...
#LDLIBS                 = -lm -L$(LIB_FFTDECOMP) -rpath $(LIB_CPP)#-L$(LIB_CPP)
LDLIBS                 = -lm -L$(LIB_FFTDECOMP) -L$(LIB_CPP)
#LDLIBS                 = -lucm -lucs -lucp -luct -lm -L$(LIB_FFTDECOMP) -L$(LIB_CPP)
ifeq ($(add_mkl__),yes)
   LDLIBS             += -L$(LIB_MKL)
else
   LDLIBS             += $(LIB_LAPACK)
