...
!     ########################################################################
!     ##                                                                    ##
!     ##  module QTB  --  parameters and arrays for Quantum Thermal Bath    ##
!     ##                                                                    ##
!     ########################################################################
!     omegacut : cutting frequency for the QTB random force
!     domega : frequency discretization in THz (ps-1)
!     omegasmear : length of the smearing window near omegacut
!     skipseg : number to seg to be skipped (because the system has not
!     vad : past velocities used for correlation computations
!
!     n_of_type: number of atoms of each type
!     adqtb_type: map the atoms to their adqtb type
!     ntype: number of different types
!
...
!        logical, allocatable :: proc_has_data(:)
!        logical :: currently_loc
   end type
...
!
!      subroutine qtbinit: initialize the QTB
!
...
!        qtbdata(iglob)%currently_loc=.TRUE.
      enddo
...
!
!     Order the atom type for the adQTB
!
         ...
         if(rank==0) then
            ...
            do i=1,ntype
               ...
               SELECT CASE(opttype)
                ...
                CASE("ADABELIEF")            
                  adapt_gamma(i)%p => adapt_gamma_adabelief
                CASE DEFAULT
...
!      nstep_therm=0
   ...
   subroutine initialize_adaptation_parameters()
      ...
      do i = 1, nkey
         ...
         call gettext (record,opttype,next)
         call upcase (opttype)
         ...
         if (opttype=="RATIO") then
            read (string,*,iostat=ios) tmp,smooth
         ...
         elseif (opttype=="SIMPLE") then
            ...
            A_gamma(k)=tmp
         elseif (opttype=="ADABELIEF") then
            read (string,*,iostat=ios) A_gamma(k),adqtb_tau_avg(k),smooth
            if (A_gamma(k)<0) A_gamma(k)=A_gamma_default
            if (adqtb_tau_avg(k)<0) adqtb_tau_avg(k)=adqtb_tau_avg_default
         else
...
!$acc parallel loop collapse(3) async present(qtbdata(iglob)%vad)
...
!        allocate(qtbdata(iglob)%proc_has_data(nproc))
!        qtbdata(iglob)%proc_has_data(:)=.FALSE.
!        qtbdata(iglob)%proc_has_data(rank+1)=.TRUE.
...
!        deallocate(qtbdata(iglob)%proc_has_data)
      qtbdata_allocated(iglob)=.FALSE.

   end subroutine deallocate_qtbdata

!
!     subroutine adHnoise: compute the kernel for generating the colored noise (fill Htilde)
!
...
!
!     subroutine convolseg: generate the colored noise by performing a convolution
!                             between the white noise and the kernel Htilde
!
   subroutine convolseg()
      ...
      use spectra
      implicit none
      integer :: i,j,k,ibatch,ibeg,iend,nloc_,ibegbeg,iendend
...
!
!     subroutine qtbrandom: propagate O block (from BAOAB) using the QTB random force
!
...
!
!     save velocity and random force if you need spectra
!     apply QTB random force
!
      if (register_spectra) then
...
!      end subroutine next_qtb_kick
   ...
   subroutine QTB_update_state()
      ...
      implicit none
!
!     At the end of each segment compute the the new gamma and generate
!     new noise
!
...
!                                              and adapt gammar if adaptive
   subroutine QTB_spectra_adaptation_gamma()
      ...
      call update_fft_plan_qtb(3*nseg)
!
!    reset averages if still in thermalization
!
...
!     Allocation of the different used for the adqtb
...
!$acc wait
      deallocate(sv,sf)
!
!     the master gets the sums of Cvv and Cvf contributions
!
!      if(piqtb .and. nproc_polymer>1) then
!        ! allreduce on the polymer communicator
!        ! so that only spatial comm is needed now
!        call reduce_qtb_spectrum(mCvv_type,nbeads*ntype*3*nseg*3
!     &                            ,COMM_POLYMER,rank_polymer,.TRUE.)
...
!       WRITE SPECTRA AND GAMMAR
      ...
      if(adaptive_qtb) then
!     the master sends the adapted gamma_r to everybody
         call MPI_BCAST(gamma_type,ntype*nad*nbeads,MPI_RPREC&
            &,0,MPI_COMM_WORLD,ierr)
!     recompute the noise kernel with the new gamma
         call adHnoise(.FALSE.)
      endif

!
!       COMPUTE corr_fact_qtb from deconvolution
!
...
!       COMMUNICATION ROUTINES
   ...
   subroutine reassignqtb(nloc_save,max_data_recv&
      ...
      use atomsMirror
      use mpi
      use domdec, only: nproc,COMM_TINKER
      use spectra
      use inform, only: deb_Path
      use beads
      implicit none
...
!            ! IF I THINK THAT pn DOES NOT HAVE DATA, IT MEANS WE NEVER cCOMMUNICATED (THIS ATOMS) DURING SEGMENT
!            ! => I SHOULD ASK THEM IF THEY HAVE DATA OR NOT
!            if(.not.qtbdata(iglob)%proc_has_data(pn+1)) then
!              n_ask_send(ineighbor)=n_ask_send(ineighbor) + 1
!            endif
!          enddo
!          if(n_ask_send(ineighbor)>0) then
!            ii=ii+1
!            call MPI_IRECV(has_data_send(1,ineighbor)
!     &      ,n_ask_send(ineighbor), MPI_LOGICAL, pn, 0
!     &      , COMM_TINKER, req_data_send(ii),ierr)
!          endif
!        enddo
!
!        n_ask_recv(:)=0
...
!            if(.not.qtbdata(iglob)%proc_has_data(pn+1)) then
!              n_ask_recv(ineighbor)=n_ask_recv(ineighbor) + 1
!              has_data_recv(n_ask_recv(ineighbor),ineighbor)
!     &           =was_allocated
!              qtbdata(iglob)%proc_has_data(pn+1)=.TRUE.
!            endif
!            qtbdata(iglob)%currently_loc=.TRUE.
         enddo
!          if(n_data_r(ineighbor)>max_atoms_r)
!     &        max_atoms_r=n_data_r(ineighbor)
!          if(n_ask_recv(ineighbor)>0) then
!            jj=jj+1
!            call MPI_ISEND(has_data_recv(1,ineighbor)
!     &      ,n_ask_recv(ineighbor), MPI_LOGICAL, pn, 0
!     &      , COMM_TINKER, req_data_recv(jj),ierr)
!          endif
      enddo

!        call MPI_WAITALL(ii,req_data_send
!     &                  ,MPI_STATUSES_IGNORE, ierr)
!        call MPI_WAITALL(jj,req_data_recv
!     &                  ,MPI_STATUSES_IGNORE, ierr)
...
!            !if(.not.qtbdata(iglob)%proc_has_data(pn+1)) then
!            !  ii=ii+1
!            !  if(.not. has_data_send(ii,ineighbor)) then
!            !    !send data if neigh does not have data yet
!            !    n_data_s(ineighbor)=n_data_s(ineighbor)+1
!            !    iglob_s(n_data_s(ineighbor),ineighbor)=iglob
!            !  endif
!            !endif
!c            qtbdata(iglob)%proc_has_data(pn+1)=.TRUE.
!            !mark atom as not local
!            qtbdata(iglob)%currently_loc=.FALSE.
!c            call MPI_Isend(qtbdata(iglob)%proc_has_data
!c     &        ,nproc, MPI_LOGICAL
!c     &        ,pneig_send(ineighbor),1,COMM_TINKER
!c     &        ,req_data_send(ineighbor), ierr)
!
!          enddo
...
!              data_send(j,iseg,i,ineighbor)
!     &           =qtbdata(iglob)%fad(j,iseg,1)
                  enddo; enddo ; enddo; enddo
      enddo
...
!      do i=1,n
!        if(qtbdata_allocated(i)) then
!          if(.not. qtbdata(i)%currently_loc) then
!            qtbdata(i)%proc_has_data(:)=.FALSE.
!            qtbdata(i)%proc_has_data(rank+1)=.TRUE.
!          else
!            call deallocate_qtbdata(i)
!          endif
!        endif
!      enddo
...
!      do i=1,nloc
!        iglob=glob(i)
!        if(.not. qtbdata(iglob)%currently_loc) then
!          write(0,*) rank,"atom",iglob,"should be local"
!          call fatal
!        endif
!      enddo
...
!      use inform, only: deb_Path
!      implicit none
!      integer :: i,j,k,iproc,ii,jj,ierr,iglob,nsendtot
!      real(4), allocatable :: data_send(:,:,:)
!      real(4), allocatable :: data_recv(:,:,:)
!      integer, allocatable :: iglob_send(:)
!      integer, allocatable :: iproc_send(:),iproc_recv(:)
!      integer :: nproc_send, maxsend
!      integer :: nproc_recv, maxrecv,nsend_capture
!      integer :: req_s(nproc),req_r(nproc),req_size(2*nproc)
!      integer :: nsend(nproc),nrecv(nproc),procnum(nproc)
!     &         ,nsend_bis(nproc)
!      integer, parameter :: mptag_size=0,mptag_data=1
!
!      if(nproc==1) return
!      if (deb_Path) write(*,*) '   >> gather_vad'
!
!!$acc update host(repart) async
!      ! COUNT DATA TO SEND
!      nsend(:)=0
!      allocate(iglob_send(n))
!
!      nsendtot=0
!!$acc wait
!      do i=1,n
!        iproc=repart(i)
!        if(qtbdata_allocated(i)
!     &   .AND. iproc/=rank) then
!          nsend(iproc+1)=nsend(iproc+1)+1
!          nsendtot=nsendtot+1
!          iglob_send(nsendtot)=i
!        endif
!      enddo
!      nproc_send=COUNT(nsend>0)
!      maxsend=maxval(nsend)
!
!      !SEND AND RECEIVE SIZES
!      req_size(:)=MPI_REQUEST_NULL
!      ii=0
!      procnum(:)=-1
!      nrecv(:)=0
!      allocate(iproc_send(nproc_send))
!      do iproc=1,nproc
!        if(iproc-1==rank) CYCLE
!        CALL MPI_ISEND(nsend(iproc),1,MPI_INTEGER
!     &   , iproc-1, mptag_size , COMM_TINKER, req_size(iproc),ierr)
!        CALL MPI_IRECV(nrecv(iproc),1,MPI_INTEGER
!     &   , iproc-1, mptag_size ,COMM_TINKER,req_size(nproc+iproc),ierr)
!        if(nsend(iproc)==0) CYCLE
!        !TWO WAY MAP OF PROCS TO SEND DATA
!        ii=ii+1
!        procnum(iproc)=ii
!        iproc_send(ii)=iproc
!      enddo
!
!       !WAIT FOR SIZES
!      call MPI_Waitall(2*nproc,req_size
!     &       ,MPI_STATUSES_IGNORE, ierr)
!
!      !ORGANIZE DATA TO SEND
!      nsend_bis(:)=0
!      maxrecv=maxval(nrecv)
!      nproc_recv=COUNT(nrecv>0)
!      allocate(data_send(3*nseg+1,maxsend,nproc_send))
!      allocate(data_recv(3*nseg+1,maxrecv,nproc_recv))
!
!!$acc data create(data_send,data_recv)
!!$acc copyin(iglob_send,procnum,nsend_bis,nrecv)
!
!!$acc parallel loop async default(present)
!      do i=1,nsendtot
!        iglob=iglob_send(i)
!        iproc=repart(iglob)+1
!!$acc atomic capture
!        nsend_bis(iproc)=nsend_bis(iproc)+1
!        nsend_capture=nsend_bis(iproc)
!!$acc end atomic
!        data_send(1,nsend_capture,procnum(iproc))=iglob
!!$acc loop seq collapse(2)
!        do k=1,nseg; do j=1,3
!          data_send(3*(k-1)+j+1,nsend_capture,procnum(iproc))
!     &       =qtbdata(iglob)%vad(j,k)
!        enddo; enddo
!      enddo
!
!      !RECEIVE DATA
...
!
!      allocate(iproc_recv(nproc_recv))
!      jj=0
!
!!$acc wait
!!$acc host_data use_device(data_send, data_recv)
!      ! SEND DATA
!      do ii=1,nproc_send
!        iproc=iproc_send(ii)
!        CALL MPI_ISEND(data_send(1,1,ii),(3*nseg+1)*nsend(iproc)
!     &   ,MPI_REAL4, iproc-1
!     &   ,mptag_data, COMM_TINKER,req_s(ii),ierr)
!      enddo
!
!      do iproc=1,nproc
!        if(iproc-1==rank .OR. nrecv(iproc)==0) CYCLE
!        jj=jj+1
!        iproc_recv(jj)=iproc
!        CALL MPI_IRECV(data_recv(1,1,jj),(3*nseg+1)*nrecv(iproc)
!     &   ,MPI_REAL4, iproc-1
!     &   ,mptag_data,COMM_TINKER,req_r(jj),ierr)
!      enddo
!!$acc end host_data
!
!      !WAIT FOR DATA
!      call MPI_Waitall(ii,req_s
!     &       ,MPI_STATUSES_IGNORE, ierr)
!      call MPI_Waitall(jj,req_r
!     &       ,MPI_STATUSES_IGNORE, ierr)
!
!
!      !REDUCE DATA
!!$acc parallel loop async default(present)
!!$acc         vector_length(512) copyin(iproc_recv)
!      do ii=1,nproc_recv
!!$acc loop vector collapse(3)
!        do i=1,nrecv(iproc_recv(ii))
!          do k=1,nseg; do j=1,3
!            iglob=nint(data_recv(1,i,ii))
!!$acc atomic update
!            qtbdata(iglob)%vad(j,k)=qtbdata(iglob)%vad(j,k)
!     &         +data_recv(3*(k-1)+j+1,i,ii)
!          enddo; enddo
!        enddo
!      enddo
!!$acc end data
!
!      deallocate(data_recv, data_send)
!
!      if (deb_Path) write(*,*) '  << gather_vad'
!
!      end subroutine gather_vad
!
!      subroutine gather_noise()
!      use atoms
!      use domdec
!      use spectra
!      use mpi
!      use inform, only: deb_Path
!      implicit none
!      integer :: i,j,k,iproc_s,iproc_r,ierr,nseg_comm
!      integer :: maxsend,maxrecv,nproc_send,nproc_recv
!      integer :: nsendtot,ii,jj,iproc,iglob,nsend_capture
!      integer :: req_s(nproc),req_r(nproc)
!      integer :: nsend(nproc),nrecv(nproc),procnum(nproc)
!     &             ,nsend_bis(nproc)
!      integer, allocatable :: iglob_send(:)
!      integer, allocatable :: iproc_send(:),iproc_recv(:)
!      real(4), allocatable :: data_send(:,:,:),data_recv(:,:,:)
!
!      if(nproc==1) return
!      if (deb_Path) write(*,*) '   >> gather_noise'
!
!!$acc update host(repart) async
!      nseg_comm=2*nseg
!
!      !COUNT DATA TO SEND/RECEIVE
!      nrecv(:)=0
!      nsend(:)=0
!      nsendtot=0
!      allocate(iglob_send(n))
!!$acc wait
!      do i=1,n
!        iproc_s=repartnoise(i)+1
!        iproc_r=repart(i)+1
!        if(iproc_r-1==rank .AND. iproc_s-1/=rank) then
!          !RECEIVE IF I HAVE AN ATOM FOR WHICH
!          ! I DID NOT GENERATE THE NOISE
!          nrecv(iproc_s)=nrecv(iproc_s)+1
!        elseif(iproc_s-1==rank .AND. iproc_r-1/=rank) then
!          !SEND IF I GENERATED THE NOISE FOR AN ATOM
!          ! I DON'T HAVE
!          nsend(iproc_r)=nsend(iproc_r)+1
!          nsendtot=nsendtot+1
!          iglob_send(nsendtot)=i
!        endif
!      enddo
...
!      nproc_recv=COUNT(nrecv>0)
!
!      !MAP OF PROCS TO SEND/RECEIVE DATA
!      ii=0
!      jj=0
!      procnum(:)=-1
!      allocate(iproc_send(nproc_send))
...
!
!      !ORGANIZE DATA TO SEND
!      allocate(data_send(3*nseg_comm+1,maxsend,nproc_send))
!      allocate(data_recv(3*nseg_comm+1,maxrecv,nproc_recv))
!      nsend_bis(:)=0
!!$acc data create(data_send,data_recv)
!!$acc copyin(nrecv,iproc_recv,iglob_send,procnum,nsend_bis)
!
!!$acc parallel loop async default(present)
!      do i=1,nsendtot
!        iglob=iglob_send(i)
...
!        data_send(1,nsend_capture,procnum(iproc))=iglob
!!$acc loop seq collapse(2)
!        do k=1,2*nseg; do j=1,3
!          data_send(3*(k-1)+j+1,nsend_capture,procnum(iproc))
!     &       =qtbdata(iglob)%noise(j,k)
!        enddo; enddo
!      enddo
!
!      ! SEND DATA
...
!      do ii=1,nproc_send
!        iproc=iproc_send(ii)
!        CALL MPI_ISEND(data_send(1,1,ii),(3*nseg_comm+1)*nsend(iproc)
!     &   ,MPI_REAL4, iproc-1
!     &   ,0, COMM_TINKER,req_s(ii),ierr)
!      enddo
!
!      do jj=1,nproc_recv
!        iproc=iproc_recv(jj)
!        CALL MPI_IRECV(data_recv(1,1,jj),(3*nseg_comm+1)*nrecv(iproc)
!     &   ,MPI_REAL4, iproc-1
!     &   ,0,COMM_TINKER,req_r(jj),ierr)
!      enddo
!!$acc end host_data
!
!      !WAIT FOR DATA
!      call MPI_Waitall(nproc_send,req_s,MPI_STATUSES_IGNORE,ierr)
!      call MPI_Waitall(nproc_recv,req_r,MPI_STATUSES_IGNORE,ierr)
!
...
!          do k=1,2*nseg; do j=1,3
!            iglob=nint(data_recv(1,i,ii))
!            qtbdata(iglob)%noise(j,k)=data_recv(3*(k-1)+j+1,i,ii)
!          enddo; enddo
!        enddo
!      enddo
!!$acc end data
!
!!$acc exit data delete(data_recv,data_send) async
!      deallocate(data_recv,data_send)
!      if (deb_Path) write(*,*) '   << gather_noise'
!
!      end subroutine gather_noise
...
!     adQTB OPTIMIZERS
   subroutine adapt_gamma_simple(itype,mCvv,Cvf,dFDR&
      ...
      do ibead=1,nbeads
         ...
         dfdr_norm_instant=NORM2(dFDR(:,ibead))
...
!           if(nadapt(itype)==0) then
!             write(*,*) "Started adapting Type_"
!     &         //trim(adqtb_name(itype))
!     &         //"_"//int_to_str(adqtb_type_map(itype))
!           endif
...
!     &           *real(n,8)/real(n_of_type(itype),8)
...
!         endif
   ...
   subroutine adapt_gamma_adabelief(itype,mCvv,Cvf,dFDR,qtb_therm)
      use spectra
      use atoms
      use langevin
      use ascii, only: int_to_str
      ...
      real(r_p), intent(in) :: mCvv(:,:),Cvf(:,:)
      logical,intent(in) :: qtb_therm
      real(r_p) :: b1,tau1,sqrtm,w,b1_avg,avg_correct_1,a1
      real(r_p) :: tau2,b2_avg,avg_correct_2
      real(r_p) :: adapt_correct
      real(r_p) :: smooth,mt,st
      real(r_p), allocatable, save :: dFDR_m(:,:,:)
      real(r_p), allocatable, save :: dFDR_d2(:,:,:)
      integer :: j,ibead
      integer, allocatable, save :: cnt_type(:)
      real(r_p) :: dFDRs(nad,nbeads)
      integer :: cnt
      ...
      if(.not. allocated(cnt_type)) then
         ...
         cnt_type(:)=0
         allocate(dFDR_m(nad,nbeads,ntype))
         allocate(dFDR_d2(nad,nbeads,ntype))
         dFDR_m=0.d0
         dFDR_d2=0.d0
      endif
      cnt_type(itype)=cnt_type(itype)+1
      ...
      a1 = A_gamma(adqtb_type_map(itype))

      w=weight(adqtb_type_map(itype))
      sqrtm=merge(sqrt(w),1.d0,w>0)   

      tau1=adqtb_tau_avg(adqtb_type_map(itype))
      if (adqtb_tau_adapt>0) then
         tau2=adqtb_tau_adapt*tau1
      else
         tau2=100*tau1
      endif
      b1_avg = exp(-Tseg*adqtb_avg_adapt/tau1)
      b2_avg = exp(-Tseg*adqtb_avg_adapt/tau2)
      avg_correct_1 = 1.d0 - b1_avg**cnt
      avg_correct_2 = 1.d0 - b2_avg**cnt

      do ibead=1,nbeads
         do j=1,nad
            dFDR_m(j,ibead,itype)=dFDR_m(j,ibead,itype)*b1_avg &
   &       + (1.d0-b1_avg)*dFDRs(j,ibead)
            dFDR_d2(j,ibead,itype)=dFDR_d2(j,ibead,itype)*b2_avg &
   &       + (1.d0-b2_avg)*(dFDRs(j,ibead)-dFDR_m(j,ibead,itype))**2 &
   &       + 1.d-8

            mt = dFDR_m(j,ibead,itype) / avg_correct_1
            st = dFDR_d2(j,ibead,itype) / avg_correct_2

            gamma_type(j,itype,ibead)=max(gammar_min &
   &         ,gamma_type(j,itype,ibead) - a1*mt/sqrt(st) )
         enddo
      enddo

   end subroutine adapt_gamma_adabelief
...
!!$acc wait
!      call MPI_BARRIER(COMM_POLYMER,ierr)
...
!$acc wait
   ...
   subroutine reduce_piqtb_vad()
      ...
      use spectra
      implicit none
      integer :: k,i,j,iglob,iproc,maxsend,nsend,maxloc
      integer :: maxrecv,nproc_recv
      integer :: isend,ii,ierr,ns,nr,iloc,kk,l
      integer, parameter :: mptag_size=0,mptag_data=1
      integer :: reqs(2*nproc_polymer)
