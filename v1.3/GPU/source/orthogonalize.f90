...
!        if (i.eq.1450.and.mu.eq.3.125d0) then
!41         format(I6,d20.6,2F11.6,I6)
!           print 41, k,p2,Diag(k),SubDiag(k-1)**2,Nracines
!        end if
      old_l = new_l
...
!12         format(A,I3,2I6,3F16.7)
!           write(*,12) 'bissG',it,ii,Nrac,r,s,norm
      end do
      Eig(k+1) = (r+s)/2.0
   end do
...
!        if (EigenVec_l.and.kk.gt.lossOrthIt) then
!           !cfg=merge('verb    ','lz_Q    ',kk.gt.286.and.kk.lt.291)
!           cfg='lz_Q'
!           call chkOrthogonality(lz_Q(1,1,1),3*npoleloc,kk
!    &                           ,3*npoleloc,cfg,'lz_Q    ')
!        end if
      end if  ! (DEBUG INFO)
...
!$acc wait
!!$acc update host(scalV)
!               call printVec("scalV",scalV,kk-40,precd=12,prec=15)
         end if
...
!     if (krylovDim.gt.120.and.OrthBase.and.EigenVec_l) then
!        call OrthogonalizeBase1(lz_Q,3*npoleloc,3*npoleloc
!    &       ,krylovDim,120)
!     end if
   if (tinkerdebug.gt.0) then
...
!!$acc parallel loop async collapse(3) default(present)
!         do k = 1,npoleloc; do j = 1,2; do i = 1,3
!            if (btest(j,0)) then; lz_V(i,j,k,1) = Vp (i,k,it)
!            else;                 lz_V(i,j,k,1) = Vp1(i,k,it); end if
!         end do; end do; end do
!         call Matxvec(lz_V(:,:,:,1),lz_V(:,:,:,2))
!!$acc parallel loop async collapse(3) default(present)
!         do k = 1,npoleloc; do j=1,2; do i = 1,3
!            if (btest(j,0)) then; TVp (i,k,it) = lz_V(i,j,k,2)
!            else;                 TVp1(i,k,it) = lz_V(i,j,k,2); end if
...
!!$acc host_data use_device(MatE_,MatE1_,Ipivot,MIdk)
!      call cuGESV(nEigenV,nEigenV,MatE_,nEigenV,Ipivot,MIdk,nEigenV
!     &           ,rec_stream)
!!$acc parallel loop async collapse(2) deviceptr(MatE_,MIdk)
!      do i = 1,nEigenV**2
!         do j= 1,nEigenV
!            MatE_(j,i) = MIdk(j,i)
!            Midk (j,i) = merge(1,0,(i.eq.j))
!         end do
!      end do
!      call cuGESV(nEigenV,nEigenV,MatE1_,nEigenV,Ipivot,MIdk,nEigenV
!     &           ,rec_stream)
!!$acc parallel loop async collapse(2) deviceptr(MatE_,MIdk)
!      do i = 1,nEigenV**2
!         do j= 1,nEigenV
...
!     call printVec(" MatE_ ",MatE_,nEigenV**2, prec=12, precd=8,
!    &      period=min(nEigenV,20))
...
!
!     Compute projection of Vin in the deflated subspace
!
...
!
!     Compute Vout = alpha*Vout + Vp*EigVal^{-1}*Vp^{T}
!
...
subroutine polarEingenVal
   ...
   if (EigenVec_l) then
      ...
      ncount = sz_init
      call getEigenVec
!        call bissectionGivens1(lz_T,lz_T(sz_init+1),sz_init
!    &                         ,ncount,ri,si,tol,EigVal)
   end if
   Eig = EigVal
