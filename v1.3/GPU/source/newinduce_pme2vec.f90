...
!new  allocate (ef(max(1,npolebloc),3,nrhs))
   ef = 0_ti_p
...
!new       ef(i,j,1)  = ef(i,j,1) - cphi(j+1,i)
!new       ef(i,j,2)  = ef(i,j,2) - cphi(j+1,i)
      end do
   end do
!
...
!new      ef(i,j,1)  = ef(i,j,1) + term*rpole(j+1,iipole)
!new      ef(i,j,2)  = ef(i,j,2) + term*rpole(j+1,iipole)
      end do
   end do
   wtime1 = mpi_wtime()
!
...
!new          mu(j,k,i) = polarity(iipole)*ef(i,j,k)
            end do
         end do
      end do
   else
...
!new  real(t_p)  ef(npolebloc,3,nrhs), mu(3,nrhs,*), murec(3,nrhs,*)
   logical precnd
...
!new     do j = 1,nrhs
!new        do k = 1,npoleloc
!new           res(i,j,k) = ef(k,i,j)-h(i,j,k)
!new        enddo
!new     enddo
!new  enddo
   do k = 1, nrhs
...
!new    do i = 1, 3
!new    do j = 1,npoleloc
!new       ggnew(k) = ggnew(k)+res(i,k,j)*zr(i,k,j)
!new       ene(k) = ene(k) -pt5*mu(i,k,j)*(res(i,k,j)+ef(j,i,k))
!new    end do
!new    end do
      end do
...
!
   ...
   real(t_p)  ef(3,nrhs,*), mu(3,nrhs,*), murec(3,nrhs,*)
!new  real(t_p)  ef(max(1,npolebloc),3,nrhs), mu(3,nrhs,*), murec(3,nrhs,*)
...
!new          munew(j,k,i) = polarity(iipole)*(ef(i,j,k) - h(j,k,i))
            end do
         end do
      end do
!
...
subroutine commfieldvec(nrhs,ef)
   ...
   integer, allocatable :: reqrec(:), reqsend(:)
   real(t_p) ef(3,nrhs,*)
!new  real(t_p) ef(max(1,npolebloc),3,nrhs)
...
!new  allocate (buffer(max(npoleloc,1),3,nrhs,n_send1))
   allocate (reqrec(nproc))
   allocate (reqsend(nproc))
!
   do i = 1, n_send1
...
!new    call MPI_ISEND(ef(bufbegpole(p_recep1(i)+1),1,1),
...
!new        ef(j,1,k) = ef(j,1,k) + buffer(j,1,k,i)
!new        ef(j,2,k) = ef(j,2,k) + buffer(j,2,k,i)
!new        ef(j,3,k) = ef(j,3,k) + buffer(j,3,k,i)
         end do
      end do
   end do
!
...
!
!non  subroutine commfieldshort(nrhs,ef)
!non  use atoms
!non  use domdec
!non  use mpole
!non  use potent
!non  use mpi
!non  implicit none
!non  integer nrhs,i,j,k,tag,status(MPI_STATUS_SIZE)
!non  integer ierr,commloc
!non  integer, allocatable :: reqrec(:), reqsend(:)
!non  real(t_p) ef(3,nrhs,*)
!non  real(t_p), allocatable :: buffer(:,:,:,:)
!non
!non  if (use_pmecore) then
!non    commloc = comm_dir
!non  else
!non    commloc = COMM_TINKER
!non  end if
!non
!non  allocate (buffer(3,nrhs,max(npoleloc,1),n_sendshort1))
!non  allocate (reqrec(nproc))
!non  allocate (reqsend(nproc))
!non
!non  do i = 1, n_sendshort1
!non    tag = nproc*rank + p_sendshort1(i) + 1
!non    call MPI_IRECV(buffer(1,1,1,i),3*nrhs*npoleloc,MPI_TPREC,
!non $   p_sendshort1(i),tag,commloc,reqrec(i),ierr)
!non  end do
!non
!non  do i = 1, n_recepshort1
!non    tag = nproc*p_recepshort1(i) + rank + 1
!non    call MPI_ISEND(ef(1,1,bufbegpole(p_recepshort1(i)+1)),
!non $   3*nrhs*domlenpole(p_recepshort1(i)+1),MPI_TPREC,
!non $   p_recepshort1(i),tag,commloc,reqsend(i),ierr)
!non  end do
!non
!non  do i = 1, n_sendshort1
!non    tag = nproc*rank + p_sendshort1(i) + 1
!non    call MPI_WAIT(reqrec(i),status,ierr)
!non  end do
!non  do i = 1, n_recepshort1
!non    tag = nproc*p_recepshort1(i) + rank + 1
!non    call MPI_WAIT(reqsend(i),status,ierr)
!non  end do
!non
!non  do i = 1, n_sendshort1
!non    do j = 1, npoleloc
!non      do k = 1, nrhs
!non        ef(1,k,j) = ef(1,k,j) + buffer(1,k,j,i)
!non        ef(2,k,j) = ef(2,k,j) + buffer(2,k,j,i)
!non        ef(3,k,j) = ef(3,k,j) + buffer(3,k,j,i)
!non      end do
!non    end do
!non  end do
!non
!non  deallocate (buffer)
!non  deallocate (reqrec)
!non  deallocate (reqsend)
!non  return
!non  end
!
!===================================================
