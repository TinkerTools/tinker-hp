...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine ehal1gpu  --  buffered 14-7 energy & derivatives  ##
!     ##                                                               ##
!     ###################################################################
!
!
...
!
!     save delambdav for short range computation
!
...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine ehal1c  --  buffered 14-7 vdw derivs via list  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!$acc         async(def_queue)
   ...
   enddo
!
!     set the configuration for the main loop
!
   if    (use_vdwshort) then
      mode = 'SHORTVDW'
      loff2= 0.0
...
!
!     Compute vdw pairwise scaling interactions
!
...
!$acc     reduction(+:ev,delambdav,g_vxx,g_vxy,g_vxz,g_vyy,g_vyz,g_vzz)
   SCALING_CORRECTION:do ii = 1,n_vscale
...
!      subroutine apply_vdw_reduction_factor_
...
!
!     Call Vdw kernel in CUDA using C2 nblist
!
...
#ifdef _OPENACC
   use utilgpu   ,only: dir_stream,def_stream&
      &,rec_stream,rec_event,stream_wait_async
#endif
   use virial
   implicit none
...
!!$acc parallel loop async
!!$acc         present(dibuff,xold_nl,yold_nl,zold_nl)
!      do i = 1,nbloc
!         iglob    = dibuff(i)
!         x(iglob) = xold_nl(iglob)
!         y(iglob) = yold_nl(iglob)
!         z(iglob) = zold_nl(iglob)
!         loc(iglob) = i
!      end do
