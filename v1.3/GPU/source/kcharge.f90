...
!
!       use special charge parameter assignment method for MMFF
!
...
!
!       copy original charge values that won't change during mutation
!
...
!
!     remove zero partial charges from the list of local charges
!
   nionloc = 0
...
!         chgloc(ionloc)= icap
   ...
   nionbloc = nionloc
!     nionlocloop is nionloc if nionloc is a multiple of 16, or the first one greater
...
!$acc         present(ineignl,chglist,chgglobnl,chglocnl)
   do i = 1, nlocnl
      iglob  = ineignl(i)
      ionloc = chglist(iglob)
...
!     nionlocnlloop is nionlocnl if nionlocnl is a multiple of 16, or the first one greater
...
!     ##############################################################
!     ##                                                          ##
!     ##  subroutine kchargem  --  assign MMFF charge parameters  ##
!     ##                                                          ##
!     ##############################################################
!
!
...
!
!
!     set and store MMFF base atomic partial charge values
!
   do i = 1, n
      it = type(i)
      pchg(i) = 0.0_ti_p
...
!
   allocate (pbase(n))
!
!     modify MMFF base charges using a bond increment scheme
!
   do i = 1, n
      pbase(i) = pchg(i)
   end do
...
!
!     subroutine dealloc_shared_chg : deallocate shared memory pointers for chg
!     parameter arrays
!
...
!
!     subroutine alloc_shared_chg : allocate shared memory pointers for chg
!     parameter arrays
!
