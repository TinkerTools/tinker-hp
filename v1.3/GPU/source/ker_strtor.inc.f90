...
!
!     calculate the bond-stretch for the first bond
!
   k   = ist(2,iistrtor)
   v1  = kst(1,iistrtor)
   v2  = kst(2,iistrtor)
...
!
      IF (IAND(ver,grd).NE.0) THEN
         ...
         dedzu = -dedphi * (xu*ycb - xcb*yu) * (ru2)
!
!     increment chain rule components for the second bond
!
...
!          if(dedxia-dedxia.ne.0.0)
!    &        print*,'xa',zcb,dedyt , ycb,dedzt
!          if(dedyia-dedyia.ne.0.0)
!    &        print*,'ya',xcb,dedzt , zcb,dedxt
!          if(dedzia-dedzia.ne.0.0)
!    &        print*,'za',ycb,dedxt , xcb,dedyt
!          if(dedxib-dedxib.ne.0.0)
!    &     print*,'xb',yca,dedzt,zca,dedyt,zdc,dedyu,ydc,dedzu,ddrdx
!          if(dedyib-dedyib.ne.0.0)
!    &     print*,'yb',zca,dedxt,xca,dedzt,xdc,dedzu,zdc,dedxu,ddrdy
!          if(dedzib-dedzib.ne.0.0)
!    &     print*,'zb',xca,dedyt,yca,dedxt,ydc,dedxu,xdc,dedyu,ddrdz
!          if(dedxic-dedxic.ne.0.0)
!    & print*,'xc',zba,dedyt,yba,dedzt,ydb,dedzu,zdb,dedyu,ddrdx,xcb,ddr
!             if(dedyic-dedyic.ne.0.0)
!    & print*,'yc',xba,dedzt,zba,dedxt,zdb,dedxu,xdb,dedzu,ddrdy,ycb,ddr
!             if(dedzic-dedzic.ne.0.0)
!    & print*,'zc',yba,dedxt,xba,dedyt,xdb,dedyu,ydb,dedxu,ddrdz,zcb,ddr
!             if(dedxid-dedxid.ne.0.0)
!    &           print*,'xd',zcb,dedyu,ycb,dedzu
!             if(dedyid-dedyid.ne.0.0)
!    &           print*,'yd',xcb,dedzu , zcb,dedxu
!             if(dedzid-dedzid.ne.0.0)
!    &           print*,'zd',ycb,dedxu , xcb,dedyu
!
...
!
      IF (IAND(ver,grd).NE.0) THEN
         ...
         dedzu = -dedphi * (xu*ycb - xcb*yu) * (ru2)
!
!     increment chain rule components for the third bond
!
         dedxia = dedxia + zcb*dedyt - ycb*dedzt
         dedyia = dedyia + xcb*dedzt - zcb*dedxt
         dedzia = dedzia + ycb*dedxt - xcb*dedyt
