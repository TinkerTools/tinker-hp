...
!
   if (.not.use_mlpot_only) call kewald
!
...
#ifdef _OPENACC
!
!     init cuda rand engine
!
   call init_curand_engine(n,rec_queue)
#endif
!
!     associate pointers routine
!
...
subroutine print_field
   ...
   use polpot,only:use_thole,use_tholed
   use potent
...
!
!     subroutine mechanicstep : fill the array parameters between two time steps
!
...
!
!     subroutine mechanicsteprespa: fill the array parameters between two time steps
!
...
!$acc wait
!        print*,use_strbnd,use_urey,use_angang
!        print*,use_angle,use_opbend,use_opdist
!        print*,use_improp,use_imptor,use_pitors
!        print*,use_strtor,use_tortor,use_geom
...
!
!     subroutine mechanicsteprespa1: fill the array parameters between two time steps
!
...
subroutine configure_routine
   ...
   use interfaces
   use charge ,only: nion
   use domdec ,only: rank,nproc,nrec
   ...
   use polpot ,only: use_tholed, use_thole&
   ...
   if (use_polar) then
      ...
      if      (configure.eq.conf_tmatxb_ref) then
         ...
         mlst_enable = .true.
33       format(/,' --- Tinker-HP: Enabling precomputation in polarisation solver')
         if (rank.eq.0.and.tinkerdebug.gt.0) print 33
      ...
      if      (configure.eq.conf_epreal1c_1) then
         ...
         use_mpolar_ker  =&
            ...
            &.and..not.use_tholed.and..not.use_group&
            &.and..not.use_chgflx)
         !TODO Add group
         call attach_mpolar_pointer
#endif
...
!     print '(/,A21,Z16,I30)', 'routine config number ',
...
!
!     Associate routine pointers related to
!           MPI Async Overlapping computation (Real V Rec)
!
...
subroutine gsync(cha,lin)
   ...
   character(*) cha
   integer  lin
   integer ierrSync,ierr

#ifdef _OPENACC
   !if (tinkerdebug.gt.0) then
      !ierrSync = cudaDeviceSynchronize()
      ierrSync = cudaStreamSynchronize(rec_stream)
   !else
   !   ierrSync = cudaGetLastError()
   !end if

120 format(" ------->>>> ",I3,A,":",I0)
125 format("MPIError ",I5," rank ",I3, A,":",I0)
126 format("Error ",I5," made launching kernel ",A,2x,I0,/&
          ,3x,A)

   call MPI_BARRIER(MPI_COMM_WORLD,ierr)
   if (ierrSync.ne.0) then
      write(0,126) ierrSync,cha,lin,cudaGetErrorString(ierrSync)
   end if
   if (ierr.ne.0) write(0,125), ierr,rank,cha,lin
   if (rank.eq.1) write(0,120) rank,cha,lin
#endif
end subroutine
