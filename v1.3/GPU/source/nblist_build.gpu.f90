...
!     #######################################################################
!     ##                                                                   ##
!     ##  subroutine nblist_vdw  -- set of vdw neighbor lists subroutines  ##
!     ##                                                                   ##
!     #######################################################################
!
!
...
!
!     spatial reordering of vdw data
!
...
!
!     build vdw block-block &  block-atoms neighbor list
!
...
!$acc    ,so_x,so_y,so_z,b_stat,b_rmid)
   ...
   call blocksat_kcu<<<nGS,128,0,rec_stream>>>&
        (atomk,atomgid,s_key,x,y,z,nb,na,nab,n,ctbuf&
   ...
   if (app_id==pimd_a .or. app_id==analyze_beads_a.or. app_id==pibar_a&
      .or.l%use2lists.or..not.orthogonal ) then
...
!
!     From Block-Block to compressed Block-Atoms list
!
   if (l%use2lists) then
...
!!$acc update host(b2pl,bapl,abpl)
!      do i = 0,trackb(3)-1
!         print*, i+1,b2pl(2*i+1),b2pl(2*i+2)
!      end do
!      do i = 0,trackb(1)-1
!         ibl = bapl(i+1)
!         if (ibl.eq.13.or.ibl.eq.14) then
!         iblv(:) = [(abpl(32*i+j),j=1,32)]
! 43      format( I6,32I5 )
!         print 43, ibl, iblv
!         end if
!      end do
...
!     call cu_filter_lst_sparse
!    &     (celle_glob,cell_scan,celle_x,celle_y,celle_z,matb_lst,
!    &      npolelocnlb,nblock,szMatb,npolelocnlb_pair,mbuf2,
!    &      eblst,cell_len,xcell,ycell,zcell,xcell2,ycell2,zcell2,
!    &      rec_stream)
!     call filter_lst_sparse<<<*,128,0,rec_stream>>>
!    &     (celle_glob,cell_scan,celle_x,celle_y,celle_z,matb_lst,
!    &      npolelocnlb,nblock,szMatb,npolelocnlb_pair,mbuf2,
...
!
!    "clist_block" performs a complete rebuild of the block
...
!     call filter_lst_sparse<<<*,128,0,rec_stream>>>
!    &     (celle_glob,cell_scan,celle_x,celle_y,celle_z,matb_lst,
!    &      nionlocnlb,nblock,szMatb,nionlocnlb_pair,mbuf2,
!    &      eblst,cell_len)
      call filter_lst_sparse1<<<*,128,0,rec_stream>>>&
         &(celle_glob,cell_scan,celle_x,celle_y,celle_z,matb_lst,&
         &nionlocnlb,nblock,szMatb,nionlocnlb_pair,cutb2,&
...
!
!    "vlistcell2gpu" performs a complete rebuild of the
...
!
!    "mlistcellgpu" rebuilds the regular electrostatic neighbor lists
!     for  multipoles using linked cells method
!
...
!     perform a complete list build
!
!     print*,'mean cell_len ',sum(cell_len)/size(cell_len)
!$acc parallel loop vector_length(32) private(j) async(rec_queue)
   do i = 1, npolelocnl
      iipole    = poleglobnl(i)
      iglob     = ipole(iipole)
...
!
!    "mlistcell2gpu" performs a complete rebuild of the
...
!
!    "clistcell2gpu" performs a complete rebuild of the
