...
!$acc         default(present)
      do i = 1, npolebloc; do j = 1, 3
         ...
         if (polarity(iipole).eq.0) then
            uind(j,iipole) = 0.0_ti_p
            uinp(j,iipole) = 0.0_ti_p
         else
...
!     real(t_p),dimension(10,npolerecloc)::cmp,fmp
   integer, allocatable :: reqrec(:),reqsend(:)
...
!     real(t_p),dimension(3,npolerecloc)::fuind,fuinp
!     real(t_p),dimension(10,npolerecloc)::fdip_phi1,fdip_phi2
...
!
         if (repart(iglob).ne.rank) then
            fuind(k,i) = a(k,1)*murec(1,1,i)+ a(k,2)*murec(2,1,i)+ a(k,3)*murec(3,1,i)
            fuinp(k,i) = a(k,1)*murec(1,2,i)+ a(k,2)*murec(2,2,i)+ a(k,3)*murec(3,2,i)
         else
            fuind(k,i) = a(k,1)*mu(1,1,iloc)+ a(k,2)*mu(2,1,iloc)+ a(k,3)*mu(3,1,iloc)
            fuinp(k,i) = a(k,1)*mu(1,2,iloc)+ a(k,2)*mu(2,2,iloc)+ a(k,3)*mu(3,2,iloc)
         end if
      end do
   end do
!$acc end host_data
...
!
            dipfield(k,1,iloc) = a(1,k)*fdip_phi1(2,i)+ a(2,k)*fdip_phi1(3,i)+ a(3,k)*fdip_phi1(4,i)
            dipfield(k,2,iloc) = a(1,k)*fdip_phi2(2,i)+ a(2,k)*fdip_phi2(3,i)+ a(3,k)*fdip_phi2(4,i)
         else                                                                       
            dipfieldbis(k,1,i) = a(1,k)*fdip_phi1(2,i)+ a(2,k)*fdip_phi1(3,i)+ a(3,k)*fdip_phi1(4,i)
            dipfieldbis(k,2,i) = a(1,k)*fdip_phi2(2,i)+ a(2,k)*fdip_phi2(3,i)+ a(3,k)*fdip_phi2(4,i)
         end if
      end do
   end do
!$acc end host_data
