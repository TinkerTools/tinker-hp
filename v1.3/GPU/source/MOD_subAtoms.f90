...
!     #######################################################################
!     ##                                                                   ##
!     ##  module subAtoms  --  number, position and type of current atoms  ##
!     ##                                                                   ##
!     #######################################################################
!
!
...
submodule(atomsMirror) subAtoms
   ...
   use cell
   use domdec    ,only: rank,nproc,hostcomm,COMM_TINKER,nloc,nbloc,glob
...
!$acc parallel loop copy(ierr) async default(present)
         do i = 1,nloc
            iglob = glob(i)
            if (use(iglob)) then
               if (abs(x(iglob)).gt.3*xcell2) then
                  !print*, "Too Far X", rank, iglob, real(x(iglob),4), real(v(1,iglob)*dt,4)
!$acc atomic
                  ierr = ierr + 1
               end if
               if (abs(y(iglob)).gt.3*ycell2) then
                  !print*, "Too Far Y", rank, iglob, real(y(iglob),4), real(v(2,iglob)*dt,4)
!$acc atomic
                  ierr = ierr + 1
               end if
               if (abs(z(iglob)).gt.3*zcell2) then
                  !print*, "Too Far Z", rank, iglob, real(z(iglob),4), real(v(3,iglob)*dt,4)
!$acc atomic
                  ierr = ierr + 1
               end if
            end if
         ...
         if (ierr.gt.0) then
      66    format(4x,"I Rank ",I0," encouter ",I0   &
                  ," excessive Integration(s) due to high velocities")
            write(0,66) rank,ierr
            __TINKER_FATAL__
         end if
