...
!
! conversion factors to go from TINKER's units to PLUMED
!
! kcal/mol -> kJ/mol
...
! fs -> ps
      ...
      if (use_reps) then
          call plumed_f_gcmd("GREX setMPIFIntercomm"//char(0),&
          &COMM_ROOT2ROOT)
...
!
! cell vectors
! check for non orthorhombic systems whether it's by rows or by columns)
      call plumed_f_gcmd("setBox"//char(0),lvec)
!
! virial should be both input and out
! unclear if the PLUMED virial is correct
...
!     if (iand(ncount,511).eq.0)
!    &print '(A,f16.8,d19.10,I7)'
!    &     ,'plu',sum(pl_virial(:,:)**2),sum(pl_force(:,1:nloc)**2)
!    &     ,ncount
...
!     !pl_force is in kcal/mol/A; no conversion should be needed
!
      if (ftot_l) then
