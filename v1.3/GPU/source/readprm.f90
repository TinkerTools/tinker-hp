...
subroutine readprm
   ...
   use katoms
   use kbonds
   use bndpot, only: flatbottom_delta
   use kcflux
   use kchrge
   use kcpen
   use kctrn
   ...
   integer nb,nb5,nb4,nb3,nel,nbm,nbm4,nbflt
...
110      continue
         ...
         bmor4(2,nbm4) = bd
         bmor4(3,nbm4) = ep
      else if (keyword(1:8) .eq. 'FLATBTM ') then
         ia = 0
         ib = 0
         fc = 0.0d0
         bd = 0.0d0
...
200      continue
         ...
         angf(2,naf) = pr
!
!     Partridge-Schwenke bond angle bending parameters
!
...
!              ku(nu) = pc//pb//pa
         ...
         dst13(nu) = ds
!
!     Partridge-Schwenke-type Angle repulsion (HH potential)
!
...
430      continue
         ...
         multip(13,nmp) = pl(13)
!
!     atomic emtp formula parameters
!
...
!
!     MMFF out-of-plane bend parameters
!
...
!              kopb(nopb) = pa//pb//pd//pc
         ...
         opbn(nopb) = fc
!           if (ic.gt.0 .or. id.gt.0) then
!              nopb = nopb + 1
!              if (ib .le. id) then
!                 kopb(nopb) = pc//pb//pb//pd
!              else
!                 kopb(nopb) = pc//pb//pd//pb
!              end if
!              opbn(nopb) = fc
!              nopb = nopb + 1
!              if (ia .le. ic) then
!                 kopb(nopb) = pd//pb//pa//pc
!              else
!                 kopb(nopb) = pd//pb//pc//pa
!              end if
!              opbn(nopb) = fc
!           end if
!
!     MMFF torsional parameters
!
...
!
!     MMFF bond charge increment parameters
!
...
!
!     MMFF partial bond charge increment parameters
!
...
680      continue
         ...
         ncfb = ncfb + 1
         if (ia .lt. ib) then
            call front_convert_base(0,ia,ib,kcfb(ncfb))
            cflb(ncfb) = cfb
         else if (ib .lt. ia) then
            call front_convert_base(0,ib,ia,kcfb(ncfb))
            cflb(ncfb) = -cfb
         else
            call front_convert_base(0,ia,ib,kcfb(ncfb))
            cflb(ncfb) = 0.0d0
...
690      continue
         ...
         ncfa = ncfa + 1
         if (ia .le. ic) then
            call front_convert_base(ia,ib,ic,kcfa(ncfa))
         else
            call front_convert_base(ic,ib,ia,kcfa(ncfa))
         end if
         cfla(1,ncfa) = cfa1
         cfla(2,ncfa) = cfa2
         cflab(1,ncfa) = cfb1
