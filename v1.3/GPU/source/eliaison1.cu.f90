...
module eliaisoncu
   ...
   use bndpot   ,only: BND_HARMONIC,BND_MORSE,BND_NO_TYPE, BND_MORSE4,BND_FLATBOTTOM
...
!
!       torsion-torsion interaction kernel energy term
!
...
!
!       eliaison1_cu gathers all bonded routines in a single kernel
!
attributes(global) subroutine __Cat(eliaison1_kcu,__sufx__)&
   &(& !bond
   &bndglob,ibnd,bl,bk,ba,typeAtm,nbondloc,bndunit,cbnd,qbnd,bndtypI&
   &,use_polymer,deb&
   &,off_bond,num_bond&
   &&!urey
...
integer  ,device,intent(in),dimension(*)::&
   ...
   &,imptorglob,iopb,angtypI,bndtypI,typeAtm,atomic,loc,iuse
integer  ,device,intent(in)             ::&
   ...
   &,aMDwattype(2)
!       logical  ,device,intent(in):: iuse(0:n)
real(t_p),device,intent(in),dimension(*)::&
   &x,y,z,bl,bk,ba,ul,uk,opbk,anat,ak,afld,kpit,vprop,kprop
...
!
            ...
            deddt = bndunit * force * dt&
               ...
               &+ 4._ti_p*ba2*dt2)
         else if (bndtyp_i .eq. BND_FLATBOTTOM) then
            dt = dt - alp
            if (dt > 0.0_ti_p) then
               dt2 = dt*dt
               e = bndunit *force *dt2
...
!
!       increment the out-of-plane bending energy and gradient and virial tensor
!
#if __tver__ & __use_ene__
...
!
               ...
               dedzid = ycb*dedxu - xcb*dedyu
!
!       calculate improper dihedral energy and derivatives
!
#if __tver__ & __use_ene__
               !call atomic�addçm( eid,e )
               enrgy = enrgy + tp2enr(e)
#endif
...
!
!       increment chain rule components for the first angle
!
               ...
               dedzid = ycb*dedxu - xcb*dedyu
!
!       get the angle-torsion values for the second angle
!
               v1 = kant(4,iiangtor)
               v2 = kant(5,iiangtor)
               v3 = kant(6,iiangtor)
...
!
               ...
               dedzu = dedphi * (xcb*yu-ycb*xu) / (ru2*rcb)
!
!       increment chain rule components for the second angle
!
