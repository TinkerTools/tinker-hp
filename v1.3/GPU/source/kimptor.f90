                     ...
                     if (btc .eq. itc) then
                        do k = 1, 6
                           if (kti(j) .eq. pt(k)) then
                              ...
                              ! Build system kti
                              if (.not.is_find8(kti_sys(1),ntis,kti(j))) then
                                 ntis = ntis + 1
                                 kti_sys(ntis) = kti(j)
                              end if
                           end if
...
!
      ...
      do i = 1, nkey
         ...
         if (keyword(1:8) .eq. 'IMPTORS ') then
            ...
            string = record(next:240)
            read (string,*,err=60,end=60)  ia,ib,ic,id,(vt(j),st(j),ft(j),j=1,3)
   60       continue
            if (min(ia,ib,ic,id) .lt. 0) then
               ...
               if (header .and. .not.silent) then
                  header = .false.
                  write (iout,70)
   70             format (/,' Additional Improper Torsion Specific Parameters :'&
   ...
   80             format (2x,4i4,3x,3(f9.3,f6.1))
               ...
               do j = 1, nitors
                  ...
                  itc = iitors(3,j)
                  itd = iitors(4,j)
                  if (ia.eq.ita .and. ib.eq.itb .and.ic.eq.itc .and. id.eq.itd) then
                     itors1(1,j) = vt(1)
                     itors1(2,j) = st(1)
                     itors2(1,j) = vt(2)
                     itors2(2,j) = st(2)
...
!$acc end atomic
            ...
            if (.not. done) then
               call front_convert_base(0,0,itb,itc,itd,pt(1))
               call front_convert_base(0,0,ita,itc,itd,pt(2))
               call front_convert_base(0,0,itd,itc,itb,pt(3))
               call front_convert_base(0,0,ita,itc,itb,pt(4))
               call front_convert_base(0,0,itd,itc,ita,pt(5))
               call front_convert_base(0,0,itb,itc,ita,pt(6))
               do j = 1, ntis
                  call back_convert_base(bte,bta,btb,btc,btd,kti_sys(j))
                  if (btc .eq. itc) then
                     do k = 1, 6
