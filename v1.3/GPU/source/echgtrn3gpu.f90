...
!     ################################################################
!     ##                                                            ##
!     ##  subroutine echgtrn3  --  charge transfer energy & derivs  ##
!     ##                                                            ##
!     ################################################################
!
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine echgtrn1cgpu  --  charge transfer derivs via list  ##
!     ##                                                                ##
!     ####################################################################
!
!
!     "echgtrn1cgpu" calculates the charge transfer energy and first
!     derivatives using a pairwise neighbor list on device
!
!
...
!      call echgtrn3l_kcu<<<gS,BLOCK_DIM,0,def_stream>>>
!           ( ipole_s,pglob_s,loc_s,ieblst,eblst,x_s,y_s,z_s,chgct&
!           , dmpct,mutInt,grplist,wgrp&
!           , dect,ered_buf1,lam_buff,vred_buff,nred_buff&
!           , npolelocnl, nbloc, npolelocnlb, npolelocnlb2_pair&
!           , ctrntyp_ID, use_group, use_lambdadyn&
!           , f,elambda,ctrnshortcut,ctrnshortcut2,off2,off,cut2,shortheal,rinv&
!           , p_xbeg,p_xend,p_ybeg,p_yend,p_zbeg,p_zend&
!           , mcorrect_ik, mcorrect_scale, ipole, loc, x, y, z&
!           , n_mscale&
!           )
!      call check_launch_kernel("echgtrn3l_kcu")
!!$acc end host_data
   else
!$acc host_data use_device(ipole_s,pglob_s,loc_s,ieblst,eblst &
!$acc    ,x_s,y_s,z_s &
!$acc    ,chgct,dmpct,grplist,wgrp,mutInt,dect,ered_buf1,lam_buff,vred_buff,nred_buff &
!$acc    ,mcorrect_ik,mcorrect_scale,ipole,loc,x,y,z)
      call echgtrn3_kcu<<<gS,BLOCK_DIM,0,def_stream>>>&
          ( ipole_s,pglob_s,loc_s,ieblst,eblst(st),x_s,y_s,z_s&
          , chgct,dmpct,mutInt,grplist,wgrp&
          , dect,ered_buf1,lam_buff,vred_buff,nred_buff&
          , npolelocnl, nbloc, npolelocnlb, npolelocnlb2_pair&
          , ctrntyp_ID, use_group, use_lambdadyn&
          , f,elambda,ctrnshortcut,ctrnshortcut2,off2,off,cut2,shortheal,rinv&
