...
!
      ...
      if (bndtyp.eq.'MORSE') then
         bndtyp_i = BND_MORSE
         write(*,*) ' Defaulting to MORSE bond type '
      else if (bndtyp.eq.'MORSE4') then
         bndtyp_i = BND_MORSE4
         write(*,*) ' Defaulting to MORSE4 bond type '
      else if (bndtyp.eq.'FLATBTM') then
         bndtyp_i = BND_FLATBOTTOM
         write(*,*) ' Defaulting to FLATBTM bond type '
      else if (bndtyp.eq.'HARMONIC') then
         bndtyp_i = BND_HARMONIC
         write(*,*) ' Defaulting to HARMONIC bond type '
      else
   ...
   else if (keyword(1:14) .eq. 'FLATBTM-DELTA ') then
      read (string,*,err=10,end=10)  flatbottom_delta
!
...
!
   ...
   else if (keyword(1:18) .eq. 'POLAR-PREDICT-DIM ') then
      read(string,*,err=10,end=10) maxualt
      if (maxualt.gt.maxualt_prm) then
         maxualt = maxualt_prm
         write(*,*) ' WARNING: POLAR-PREDICT-DIM cannot exceed ',maxualt_prm
         write(*,*) ' WARNING:      ReSetting maxualt_prm !'
      end if
