...
!
!
!     "epolar1cu" calculates the induced dipole polarization energy
...
#include "pair_polar.inc.f90"
   ...
   attributes(global) subroutine epreal1c_core_cu&
      ...
      &, x, y, z, rpole, pdamp, tholed, thole, uind, uinp&
   ...
   real(t_p),device,intent(in):: x(*),y(*),z(*),rpole(13,*)&
      &,pdamp(*),tholed(*),thole(*),uind(3,*),uinp(3,*)
   ...
   do ii = iwarp, npolelocnlb_pair, nwarp
      ...
      kpgm1(threadIdx%x)   = tholed(kpole)
...
attributes(global) subroutine epreal3_cu&
   ...
   &, use_short, use_tholed&
   &,p_xbeg,p_xend,p_ybeg,p_yend,p_zbeg,p_zend )
implicit none
integer,value,intent(in)::npolelocnlb,npolebloc,n&
   &,npolelocnlb_pair
logical,value,intent(in)::use_short,use_tholed
real(t_p),value:: p_xbeg,p_xend,p_ybeg,p_yend&
   &,p_zbeg,p_zend&
   ...
   do j = 1,warpsize
      ...
      if (do_pair.and.d2<=off2.and.accept_mid) then
         ...
         call epolar3_couple(dpui,ip,dpuk(klane),kp_,d2,pos&
            &,aewald,alsq2,alsq2n,pgm,pdp,use_tholed&
