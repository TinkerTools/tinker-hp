...
!          rik     =  f_sqrt(rik2)
!          rik6    =  rik2 * rik2 * rik2
!          rik7    =  rik2 * rik2 * rik2 * rik
!          dtau    =  (rik + dhal * rv2)**(-1) !tau / (dhal + 1.0)
!          rv7     =  rv2**7
!          rv7orho =  rv7 / (rik7 + ghal * rv7)
!          tau7    =  (dtau * (dhal + 1.0))**7
!          !tau7    =  tau ** 7.0
!          gtau    =  eps2 * tau7 * rik6
!    &              * (ghal + 1.0) * rv7orho * rv7orho * vscale
!          e       =  eps2 * tau7 * rv7
!    &              * ((ghal + 1.0) * rv7orho - 2.0) * vscale
!          de      = - 7.0 * (dtau*e + gtau)
!          if (iand(fea,__use_shortRange__+__use_longRange__).gt.0) then !CONST TEST
!             call switch_respa_inl(rik,shortcut,shortheal,s,ds)
!             if (iand(ver,__use_grd__).gt.0) de = ( e*ds + de*s )/rik
!             if (iand(ver,__use_ene__).gt.0) e  = e*s
!          end if
!
...
!             taper  =  c5 * rik2*rik2*rik + c4 * rik2*rik2
!    &                + c3 * rik2*rik      + c2 * rik2
!    &                + c1 * rik           + c0
!             dtaper =  5.0 * c5 * rik2*rik2
!    &                + 4.0 * c4 * rik2*rik
!    &                + 3.0 * c3 * rik2
!    &                + 2.0 * c2 * rik
!    &                +       c1
...
parameter (ene=__use_ene__&
   ...
   &)
!          real(t_p) ds1dlambda,ds2dlambda,dt1dlambda,dt2dlambda
!
!          compute the energy contribution for this interaction
!
rik  = f_sqrt(rik2)
