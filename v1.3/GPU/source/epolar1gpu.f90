...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine epolar1c  --  Ewald polarization derivs via list  ##
!     ##                                                               ##
!     ###################################################################
!
!
...
#endif
!
!     Sum contribution of all energy
!
...
!     #################################################################
!     ##                                                             ##
!     ## subroutine epreal1cpgu -- Ewald real space derivs via list  ##
!     ##                                                             ##
!     #################################################################
!
!
...
subroutine epreal1cgpu
   ...
   if (f_in) then
      f_in=.false.
!
!     zero out temporary accumulation virial components
!
      vxx = 0.0_re_p
      vxy = 0.0_re_p
      vxz = 0.0_re_p
...
!
         ...
         if (use_tholed) then
            pgama1 = min( ddi,tholed(kpole) )
            if (pgama1.eq.0.0) pgama1=max(ddi,tholed(kpole) )
         end if
...
!
         ...
         call epolar1_couple(dpui,ip,dpuk,kp,r2,pos,&
            ...
            &dscale,pscale,uscale,use_tholed,&
...
!
         ...
         call epolar1_couple(dpui,ip,dpuk,kp,r2,pos&
            &,aewald,alsq2,alsq2n,pgamma,pgama1,damp,f&
            &,1.0_ti_p,1.0_ti_p,1.0_ti_p,use_tholed&
            &,use_chgflx,poti,potk&
            &,e,frc,frc_r,trqi,trqk,.false.)
!
!     increment energy
...
!$acc    ,iseblst_s,eblst_s,seblst_s,x_s,y_s,z_s,rpole,pdamp,tholed &
...
!$acc    ,pot,dep,trqvec,ered_buff,vred_buff)
      ...
      if (use_polarshortreal) then
         ...
         call epreal1c_core_cu<<<gS,BLOCK_DIM,0,def_stream>>>&
            ...
            &,x_s,y_s,z_s,rpole,pdamp,tholed,thole,uind,uinp&
            &,pot,dep,trqvec,ered_buff,vred_buff&
            &,npolelocnlb,nspnlb2,npolebloc,n,use_chgflx,use_tholed&
         ...
         do i = 1,ndec
            ...
            call epreal1c_core_cu<<<gS,BLOCK_DIM,0,def_stream>>>&
               ...
               &,x_s,y_s,z_s,rpole,pdamp,tholed,thole,uind,uinp&
               &,pot,dep,trqvec,ered_buff,vred_buff&
               &,npolelocnlb,sized,npolebloc,n,use_chgflx,use_tholed&
               &,off2,f,alsq2,alsq2n,aewald&
               &,p_xbeg,p_xend,p_ybeg,p_yend,p_zbeg,p_zend)
      ...
      call epreal1c_scaling_cu<<<gS1,BLOCK_DIM,0,def_stream>>>&
         ...
         &, polelocnl,x,y,z,pdamp,tholed,thole,rpole,uind,uinp&
         &, pot,dep,trqvec,ered_buff,vred_buff&
         &, n,n_dpuscale,nbloc,use_chgflx,use_tholed&
...
!$acc     present(poleglobnl,ipole,rpole,thole,pdamp,tholed, &
...
!
!     store the potential at each site for use in charge flux
!
      if (use_chgflx) then
!$acc atomic
         pot(i)    = pot(i) + poti
...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine eprecip1  --  PME recip polarize energy & derivs  ##
!     ##                                                               ##
!     ###################################################################
!
!
...
!
   ...
   nf3     = (nfft3+1) / 2
!
!     remove scalar sum virial from prior multipole 3-D FFT
!
...
!     fphip(1,:) = 0.0_ti_p !flush first line to zero? never use after
!     fphid(1,:) = 0.0_ti_p !flush first line to zero? same
...
!
!     induced dipole contribution to the internal virial
!
...
!
!
!     Proceed to atomic update to avoid collision with direct queue
...
!
!     get contribution of the reciprocal permanent electric field to delambdae
!
   if (use_lambdadyn) then
!$acc parallel loop async(rec_queue) default(present) present(delambdae) &
!$acc reduction(+:delambdae)
...
!!$acc update host(vxx,vxy,vxz,eprec) async(rec_queue)
!!$acc update host(deprec,fmp) async
!!$acc update host(fphip,fphid,fphidp) async(rec_queue)
!      do i = 0,nproc-1
!         if (rank.eq.i) then
!         print*,comput_norm( uind,3*n,2),
!     &          comput_norm( uinp,3*n,2),
!     &          comput_norm(fuind,3*npolerecloc,2)
!         print*,comput_normr(deprec,3*nlocrec2,2)
!     &         ,sum(abs(a))
!     &         ,comput_norm(fuinp,3*npolerecloc,2)
!          print*,vxx,vxy,eprec,ep
!          print*,comput_norm(qgrid2in_2d,
!     &       2*n1mpimax*n2mpimax*n3mpimax*(nrec_send+1),2)
!         end if
