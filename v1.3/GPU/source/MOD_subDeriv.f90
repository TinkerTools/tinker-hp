...
!     ##                                                                ##
!     ##  module subderiv                                               ##
!     ##  --  Routines associated to derivatives component managment    ##
!     ##                                                                ##
!     ####################################################################
!
...
11       format ( " WARNING !!! ConfigPotential Routine ",/,&
      ...
      block
         ...
         ftot_l = (integrate.eq.'RESPA'.or.respa1_l&
            ...
            &.or.integrate.eq.'BEEMAN').and.(app_id.ne.pimd_a.and..not.gamd_l)
...
!$acc parallel loop collapse(2) &
!$acc         present(derivs,desave) async
         do i = 1,nbloc; do j = 1,3
...
!$acc parallel loop collapse(2) async default(present)
         ...
         !call mmo(de_mpi,3*dr_stride,"deri")
...
16       format(' ERROR! routine comm_forces_dd'&
      ...
      ! Required, for buffMpi_p1 is real(r_p) and buffer mdyn_rtyp 
...
!$acc wait(rec_queue)
      ...
      !   - derivs must be ready before sending
      !   - buffer must be associated through c_f_pointer before reception
...
13    format(F30.10)
   ...
   module subroutine info_forces(rule)
      ...
      integer,parameter:: nf=29  !number of forces
      integer i,j,sze,ids
      ...
      if (btest(rule,idBond)) then
         ...
         if (.not.fdebs_l) then
            ...
            if(use_mlpot.or.use_ml_embedding)&
               &call minmaxone(mmx(29),mmx(nf+29),mmx(2*nf+29),dmlpot&
               &,sze,'dmlpot');
...
!     if (allocated(deamdD)) then
!        call minmaxone(mmx(23),mmx(nf+23),mmx(2*nf+23),deamdD ,3*nloc);
!        call minmaxone(mmx(24),mmx(nf+24),mmx(2*nf+24),deW1aMD,3*nloc);
!     end if
...
40       format(80('='))
         ...
         if(mmx(2*nf+29)/=0.0) print 30,'dmlpot =>',&
            &mmx(29),mmx(29+nf),mmx(29+nf*2)
      end if
