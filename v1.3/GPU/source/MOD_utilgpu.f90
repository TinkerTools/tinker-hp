...
!     ####################################################################
!     ##                                                                ##
!     ##  module utilgpu  -- GPU utility functions and params           ##
!     ##                                                                ##
!     ####################################################################
!
!     ngpus        numbers of gpus available
!     devicenum    device attach to MPI hostrank
!     gpu_gangs    attribute #gangs to create inside a kernels
!     gpu_workers  attribute #workers for parallelism mapping
!     gpu_vector   attribute #vector_length in kernels or parallel clauses
!     dir_queue    direct space computation async queue
!     rec_queue    reciprocal space computation async queue
!     def_queue    default async queue
!     Nblock       number of k block kernel comput in direct space
!     maxscaling   maximum number of scaling factor of an atom among interactions 1-2 to 1-5
!     maxscaling1  maximum number of scaling factor of an atom among p_interactions
!     warning      warning message to print in case of trouble
!     ftc          a transformation matrix to convert a multipole object in fraction coordinates to Cartesian
!     ctf          a transformation matrix to convert a multipole object in Cartesian coordinates to fractional
!     qi1 and qi2  are useful to compute ftc and ctf
!     devProp      Properties of the device
!     nSMP         number of streaming Multi-processor on the device
!     nSPcores     number of cores od the device
!
!
...
!     ########################################################################
!     ##                                                                    ##
!     ##  subroutine imagevec_acc  --  compute the minimum image distance  ##
!     ##                                                                    ##
!     ########################################################################
!
...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine setscale2_acc  --  compute the scaling factors    ##
!     ##                                                               ##
!     ###################################################################
!
!
!     "setscale2" takes the indice of the reference atom, the global indices
!     of the neighbours, the scaletype and returns the scaling factors
...
!     character(len=1) :: scaletype_temp
!     scaletype_temp = TRANSFER(scaletype,MOLD=scaletype_temp)
!deb  if(rank.eq.0.and.tinkerdebug) print*, 'setscale2_acc'
      if(nb>nscal-ntot) then
         print*, 'Data collision in scal array\n',&
            &'Fix setscale2_acc by extending scal size'
      endif
!
!     select scaling coefficients
!
...
!           scale5  = c5scale
      ...
      endselect
!
!     get list of atoms indices and there scaling factor
!     give scalinf factors to selected atoms
!$acc loop vector
      do i=1,nn12
...
!
!     Find where additional factor for 1-4 intragroup polarization
...
!
!     for all indices in nblist, select the corresponding atom
!     and apply his scaling factor in kvec indexation
!
!$acc loop vector
      do i=1,nb
         j = kvec(i)
...
!
!     Replace scaling factor along kvec index
!
!$acc loop vector
      do i=1,ntot
         j=iscal(ntot+i)
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine setscale2p_acc  --  compute the scaling factors    ##
!     ##                                                                ##
!     ####################################################################
!
!
...
!deb  if(rank.eq.0.and.tinkerdebug) print*, 'setscale2p_acc'
...
!
!     for all indices in the lists, select the corresponding atom
!
!$acc loop vector
      do i=1,nb
         j = kvec(i)
...
!     character*1, intent(in) :: scaletype
...
!
!     updating the number of gangs to launch on the gpu if authorized
!
...
#ifdef _OPENACC
!
!     Allocate cache memory for Thrust cache library
!
...
!     Link a device to a MPI process
!     If not bind by slurm local ID
!
      host_rank = hostrank
      if (devicenum==-1) then
         device_type = acc_get_device_type()
...
!
!     Query device(s) propertie(s)
!
      call getDeviceProp
!
!     Display device number and MPI process
!
      status = hostnm(name)
...
!
!     Create stream(s) and attach them to Acc queue
!
...
!
!     Creating Event for stream syncrhonisation
!
      cuda_success = cuda_success +&
...
!12      format('CUDA Error destroying rec/direct streams',I10)
!        if (cuda_success.ne.0) then
!           print 12, cuda_success
!        end if
13       format(/,1x,'***** ',&
...
!
!     A cuda implementation of _pgi_acc_wait_async
!     Synchonisation within CUDA streams doesn't seem to work with OpenACC
!
