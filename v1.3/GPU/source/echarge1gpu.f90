...
!
!     "ecreal1dgpu" evaluates the real space portion of the Ewald sum
!     energy and forces due to atomic charge interactions, using a neighbor list
!
...
!
!     Scaling interaction correction subroutines
!
...
subroutine ecreal_lj1_cu
   ...
   use cutoff , only: ewaldcut,shortheal,vdwshortcut
   use deriv
   use domdec
   use eChgLjcu
   use energi , only: ec=>ec_r,calc_e
...
!$acc   )
   ...
   if (v0) then
      call echg_lj1_kcu_v0<<<gS,BLOCK_DIM,0,def_stream>>>&
         ...
         &, cut2,vdwshortcut,off2,off,rinv,shortheal&
...
!      sdx    = size(d_x)
!      gridS  = dim3((sdx-1)/TRP_BLOCK_DIM+1,1,1)
!      blockS = dim3(TRP_BLOCK_DIM,3,1)
!!$acc host_data use_device(d_x,dec,loc_s)
!      call transpose_z3fl<<<gridS,blockS,0,def_stream>>>
!     &                ( d_x,dec,sdx,nionlocnl,loc_s )
!      call check_launch_kernel(" transpose_z3fl ")
!!$acc end host_data
!      end block
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine ecrecip1gpu  --  PME recip charge energy & derivs  ##
!     ##                                                                ##
!     ####################################################################
!
!
...
#ifdef _OPENACC
   use interfaces ,only: grid_pchg_sitecu
#endif
   use math
   use mutant, only: mut,elambda
   use pme
...
!
   ...
   if (use_lambdadyn) then
      j = max(nionrecloc,1)
!        call prmem_request(cphirec,1,j,async=.false.)
...
!         call fphi_to_cphi_chg_sitegpu (fphirec,cphirec)
