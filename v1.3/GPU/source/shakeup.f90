...
subroutine readrattlekeys(fill_arrays)
   use angle
   use atmlst
   use atmtyp
   use atoms
...
!
      call readrattlekeys(.false.)
      call alloc_shared_shake
      call readrattlekeys(.true.)
...
subroutine alloc_shared_shake
   ...
   if (allocated(iratx) .and. size(iratx).eq.nratx) return
!
   ...
   allocate(iratx(nratx))
   allocate(kratx(nratx))
   allocate(irat(2,nrat))
   allocate(krat(nrat))
   allocate(ratimage(nrat))
end
