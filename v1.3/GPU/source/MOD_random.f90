...
#include "tinker_macro.h"
   ...
   logical compute
#ifdef _OPENACC
!      type(curandStateXORWOW),pointer :: curng(:)
!      integer Ncurng
!!$acc declare device_resident(curng)
!      parameter (Ncurng=5120)
...
!
      ...
      if (seed_save.eq.0) then
         ...
         seed = seed + 60*minute + second + 10*ranktot
!
...
#ifdef _OPENACC
!
!      All functions within this scope are specific to the GPU
!
!     Inititiate curand generator
!
!      subroutine init_curand_engine
!      implicit none
!      type(c_ptr) :: c_curng
!      type(c_devptr) :: cdev_curng
!      integer i
!      write(*,*),'***** Init curand generator engine with seed ',
!     &          ,seed, ' ***** '
!
!      if (first) call init_rand_engine
!
!      cdev_curng = acc_malloc(Ncurng*sizeof(curandStateXORWOW))
!      c_curng    = acc_hostptr(cdev_curng)
!c
!c     Translate c to fortran pointer
!c
!      call c_f_pointer(c_curng,curng,(/Ncurng/))
!
!!$acc parallel loop deviceptr(curng)
!      do i = 1,Ncurng
!         call curand_init(seed_save,i,0,curng(i))
!      end do
!      end
...
!
!     Select the cuRand pseudo Generator type
!
...
!
!     get a sample following uniform distribution on GPU
!
...
!
!     get a sample following normal distribution on GPU
!
...
!
!     get a sample following normal distribution on GPU with forced simple precision
!
...
!
!     Following the principle of ranvec but using the spherical approch
!     to minimize calls to cuRand library and also operates on an array
!
...
!
!     get a pair of random spherical coordinates
!
...
!
!     get a sample following uniform distribution
!
...
!
!     get a sample following normal distribution
!
