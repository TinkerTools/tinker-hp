...
!     ###################################################################
!     ##                                                               ##
!     ##  module utilvec  --   vectorized utility functions            ##
!     ##                                                               ##
!     ###################################################################
!
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine imagevec --  Use imagevec to compute minimal       ##
!     ##                          distance                              ##
!     ##                                                                ##
!     ####################################################################
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine image3dvec -- Use imagevec to compute minimal       ##
...
!     #################################################################
!     ##                                                             ##
!     ##  function setscale2  --  compute the scaling factors        ##
!     ##                                                             ##
!     #################################################################
!
!
...
!deb  if(rank.eq.0.and.tinkerdebug) write(*,*) 'setscale2'
...
!
!    get number of atoms  directly (1-2) or 1-3 or 1-4 bonded
!
      nnp11 = np11(indice)
      nn12  = n12 (indice)
      nn13  = n13 (indice)
...
!
!     imax is the biggest indice. Reduces the number of loops
!     to the minimum possible
      imax   = max(nnp11,nn12,nn13,nn14,nn15)
      indmax = (int(imax / 4 ) + 1) * 4 ! First multiple of 4
      indmax = merge(imax,indmax , mod(imax,4 ).eq.0)
!
!  initialize scaling coefficients for all tomes and  for additional factor
!  for 1-4 intragroup polarization
!
...
!DIR$ VECTOR ALIGNED
      ...
      enddo
!     default value for indices in the list of bounded atoms
!     indice idefault cannot be of a real atom
!     indice ldefault cannot be of a real atom
...
!
      select case (scaletype)
       ...
       case('pscale')
         ...
         scale5  = p5scale
!
!                       Find where additional factor for 1-4 intragroup
!                       polarization should be taken into account
!
!DIR$ ASSUME (MOD(indmax,4).eq.0)
!DIR$ SIMD
         do k = 1, nmax
...
!DIR$ ASSUME (MOD(nmax,16).eq.0)
      ...
      endselect
!
!     for all atoms in the list, give the correct scale factor
!
!DIR$ ASSUME (MOD(indmax,4).eq.0)
!DIR$ ASSUME (MOD(nmax,16).eq.0)
!DIR$ SIMD
...
!     #################################################################
!     ##                                                             ##
!     ##  function setscale2p  --  compute the scaling factors       ##
!     ##                                                             ##
!     #################################################################
!
!
...
!DIR$ ATTRIBUTES ALIGN:64::coeffvec
      real(t_p) coeffvec(nmax)
!
!     default value for indices in the list of bounded
!     indice idefault cannot be of a real atom
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine setscale -- set interaction or exclusion scaling   ##
!     ##                         coefficients for connected atoms       ##
!     ##                                                                ##
!     ####################################################################
!
...
!     ####################################################################
!     ##                                                                ##
!     ##  subroutine setscalep -- set interaction or exclusion scaling  ##
!     ##                         coefficients for connected atoms       ##
!     ##                                                                ##
!     ####################################################################
!
