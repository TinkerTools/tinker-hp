...
!
!
!     "bsplgen" gets B-spline depth 4 coefficients and derivatives for
...
!       "bsplgen" gets B-spline depth 2 coefficients and derivatives for
!       a single PME atomic site along a particular direction
!        ( Optimized for partial charge )
...
!       "bsplgen" gets B-spline depth 2 coefficients and derivatives for
!       a single PME atomic site along a particular direction
!        ( Optimized for partial charge grid interpolation on the fly )
...
!#if BSORDER_VALUE == 6
!        thetai(6) = temp(bsorder+bsorder*(6-1))
!#endif
!#if BSORDER_VALUE == 7
!        thetai(7) = temp(bsorder+bsorder*(7-1))
!#endif
!#if BSORDER_VALUE == 8
!        thetai(8) = temp(bsorder+bsorder*(8-1))
!#endif
!#if BSORDER_VALUE > 8
!#   warning(" bsorder value is not take under consideration")
!#endif
...
!       CUDA Fortran written kernel
!       Work Only in Sequential
!
!
...
do impi = blockIdx%x, npolerecloc, gridDim%x
   ...
   if (blockDim%x>warpsize) then; call syncthreads
   else;                          call syncwarp(all_lanes); end if
!
!       Three dimensional loop on the grid collapse by hand
!
   do kk = threadIdx%x-1, nlptsit, blockDim%x
...
integer  ,intent(in),value:: kstat,ked,jstat,jed,istat,ied
integer  ,intent(in),device::polerecglob(*),ipole(*),igrid(3,*)
!       real(t_p),intent(in),device::thetai1(4,order,nlocrec)
!    &           ,thetai2(4,order,nlocrec),thetai3(4,order,nlocrec)
real(t_p),device,dimension(10,npolerecloc)::fdip_phi1,fdip_phi2
