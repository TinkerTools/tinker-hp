...
subroutine dcinduce_shortrealgpu
   use atmlst
   use divcon
   use domdec
   use ewald
...
subroutine inducedc_shortrealgpu(matvec,nrhs,dodiis,ef,mu)
   use atmlst
   use divcon
   use domdec
   use ewald
...
subroutine pc_dc_tmatxb_shortrealgpu(nrhs,dodiag,mu,efi)
!
...
!
   ...
   integer ipoleloc,l
   real(t_p)  mu(3,nrhs,*), efi(3,nrhs,max(1,npolebloc))
   logical dodiag
...
subroutine otf_dc_tmatxb_shortrealgpu(nrhs,dodiag,mu,efi)
!
!     Compute the direct space contribution to the electric field due to the current value
!     of the induced dipoles
!
   ...
   real(t_p) rr3, rr5, dukx, duky, dukz, pukx, puky, pukz, puir,&
      &pukr, duir, dukr
   real(t_p) duix, duiy, duiz, puix, puiy, puiz
