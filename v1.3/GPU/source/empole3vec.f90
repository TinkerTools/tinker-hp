...
!     ########################################################################
!     ##                                                                    ##
!     ##  subroutine empole3cvec     --  Ewald multipole analysis via list  ##
!     ##                                                                    ##
!     ########################################################################
!
!
...
!deb     if(rank.eq.0.and.tinkerdebug)write(*,*)'npoleloc,npolelocloop',
!deb &             npoleloc,   npolelocloop
...
!     ###################################################################
!     ##                                                               ##
!     ##  subroutine emreal3dvec  --  real space mpole analysis via list  ##
!     ##                                                               ##
!     ###################################################################
!
!
...
!
!        mscalevec = setscale2(iglob, kglobvec1,neloop16,'mscale')
...
!DIR$ VECTOR ALIGNED
      ...
      enddo
!
!     calculate intermediate terms for multipole interaction
!
...
!DIR$ NOFUSION
      ...
      enddo
!
!     modify error function terms to account for scaling
!
      ...
      do k = 1, neloop16
         if (k.le.nnelst1) then
            em = em + term1vec(k) * rr1vec(k)&
               ...
               &+ term5vec(k) * rr9vec(k)
!
!             count interactions number with the undamped value
!
