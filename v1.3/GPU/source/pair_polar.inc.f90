...
!
!     t reciprocal distance terms for this interaction
!
invr2    = r2**(-1)
r        = f_sqrt(r2)
invr     = f_sqrt(invr2)
...
dsr7     = bn(3) - dsc7 * rr7
!
!     termediates involving moments and distance separation
!
...
!
!     compute the potential at each site for use in charge flux
!
if (u_cflx) then
...
!
!     t the dEd/dR terms used for direct polarization force
!
dterm1   =  bn(2)   -   dsc3*rr5
dterm2   =  bn(3)   -   dsc5*rr7
!
!     Straight terms ( xx, yy ,zz )
!
...
!
!     Crossed terms ( xy = yx , xz = zx ,yz = zy )
!
tmp1x    = pos%x * pos%y
tmp1y    = pos%x * pos%z
tmp1z    = pos%y * pos%z
...
!
!      Construct matrixes for dot_product
!      do Dot product
!
...
!
!     t the dEp/dR terms used for direct polarization force
!
...
!
!     t the dtau/dr terms used for mutual polarization force
!
...
!
!     t the induced dipole field used for dipole torques
!
...
!
!     t induced dipole field gradient used for quadrupole torques
!
...
M_subroutine&
   ...
   &aewald,alsq2,alsq2n,pgamma,damp,use_tholed,f,&
...
type(real3),intent(in):: pos
type(real6),intent(in):: dpuk,dpui
logical    ,intent(in):: do_correct,use_tholed
...
if (damp.ne.0.0_ti_p) then
   ...
   da       = damp1 * expdamp1
!
!     termediates involving Thole damping and scale factors
!
   sc3      = 1.0_ti_p - expdamp1
   sc5      = 1.0_ti_p - (1.0_ti_p - damp1)*expdamp1
   sc7      = 1.0_ti_p - (1.0_ti_p - damp1 + 0.6_ti_p*damp1**2)&
...
!        if (u_cflx) then
!           poti = poti+ kp%c*rr1_ - drk*rr3_ + qrrk*rr5_
!           potk = potk+ ip%c*rr1_ + dri*rr3_ + qrri*rr5_
!        end if
!
...
!
!     if (u_cflx) then
!        poti = kp%c*rr1 - drk*rr3 + qrrk*rr5 - urk*psr3 - urkp*dsr3
!        potk = ip%c*rr1 + dri*rr3 + qrri*rr5 + uri*psr3 + urip*dsr3
!     end if
