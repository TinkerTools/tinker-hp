...
!     ##################################################################
!     ##                                                              ##
!     ##  subroutine elj1vec  --  Lennard-Jones energy & derivatives  ##
!     ##                                                              ##
!     ##################################################################
!
!
...
!DIR$ VECTOR ALIGNED
      do k = 1, nvloop8
         ...
         evec(k)  = epsvec2(k) * p6vec(k) * (p6vec (k) - 2.0_ti_p)&
            &* vscalevec(k)
         devec(k) = - 12.0_ti_p * epsvec2(k) * p6vec(k)&
            &* (p6vec(k) - 1.0_ti_p)&
            &* invrikvec(k)&
            &* vscalevec(k)
