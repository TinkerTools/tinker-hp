...
!
!     "eliaison1" calculates all bonded terms in a single routine
!
...
!20   format('   ',13A9)
!30   format(A3,13I9)
!     write(*,20) "bond","urey","angle","tors","pitors","tortor"
!    &           ,"angtor","improp","imptor","opbend","stdbnd"
!    &           ,"def"
!     write(*,30) "n",nbondloc,nureyloc,nangleloc,ntorsloc,npitorsloc
!    &           ,ntortorloc,nangtorloc,niproploc,nitorsloc,nopbendloc
!    &           ,nstrbndloc
!     write(*,30) "off",off_bond,off_urey,off_angl,off_tors,off_pitr
!    &           ,off_totr,off_antr,off_impr,off_impt,off_opbd,off_stbd
!     write(*,30) "num",num_bond,num_urey,num_angl,num_tors,num_pitr
!    &           ,num_totr,num_antr,num_impr,num_impt,num_opbd,num_stbd
!    &           ,num_def
   end if

!$acc host_data use_device(bndglob,ibnd,bl,bk,ba,type,deb,aMDwattype &
!$acc         ,ureyglob,iury,ul,uk,deub &
!$acc         ,angleglob,angtypI,bndtypI,iang,anat,ak,afld,dea &
...
!$acc         ,ered_buf1,vred_buff)
   call eliaison1_kcu<<<lia_Gs,lia_Bd,0,rec_stream>>>&
      &(&!bond
      &bndglob,ibnd,bl,bk,ba,type,nbondloc,bndunit,cbnd,qbnd,bndtypI&
      &,use_polymer,deb&
      &,off_bond,num_bond&
      &&!urey
...
!      block;  type(dim3) gS,bS;
!      gS  = dim3((siz-1)/TRP_BLOCK_DIM+1,1,1)
!      bS  = dim3(TRP_BLOCK_DIM,3,1)
!!$acc host_data use_device(deb)
!      call transpose_z3m<<<gS,bS,0,rec_stream>>>( grx,deb,siz )
!!$acc end host_data
!      call check_launch_kernel(" transpose_z3f ")
!      end block
