...
!     vblst       site numbers block in neighbor list of each vdw site block
!     ivblst      first block pair of neighbor list C2
!     vb_tag      tag on each vdw block atoms  (to be use in mpi midpointimage)
!     nelst       number of sites in list for each electrostatic site
!     nelstc      number of sites under the cutoff in list for each electrostatic site
!     nshortelstc      number of sites under the cutoff in short range list for each electrostatic site
...
!     cell_len1   number of atoms in each cell, cell-list method when cells are small enough
!     cell_lenr   number of atoms in each cell, use in couple to cell_len for cell spliting
!     cell_len2   number of atoms in each cell, a partner to cell_len
!     cell_len2r   number of atoms in each cell, a partner to cell_len
!     cell_scan   partial sum of cell_len
...
!
!     cell*_order cell numbering array
!     cell*_glob    local atoms index following cell numbering
!     cell*_x      x position atoms renumber from cell_reorder
!     cell*_y      y position atoms renumber from cell_reorder
!     cell*_z      z position atoms renumber from cell_reorder
!
!
!     nx_cell     number of boxes in x dimension
!     ny_cell     number of boxes in y dimension
!     nz_cell     number of boxes in z dimension
...
!     auto_lst    logical flag to switch from automatic to manual neighbor list construction
!     rebuild_lst logical flag to trigger neighbor list reconstruction
...
!
!     matb_lst    adjacency matrix of pairwise blocks interactions
!     buffMatb    adjacency matrix maximum storage size (Go)
!     szoMatb     sizeof(matb_lst)
!     szMatb      number of element in matb_lst's line
!     nbMatb      number of blocks stored by matb_lst
!     niterMatb   number of iteration required to build Adjacency list
!     offSet[l][r]Mb  BlockId offset left and right stored by Matb
!
!     vlst_enable  logical flag to control vlst  construct
!     vlst2_enable logical flag to control vblst construct
!     mlst_enable  logical flag to control elst  construct
!     mlst2_enable logical flag to control eblst construct
!
!     boxPart contains box partitionning information
!     pair_atlst  a derived type that gather all attributes of a pairwise
!                neighbor list stored in a list structure
!
!
...
module neigh
   ...
   integer ineigup
   integer ncell_tot,ncell_nl,max_cell_len,max_numneigcell
   ...
   type(pair_atlst),target:: list_mlpot
...
!DIR$ ATTRIBUTES ALIGN:64:: xendcell,yendcell,zendcell
   ...
   interface
      subroutine build_cell_list2(ineignl,cell_order,nlocnl,buf,bP)
         import boxPart
...
contains
   ...
   subroutine init_boxPart
      ...
      c_bP%ny_l=.true.
      c_bP%nz_l=.true.
      call init_aboxPart(list_mlpot%bPar)
