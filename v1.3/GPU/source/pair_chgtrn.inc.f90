...
M_subroutine duo_chgtrn&
    ...
    ,rinv,off,ctrnscut,cut2,sheal,ctrntyp,u_grp,u_lamdyn,mutik,elambda,delambdae,e,frc&
    ,ver,fea)
...
logical    ,intent(in):: u_grp,u_lamdyn
real(t_p)  ,intent(in):: r2,xr,yr,zr,chgi,chgk,alphai,alphak,fgrp&
    ,f,scal,off,ctrnscut,cut2,sheal,rinv,elambda
real(t_p)  ,intent(inout):: delambdae,e
type(real3),intent(out):: frc

integer(1) one1
integer    grd,ene,sca,grp,shr,lgr,sfc
real(t_p)  rik,rr1,expi,expk,de,chgik,alphaik
