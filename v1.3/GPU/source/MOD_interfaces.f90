...
!     #################################################################################
!     ##                                                                             ##
!     ##  module interfaces  -- Tinker-HP interfaces functions and subroutines       ##
!     ##                                                                             ##
!     #################################################################################
!
...
!  #############################################################################
   ...
   interface
      ...
      subroutine C_get_cell(xcell,ycell,zcell,eps_cell&
          ,beta_sin,beta_cos,beta_term,gamma_sin,gamma_cos,gamma_term&
          ,monoclinic,triclinic,octahedron,box34) bind(C,name="C_get_cell")
         real(t_p),intent(in),value::xcell,ycell,zcell,eps_cell&
                  ,beta_sin,beta_cos,beta_term,gamma_sin,gamma_cos,gamma_term,box34
         integer  ,intent(in),value::monoclinic,triclinic,octahedron
      end subroutine
      ...
      integer(c_int) function solve_pguess_sls( m,n,AQ,B,Sig,Vt,z,stream )&
         bind(C,name="solve_polguess_svd_ls")
         import cuda_stream_kind,c_int
         integer(c_int),value:: m,n
         real(t_p),device::  AQ(*),B(*),Sig(*),Vt(*),z(*)
         integer(cuda_stream_kind),value:: stream
      end function
