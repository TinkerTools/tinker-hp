...
!
!     set radmin epsilon rule optimisation
!
      radepsOpt_l = (radrule_i.eq.ARITHMETIC_RL.or.&
         ...
         &.and..not.vdwpr_l
!
!     remove zero-sized atoms from the list of local vdw sites
!
      nvdw = 0
      do i = 1, n
...
!
!     Count number of system class
!     and renumber them from one
!
   nvdwclass = 1
...
subroutine kvdw_reset(istep)
   ...
   use domdec ,only : glob,pbig_recep,domlen,nlocnl,repart,rank,&
      &nbloc,nloc,bufbeg,nbig_recep
   use kvdws  ,only : rad
...
!$acc data present(vdwglob,glob,nbvdw,rad,jvdw) present(nvdwbloc,nvdwloc)&
!$acc      async(rec_queue)
!
...
!
!$acc parallel loop async(rec_queue) &
!$acc present(ineignl,nbvdw,rad,jvdw,vdwglobnl,vdwlocnl,nvdwlocnl)
   do i = 1, nlocnl
      iglob    = ineignl(i)
      vdwcount = nbvdw(iglob)
      if (rad(jvdw(iglob)) .ne. 0) then
...
!
!     Reconfigure vdw glob on host and send them on device
!
...
!  nvdwblocloop = merge( nvdwbloc,(int(nvdwbloc/16)+1)*16,(mod(nvdwbloc,16).eq.0))
...
!
!     subroutine alloc_shared_vdw : allocate shared memory pointers for vdw
!     parameter arrays
!
