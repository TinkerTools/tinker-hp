...
!     ######################################################################
!     ##                                                                  ##
!     ##  subroutine baoarespabpi  --  BAOAB-respa Langevin PIMD step     ##
!     ##                                                                  ##
!     ######################################################################
!
!
!     "baoabrespapi" performs a single PIMD time step
...
!$acc enter data create(eslow,vir_ctr,dip,dipind) async
   ...
   !! B SLOW !!
   call apply_B_PI(polymer_main,dt2)
   !! LOAD PREVIOUS FAST FORCES ON EIGFORCES !!
   call set_eigforces_pi(polymer_main,polymer_main%forces)

   if(isobaric) then
      call plangevinpi(polymer_main,dt2,istep,'A')
      call plangevinpi(polymer_main,dt,istep,'O')
   endif
   ...
   if(isobaric) call plangevinpi(polymer_main,dt2,istep,'A')
   if(PITIMER) call stopwatchpi(timeaoa,.false.,.false.)

   call full_gradient_prepare_pi(polymer_main,polymer_ctr,istep,.true.)
...
!$acc parallel loop collapse(3) async default(present)
      DO k=1,nbeads; DO iloc=1,nloc ; DO j=1,3
               i=glob(iloc)
               polymer_main%forces_slow(j,i,k) =&
      ...
      ibeadbeg=polymer_main%ibead_beg(rank_polymer+1)
!$acc parallel loop collapse(3) async default(present)
      DO k=1,nbeads; DO iloc=1,nloc ; DO j=1,3
               i=glob(iloc)
               polymer_main%forces_slow(j,i,k) = 0._re_p
            ENDDO ; ENDDO ; ENDDO
   ...
   if(.not. contract) then
      ...
      call compute_grad_beads(eslow,derivs,vir_ctr,polymer_main&
...
!$acc parallel loop collapse(3) async default(present)
      DO k=1,nbeadsloc; DO iloc=1,nloc ; DO j=1,3
               i=glob(iloc)
               polymer_main%forces_slow(j,i,k+ibeadbeg-1) =&
                  &polymer_main%forces_slow(j,i,k+ibeadbeg-1)&
                  &-derivs(j,iloc,k)
            ENDDO ; ENDDO ; ENDDO

      call comm_for_normal_modes(polymer_main,polymer_main%forces_slow )
      if(PITIMER) call stopwatchpi(timecom,.true.,.false.)
   endif
...
!$acc end serial
      ...
      call project_forces_ctr(polymer_main,polymer_ctr,.TRUE.)
      if(PITIMER) call stopwatchpi(timecom,.true.,.false.)
   endif

   call set_eigforces_pi(polymer_main,polymer_main%forces_slow)
   call centroid_force_corrections(polymer_main)
   
   call apply_B_PI(polymer_main,dt2)
   if(PITIMER) call stopwatchpi(timeaoa,.false.,.false.)

   if (calc_e.or.use_virial) then
      call kinetic_pi(polymer_main)
      call reduce_observables_pi(polymer_main)
   endif
...
!$acc wait
   ...
   if(isobaric) call plangevinpi(polymer_main,dt,istep,'B')

   call mdstatpi(istep,dt)
   call mdsavebeads (istep,dt,polymer_main)
...
!$acc wait
      ...
      call apply_B_PI(polymer_main,dta2)
      call apply_AOA_PI(polymer_main,dta)
      if(PITIMER) call stopwatchpi(timeaoa,.false.,.false.)

      call fast_gradient_prepare_pi(polymer_main,istep)
      if(PITIMER) call stopwatchpi(timecom,.true.,.false.)

      !! GRADFAST
      call prmem_requestm(derivs,3,nbloc,nbeadsloc,async=.true.)
      call compute_grad_beads(epot,derivs,vir_ctr,polymer_main&
...
!$acc parallel loop collapse(3) async default(present)
      DO k=1,nbeadsloc; DO iloc=1,nloc ; DO j=1,3
               i=glob(iloc)
               polymer_main%forces(j,i,k+ibeadbeg-1) = -derivs(j,iloc,k)
            ENDDO ; ENDDO ; ENDDO

      call comm_for_normal_modes(polymer_main,polymer_main%forces)
      if(PITIMER) call stopwatchpi(timecom,.true.,.false.)
   ...
   if (calc_e .or. use_virial) then
      call kinetic_pi_fast(polymer_main)
   endif
