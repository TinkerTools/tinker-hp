...
!$acc enter data create(eslow,vir_ctr) async
   ...
   if(isobaric) then
      call plangevinpi(polymer_main,dt2,-1,'A')
      call plangevinpi(polymer_main,dt,-1,'O')
   endif
   call apply_AOA_PI(polymer_main,dt)
   if(isobaric) call plangevinpi(polymer_main,dt2,-1,'A')
   if(PITIMER) call stopwatchpi(timeaoa,.false.,.false.)

   call full_gradient_prepare_pi(polymer_main,polymer_ctr,istep,.false.)
   ...
   call compute_grad_beads(epotpi,derivs,virpi,polymer_main&
...
!$acc parallel loop collapse(3) async default(present)
      DO k=1,nbeadsloc; DO iloc=1,nloc ; DO j=1,3
               i=glob(iloc)
               polymer_main%forces(j,i,k+ibeadbeg-1) =&
                  &polymer_main%forces(j,i,k+ibeadbeg-1)&
...
!$acc end serial
      ...
      call project_forces_ctr(polymer_main,polymer_ctr)
      if(PITIMER) call stopwatchpi(timecom,.true.,.false.)
   endif
...
!$acc wait
   ...
   if(isobaric) then
      call plangevinpi(polymer_main,dt,-1,'B')
