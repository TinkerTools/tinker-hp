...
10    format (' System Linear Velocity :  ',3d12.2,&
         ...
         &' Kcal/mole')
       if (.not.use_bounds.and. ngrp .eq. 0) then
          write (iout,40)  (vang(j),j=1,3),erot
40        format ( ' System Angular Velocity : ',3d12.2,&
                 /,' Rotational Kinetic Energy :',13x,f12.4,' Kcal/mole')
       end if
   end if
!$acc update device(v)
   return
...
!
   ...
   if (ngrp.gt.0.and.ranktot.eq.0.and.tinkerdebug.gt.0) then
 11   format(' WARNING -- Remove Translational and Rotational Kinetic',&
             ' Energy for each group is actually impossible !')
      write(*,11)
   end if
