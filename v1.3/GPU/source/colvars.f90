...
!     #############################################################
!     ##                                                         ##
!     ##  colvars Routines  --  Colvars/Tinker-HP interface      ##
!     ##                                                         ##
!     #############################################################
...
!     get a copy of values rather than the same address (they don't
!hange during MD)
!
...
!12   format(A,I5,3F14.6)
!     print 12,'get_pos', atom_number,xr,yr,zr
end subroutine
...
!
!     the master gets all the cv positions and total forces
!
...
!$acc         copy(decv_tot,cv_pos) &
!$acc         present(x,y,z,de_tot,derivs,de_buffr &
...
!12   format(A,3f10.4)
!13   format(A,6f16.10)
!     print 13, 'pos ',cv_pos(1:6,1)
!     print 13, 'frc_',decv_tot(1:6,1)
   if (use_lambdadyn) then
      delambdae = delambdae + delambdaesave
      delambdaesave = 0d0
      delambdav = delambdav + delambdavsave
...
!
!     the master sends the new forces, he already has the bias
!
...
!12      format(A,6f16.10)
!        print 12, 'decv',decv(1:6,1)
!        print 12, 'de_t',de_tot(642*3+1:642*3+6,1)
   else
...
!      call minmaxone(decv    ,size(decv),'dcv  ')
!      call minmaxone(decv_tot,size(decv),'dcv_t')
!!$acc end data
end subroutine
...
subroutine colvars_init(dt)
   ...
   integer ierr,rank_,COMM_,nproc_

   if (app_id == pimd_a) then
      rank_ = ranktot
      nproc_ = nproctot
...
!
   ...
   if (nproc_.gt.1)&
      &call MPI_BCAST(use_colvars,1,MPI_LOGICAL,0,COMM_,ierr)
   if (use_colvars) then
!$acc wait
      if (nproc_.gt.1)&
         &call MPI_BCAST(ncvatoms,1,MPI_INT,0,COMM_,ierr)
      if (rank_.gt.0) then
         allocate (cvatoms_ids(ncvatoms))
      end if
      if (nproc_.gt.1) call MPI_BCAST&
         &(cvatoms_ids,ncvatoms,MPI_INT,0,COMM_,ierr)
...
!
!     retreive index of local replica
!
...
!
!     retreive root 2 root communicator
!
