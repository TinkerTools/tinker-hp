...
!     #################################################################
!     ##                                                             ##
!     ## subroutine epreal1cvec -- Ewald real space derivs via list  ##
!     ##                                                             ##
!     #################################################################
!
!
...
!        qivec  is qixx, qixy, qixz, qixy, qiyy, qiyz,qixz, qiyz, qizz
...
!
!        qkvec  is qkxx, qkxy, qkxz, etc  for all the sites
!    qkvec  1     2     3     4     5     6     7     8     9
!         qkxx, qkxy, qkxz, qkxy, qkyy, qkyz, qkxz, qkyz, qkzz
...
!
!           psr3vec(k) = rr3vec(k) * sc3vec(k) * pscalevec(k)
!           psr5vec(k) = rr5vec(k) * sc5vec(k) * pscalevec(k)
!           psr7vec(k) = rr7vec(k) * sc7vec(k) * pscalevec(k)
!           efullvec(k) =  term1vec(k) * psr3vec(k)
!    &                   + term2vec(k) * psr5vec(k)
!    &                   + term3vec(k) * psr7vec(k)
!        enddo
!DIR$ ASSUME (mod(neloop8,8).eq.0)
...
!
!       qipolvec  is qixx, qixy, qixz for all the npolelocnl
!        1     2     3     4     5     6     7     8     9
!       qixx, qixy, qixz, qixy, qiyy, qiyz, qixz, qiyz, qizz
