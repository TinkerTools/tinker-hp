...
!
   wtime0 = mpi_wtime()
!
!     compute the direct space contribution (fields)
!
...
!$acc parallel loop collapse(3) async(def_queue) default(present)
      do i = 1, npoleloc; do j = 1, nrhs;do k = 1, 3
         ...
         iglob  = ipole(iipole)
         if (mut(iglob)) then
            deflambda(k,j,i) = deflambda(k,j,i)+ term*rpole(k+1,iipole)/elambda
         end if
      end do; end do; end do
   end if
!
!     Reset predictor if MD State Tracking is enabled
!
...
!$acc parallel loop collapse(3) async(def_queue) default(present)
      do i= 1,npoleloc; do j= 1,nrhs; do k= 1,3
         iipole    = poleglob(i)
         mu(k,j,i) = merge(uind(k,iipole),uinp(k,iipole),btest(j,0))
      end do; end do; end do
   end if
   call commdirdirgpu(nrhs,1,mu,reqrec,reqsend)
   call commdirdirgpu(nrhs,2,mu,reqrec,reqsend)
!
   call commrecdirdipgpu(nrhs,0,murec,mu,buffermpimu1,buffermpimu2,req2rec,req2send)
   call commrecdirdipgpu(nrhs,1,murec,mu,buffermpimu1,buffermpimu2,req2rec,req2send)
   call commrecdirdipgpu(nrhs,2,murec,mu,buffermpimu1,buffermpimu2,req2rec,req2send)
!
...
subroutine pred_zero
   ...
   siz  = size(udpalt)
   call mem_set(udpalt,val,siz,rec_stream)
   if (allocated(upshortalt)) then
      siz  = size( udpalt1 )
      call mem_set(udpalt1,val,siz,rec_stream)
   end if
end subroutine
...
subroutine pred_InitField(mu)
   ...
   real(t_p),pointer:: bpred_p(:)
   integer i,ii,j,k,k1,iipole,ierr
   integer npoleloc_e
   integer calt,nual
#ifdef USE_NVSHMEM_CUDA
   integer ipe,ind
...
#endif
   if(deb_Path) print '(3x,A,2I3)',"pred_InitField",lshalt,lalt
...
#else
   if (use_polarshortreal) then
      udalt_p0(1:6,1:n,1:maxualt) => udpalt1(:)
      bpred_p => bpreds
      calt    =  lshalt-1
      nual    =  nualt1
      ...
      udalt_p0(1:6,1:n,1:maxualt) => udpalt(:)
      bpred_p => bpred
      calt    =  lalt-1
      nual    =  nualt
      npoleloc_e= npoleloc
   end if
!     write(*,'(I3,$)') (mod(calt+i-1,maxualt)+1,i=1,nual)
!     write(*,*)

!$acc parallel loop collapse(3) async(def_queue)&
!$acc          present(poleglob,bpred_p,udalt_p0,mu)
   do i=1,npoleloc_e; do ii=0,nrhs-1; do j=1,3
      iipole = poleglob(i)
      udsum  = 0.0_ti_p
      !$acc loop seq
      do k = 1, nual
         k1    = mod(calt+k-1,maxualt) + 1
         udsum = udsum + bpred_p(k)*udalt_p0(ii*3+j,iipole,k1)
      end do
      mu(j,ii+1,i) = udsum
   end do; end do; end do
#endif
...
subroutine pred_SetAlt
   ...
   use utilgpu   ,only: def_queue
   implicit none
   integer i,ii,j,k,iipole,ierr
   integer calt
#ifdef USE_NVSHMEM_CUDA
   integer ipe,ind,nual
#endif

   if (use_lambdadyn.and.elambda.gt.0.0) return
   !if (use_polarshortreal) return
...
!$acc parallel loop collapse(3) async(def_queue) &
!$acc     present(poleglob,uind,uinp,udalt_p0)
      do i= 1,npolebloc; do ii= 0,1; do j= 1,3
         iipole = poleglob(i)
         udalt_p0(ii*3+j,iipole,calt) = merge(uinp(j,i),uind(j,i),btest(ii,0))
      end do; end do; end do;
...
!
   if (nproc.gt.1) then
      call prmem_request(buffermpi1,3,nrhs,max(npoleloc,1),async=.true.)
      call prmem_request(buffermpimu1,3,nrhs,max(npoleloc,1),async=.true.)
      call prmem_request(buffermpi2,3,nrhs,max(npolerecloc,1),async=.true.)
      call prmem_request(buffermpimu2,3,nrhs,max(npolerecloc,1),async=.true.)
   end if

   call prmem_request(dipfield,3,nrhs,max(npoleloc,1),async=.true.)
   call prmem_request(dipfieldbis,3,nrhs,max(npolerecloc,1),async=.true.)
   call prmem_request(zr,3,nrhs,max(npoleloc,1),async=.true.)
   call prmem_request(res,3,nrhs,max(npoleloc,1),async=.true.)
   call prmem_request(h ,3,nrhs,max(npolebloc,1),async=.true.)
   call prmem_request(pp ,3,nrhs,max(npolebloc,1),async=.true.)
...
!
   ...
   if (.false.) then
!$acc update host(ggold1,ggold2)
!$acc wait
      gnorm(1)= sqrt(ggold1/real(3*npolar,r_p))
      gnorm(2)= sqrt(ggold2/real(3*npolar,r_p))
      resnrm  = max(gnorm(1),gnorm(2))
      it      = 0
      if (resnrm.lt.poleps) goto 10  !Guess already converged!
   end if
...
!
      ...
      skip_chk = (it.lt.2.or.(exitlast.and.(it.lt.lastIter))).and.polprt.eq.0
      ! Skip last Iteration check
      if (exitlast.and.it.eq.lastIter.and.mod(sameIter,3).ne.0.and..not.track_mds)&
         goto 10
!
      call commdirdirgpu(nrhs,1,pp,reqrec,reqsend)
      call commdirdirgpu(nrhs,0,pp,reqrec,reqsend)
...
10 continue
   ...
   if (app_id.eq.dynamic_a.or.app_id.eq.pimd_a) then
      ...
      exitlast = polprt.eq.0.and.sameIter.gt.25&
         .and.mod(indpcg_call,50).ne.0
      if (rank.eq.0.and.btest(tinkerdebug,6).ne.0) print*, 'pcg_conv iter',lastIter,sameIter,exitlast
   end if
...
!
!     MPI : begin direct space communication
!
...
!
!     Exchange data between direct and reciproqual space
!
...
#ifdef _OPENACC
   use interfaces,only: initcuSolver,cuGESVm
#endif
   use math
   use mpole
   use polar
...
!        call norm_field(res,6*npoleloc,its,ene1,ene2)
!
...
!        call minmaxone(Mres_s(1,1,1,1),6*npoleloc,'res0')
...
!$acc parallel loop collapse(3) async(rec_queue)
         ...
         call timer_exit ( timer_stepcg,quiet_timers )
!        if (its.ne.s)
!    &   call minmaxone(Mres_s(1,1,1,its+1),6*npoleloc,'res')
...
subroutine ulspredgpu
   ...
   use inform     ,only: deb_Path
#ifdef _OPENACC
   use interfaces ,only: tmatxb_gen,solve_pguess_sls,initcuSolver
#endif
   use mpole
   use polar_temp ,only: ef
   ...
   real(t_p),allocatable:: Sig(:), Vt(:)
   real(t_p), pointer:: X(:,:,:), h(:,:,:)
   parameter(zero=0.0_ti_p)

   nual = merge(nualt1,nualt,use_polarshortreal)
...
!$acc parallel loop vector_length(32) async(def_queue) default(present)
      do i = 1, nual
         coeff      = gear(i)
         bpred(i)   = coeff
         bpredp(i)  = coeff
...
!
      ...
      if (maxualt.eq.2) then
         bpred (1)= 1.3657012; bpred (2)= 1 -bpred (1)
         bpreds(1)= 1.6354331; bpreds(2)= 1 -bpreds(1)
!$acc update device(bpred,bpreds) async(def_queue)
      else if (maxualt.eq.5) then
         bpred(1)= 2.9570718; bpred(2)=-4.1849575;  bpred(3)= 3.9545205;
         bpred(4)=-2.3746951; bpred(5)= 0.6469386;
         bpreds  = bpred
!$acc update device(bpred,bpreds) async(def_queue)
      else
!$acc parallel loop vector_length(32) async(def_queue) default(present)
   ...
   else if (polpred.eq.'SVLS') then
#ifdef _OPENACC
      if ((.not.use_polarshortreal.and.(mod(Isvls, 200*maxualt).ne.0.or.Isvls .gt.1010*maxualt)) .or. &
          (     use_polarshortreal.and.(mod(Isvlss,200*maxualt).ne.0.or.Isvlss.gt.1010*maxualt))) goto 20

      call prmem_request(Audp, size(udpalt))
      allocate(  Vt(maxualt**2))
      allocate( Sig(maxualt))
      Sig= 0
      !$acc enter data copyin(Sig) create(Vt)

      if (use_polarshortreal) then
         do i = 0, maxualt-1
            X(1:3,1:2,1:n) => udpalt1(6*n*i+1:)
            h(1:3,1:2,1:n) => Audp   (6*n*i+1:)
            call tmatxb_gen( X, h, use_polarshortreal )
         end do
!$acc host_data use_device(Audp,ef,Sig,Vt,bpredps)
         iwh = solve_pguess_sls(6*n,maxualt,Audp,ef,Sig,Vt,bpredps,rec_stream)
!$acc end host_data
!    ------  Shift bpred
!$acc parallel loop default(present)
         do i = 1,maxualt
            bpreds(i) = bpredps(mod(lshalt+i-2,maxualt)+1)
         end do
      else
         do i = 0, maxualt-1
            X(1:3,1:2,1:n) => udpalt (6*n*i+1:)
            h(1:3,1:2,1:n) => Audp   (6*n*i+1:)
            call tmatxb_gen( X, h, use_polarshortreal )
         end do
!$acc host_data use_device(Audp,ef,Sig,Vt,bpredp)
         iwh = solve_pguess_sls(6*n,maxualt,Audp,ef,Sig,Vt, bpredp,rec_stream)
!$acc end host_data
!    ------  Shift bpred
!$acc parallel loop default(present)
         do i = 1,maxualt
            bpred(i) = bpredp(mod(lalt+i-2,maxualt)+1)
         end do
      end if

!$acc update host(bpred,bpreds)
      !if ( rank.eq.0.and.btest(tinkerdebug,6) ) then
      !   write(*,'(A,$)') '   Shift Least Square'
      !   write(*,'(F9.4,$)') (bpred(i),i=1,maxualt)
      !   write(*,*)
      !end if


!$acc exit data delete(Vt,Sig)
      deallocate(Sig,Vt)
