...
c
     ...
     $    MPI_COMM_WORLD,ierr)
c
         call commrattle(update,displace,dt)
         do i = 1, nbloc
            moved(i) = update(i)
            update(i) = .false.
         end do
...
c
     ...
     $    MPI_COMM_WORLD,ierr)
c
         call commrattle2(update,displace)
         do i = 1, nbloc
            moved(i) = update(i)
            update(i) = .false.
         end do
...
c
     ...
     $   pneig_send(i),tag,MPI_COMM_WORLD,reqrec(i),ierr)
        end if
      end do
      do i = 1, nneig_recep
     ...
     $     pneig_recep(i),tag,MPI_COMM_WORLD,reqsend(i),ierr)
        end if
      end do
c
      do i = 1, nneig_send
...
c
     ...
     $     MPI_INT,pneig_send(i),tag,MPI_COMM_WORLD,reqrec(i),ierr)
        end if
      end do
      do i = 1, nneig_recep
        if (pneig_recep(i).ne.rank) then
...
c
     ...
     $     9*buflenrat1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
     $     reqsend(i),ierr)
        end if
      end do
c
...
c
c     subroutine commrattle : deal with necessary comms between rattle iterations
c
      subroutine commrattle(update,displace,dt)
      use atmlst
      use atoms
      use domdec
      use freeze
      ...
      integer ialoc,ibloc
      integer tag,status(MPI_STATUS_SIZE)
      integer count,count1,ia,ib,iproc
      integer, allocatable :: reqrec(:),reqsend(:)
      logical :: update(nbloc)
      real*8 :: displace(3,nbloc)
      real*8 :: dt
      real*8, allocatable :: buffer(:,:),buffers(:,:)
      integer, allocatable :: buflenrat(:),bufbegrat3(:),buf(:)
      logical, allocatable :: done(:)
      logical neigbor
c
      allocate (reqsend(nproc))
      allocate (reqrec(nproc))
      allocate (buflenrat(nproc))
      allocate (bufbegrat3(nproc))
      bufbegrat3 = 0
      allocate (buf(max(1,3*nbloc)))
      allocate (buffer(4,max(3*nbloc,1)))
      buffer = 0d0
      allocate (buffers(4,max(3*nbloc,1)))
      buffers = 0d0
      allocate (done(nbloc))
      done = .false.
c
c     Begin the reception of the constrained atoms
c     first, size of the buffers
c
      count = 0
      do iloc = 1, nratloc
        i = ratglob(iloc)
        ...
        ib = irat(2,i)
        ibloc = loc(ib)
        if ((update(ialoc)).and.(.not.(done(ialoc)))) then
          count = count + 1
          buf(count) = ia
          done(ialoc) = .true.
        end if
        if ((update(ibloc)).and.(.not.(done(ibloc)))) then
          count = count + 1
          buf(count) = ib
          done(ibloc) = .true.
        end if
      end do
      buflenrat(rank+1) = count
c
c     send and receive size of the buffers
c
       do i = 1, nneig_send
         tag = nproc*rank + pneig_send(i) + 1
         call MPI_IRECV(buflenrat(pneig_send(i)+1),1,MPI_INT,
     $  pneig_send(i),tag,MPI_COMM_WORLD,reqrec(i),ierr)
      end do
      do i = 1, nneig_recep
        tag = nproc*pneig_recep(i) + rank + 1
        call MPI_ISEND(count,1,MPI_INT,
     $   pneig_recep(i),tag,MPI_COMM_WORLD,reqsend(i),ierr)
      end do
c
      do i = 1, nneig_send
...
c
      bufbegrat3(rank+1) = 1
      count1 = buflenrat(rank+1)
      do iproc = 1, nneig_send
        if (buflenrat(pneig_send(iproc)+1).ne.0) then
          bufbegrat3(pneig_send(iproc)+1) = count1 + 1
        else
          bufbegrat3(pneig_send(iproc)+1) = 1
        end if
        count1 = count1 + buflenrat(pneig_send(iproc)+1)
      end do
c
c     send and receive list of corresponding indexes + displacement
c
      do i = 1, nneig_send
        tag = nproc*rank + pneig_send(i) + 1
        call MPI_IRECV(buffer(1,bufbegrat3(pneig_send(i)+1)),
     $   4*buflenrat(pneig_send(i)+1),
     $   MPI_REAL8,pneig_send(i),tag,MPI_COMM_WORLD,reqrec(i),ierr)
      end do
      do i = 1, count
        buffers(1,i) = buf(i)
        buffers(2,i) = displace(1,loc(buf(i))) 
        buffers(3,i) = displace(2,loc(buf(i)))
        buffers(4,i) = displace(3,loc(buf(i)))
      end do
      do i = 1, nneig_recep
        tag = nproc*pneig_recep(i) + rank + 1
        call MPI_ISEND(buffers,4*count,MPI_REAL8,pneig_recep(i),tag,
     $   MPI_COMM_WORLD,reqsend(i),ierr)
      end do
c
      do i = 1, nneig_send
...
c
      ...
      do i = 1, nneig_send
        proc = pneig_send(i)
        do j = 0, buflenrat(proc+1)-1
          iglob = int(buffer(1,bufbegrat3(proc+1)+j))
c
c     dont take into account atoms that are too far
c
          if (loc(iglob).eq.0) cycle
          if (loc(iglob).gt.nbloc) cycle
c
          iloc = loc(iglob)
          displace(1,iloc) = displace(1,iloc)+
     $       buffer(2,bufbegrat3(proc+1)+j)
          displace(2,iloc) = displace(2,iloc)+
     $       buffer(3,bufbegrat3(proc+1)+j)
          displace(3,iloc) = displace(3,iloc)+
     $       buffer(4,bufbegrat3(proc+1)+j)
        end do
      end do
c
      do i = 1, nneig_send
...
c
          ...
          if (loc(iglob).gt.nbloc) cycle
          iloc = loc(iglob)
          x(iglob) = x(iglob) + displace(1,iloc)
          y(iglob) = y(iglob) + displace(2,iloc)
          z(iglob) = z(iglob) + displace(3,iloc)
          v(1,iglob) = v(1,iglob) + displace(1,iloc)/dt
          v(2,iglob) = v(2,iglob) + displace(2,iloc)/dt
          v(3,iglob) = v(3,iglob) + displace(3,iloc)/dt
        end do
      end do
c
      deallocate (buflenrat)
      deallocate (bufbegrat3)
      deallocate (buf)
      deallocate (buffer)
      deallocate (buffers)
      deallocate (reqsend)
...
c
c     subroutine commrattle2 : deal with necessary comms between rattle2 iterations
c
      subroutine commrattle2(update,displace)
      use atmlst
      use atoms
      use domdec
      use freeze
...
c
          ...
          if (loc(iglob).gt.nbloc) cycle
          iloc = loc(iglob)
          v(1,iglob) = v(1,iglob) + displace(1,iloc)
          v(2,iglob) = v(2,iglob) + displace(2,iloc)
          v(3,iglob) = v(3,iglob) + displace(3,iloc)
        end do
      end do
c
      deallocate (buflenrat)
