...
c
      if(precomp .eq. 1) then
        if (rank.le.ndir-1) then
C          print*,rank,"dir time",ctime
          call inducedc_pme(pc_dc_tmatxb_pme,nrhs,.true.,ef,mu,xx)
        else
C          print*,rank,"rec time",ctime
...
c
     ...
     $   bmat(:,:),bloc(:,:), cex(:)
      ...
      real*8  dtime0,dtime1,ctime,omp_get_wtime
      save    zero, one, xx
...
cc
cc     MPI : begin sending to other direct processes
cc
c          do i = 1, n_send1
c            tag = nproc*p_send1(i) + rank + 1
c            call MPI_ISEND(munew,3*nrhs*npoleloc,MPI_REAL8,p_send1(i),
c     $       tag,MPI_COMM_WORLD,reqsend(i),ierr)
c          end do
cc
cc     MPI : begin sending to PME processes
cc
c          do i = 1, nrecdir_send1
c            proc = precdir_send1(i)
c            tag = nproc*proc + rank + 1
c            do j = 0, buflen1(proc+1)-1
c              call amove(3*nrhs,munew(1,1,poleloc(
c     $          buf1(bufbeg1(proc+1)+j))),
c     $          buffermpimu(1,1,bufbeg1(proc+1)+j))
c            end do
c            call MPI_ISEND(buffermpimu(1,1,bufbeg1(proc+1)),3*nrhs*
c     $      buflen1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
c     $      req2send(i),ierr)
c          end do
cc
c          time0 = mpi_wtime()
...
C        print*,"iter",it,"rank",rank,"dir time",ctime
        else
          call commrecdirdip(nrhs,2,murec,munew,buffermpimu,
     $    buffermpimu,req2rec,req2send)
c           do i = 1, nrecdir_recep1
c             proc = precdir_recep1(i)
c            call MPI_WAIT(req2rec(i),status,ierr)
c            do j = 0, buflen2(proc+1)-1
c              call amove(3*nrhs,buffermpimu(1,1,bufbeg2(proc+1)+j),
c     $          murec(1,1,polerecloc(buf2(bufbeg2(proc+1)+j))))
c            end do
c          end do
C        print*,"iter",it,"rank",rank,"rec time",ctime
        end if
c
