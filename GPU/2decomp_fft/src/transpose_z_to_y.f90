...
#ifdef _OPENACC
  subroutine cutranspose_z_to_y_complex(src, dst, opt_decomp)
...
#ifdef EVEN
    if (decomp%even) then
!$acc host_data use_device(src,work2_c) 
       call MPI_ALLTOALL(src, decomp%z2count, &
            complex_type, work2_c, decomp%y2count, &
            complex_type, DECOMP_2D_COMM_ROW, ierror)
!$acc end host_data
...
!$acc end host_data
    ...
    if (nproc.ne.1) then
    call Decomp2d_MPI_ALLTOALLV(src, decomp%z2cnts, decomp%z2disp,complex_type, &
                            work2_c, decomp%y2cnts, decomp%y2disp,complex_type, &
...
!   !$acc end host_data
    ...
    return
  end subroutine cutranspose_z_to_y_complex
#endif
...
!$acc parallel loop collapse(3) async(rec_queue)
       do k=i1,i2
          do j=1,n2
             do i=1,n1
                out(pos+(i-1)+(j-1)*n1+(k-i1)*n1*n2) = in(i,j,k)
             end do
          end do
       end do
    end do
!$acc end data
  end subroutine cumem_split_zy_complex
...
!$acc parallel loop num_gangs(n3*n1*(i2-i1+1)/(128*4)) collapse(3) async(rec_queue)
       do k=1,n3
          do j=i1,i2
             do i=1,n1
                out(i,j,k) = in(pos+(i-1)+n1*(j-i1)+n1*(i2-i1+1)*(k-1))
             end do
          end do
       end do
    end do
!$acc end data
  ...
  end subroutine cumem_merge_zy_complex
