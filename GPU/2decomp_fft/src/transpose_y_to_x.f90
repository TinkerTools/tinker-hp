...
#ifdef _OPENACC
  subroutine cutranspose_y_to_x_complex(src, dst, opt_decomp)
    ...
    integer :: ierror
    !print*, 'trans_yx'
!$acc data present(src,dst,work1_c,work2_c)
    if (present(opt_decomp)) then
       decomp = opt_decomp
    else
       decomp = decomp_main
...
#ifdef SHM
    work1_p = decomp%COL_INFO%SND_P_c
    call cumem_split_yx_complex(src, s1, s2, s3, work1, dims(1), &
         decomp%y1dist, decomp)
#else
    call cumem_split_yx_complex(src, s1, s2, s3, work1_c, dims(1), &
         decomp%y1dist, decomp)
#endif
...
#ifdef SHM
    call MPI_BARRIER(decomp%COL_INFO%CORE_COMM, ierror)
    call cumem_merge_yx_complex(work2, d1, d2, d3, dst, dims(1), &
         decomp%x1dist, decomp)
#else
    call cumem_merge_yx_complex(work2_c, d1, d2, d3, dst, dims(1), &
         decomp%x1dist, decomp)
    !print*,'all_to_allv_yx'
#endif
!$acc end data

    return
  end subroutine cutranspose_y_to_x_complex
#endif
  ...
  subroutine cumem_split_yx_complex(in,n1,n2,n3,out,iproc,dist,decomp)
...
!$acc end data
    return
  end subroutine cumem_split_yx_complex
  ...
  subroutine cumem_merge_yx_complex(in,n1,n2,n3,out,iproc,dist,decomp)
...
!$acc parallel loop collapse(3) async(rec_queue)
       do k=1,n3
          do j=1,n2
             do i=i1,i2
                out(i,j,k) = in(pos+(i-i1)+(j-1)*(i2-i1+1)+(k-1)*(i2-i1+1)*n2)
             end do
          end do
       end do
    end do
!$acc end data
    return
  end subroutine cumem_merge_yx_complex
