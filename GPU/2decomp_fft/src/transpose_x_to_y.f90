...
#ifdef _OPENACC
  subroutine cutranspose_x_to_y_complex(src, dst, opt_decomp)
    ...
    integer :: s1,s2,s3,d1,d2,d3,x1count,y1count
    integer :: ierror
    !print*, 'trans_xy'

!$acc data present(work1_c,work2_c,src,dst)
...
#ifdef SHM
    work1_p = decomp%COL_INFO%SND_P_c
    call cumem_split_xy_complex(src, s1, s2, s3, work1, dims(1), &
         decomp%x1dist, decomp)
#else
    call cumem_split_xy_complex(src, s1, s2, s3, work1_c, dims(1), &
         decomp%x1dist, decomp)
#endif
...
#else
!$acc host_data use_device(work1_c,work2_c)
#ifdef EVEN
    call MPI_ALLTOALL(work1_c, x1count, &
         complex_type, work2_c, y1count, &
         complex_type, DECOMP_2D_COMM_COL, ierror)
#else
!$acc wait(rec_queue)
    call MPI_ALLTOALLV(work1_c, decomp%x1cnts, decomp%x1disp, &
...
#ifdef SHM
    call MPI_BARRIER(decomp%COL_INFO%CORE_COMM, ierror)
    call cumem_merge_xy_complex(work2, d1, d2, d3, dst, dims(1), &
         decomp%y1dist, decomp)
#else
    call cumem_merge_xy_complex(work2_c, d1, d2, d3, dst, dims(1), &
         decomp%y1dist, decomp)
    !print*,'all_to_allv_xy'
#endif
!$acc end data

    return
  end subroutine cutranspose_x_to_y_complex
#endif
...
#ifdef _OPENACC
  subroutine cumem_split_xy_complex(in,n1,n2,n3,out,iproc,dist,decomp)
...
!$acc parallel loop collapse(3) async(rec_queue)
       do k=1,n3
          do j=1,n2
             do i=i1,i2
                out(pos+(i-i1)+(j-1)*(i2-i1+1)+(k-1)*(i2-i1+1)*n2) = in(i,j,k)
             end do
          end do
       end do
    end do
!$acc end data
    return
  end subroutine cumem_split_xy_complex
#endif
  ...
  subroutine cumem_merge_xy_complex(in,n1,n2,n3,out,iproc,dist,decomp)
...
!$acc parallel loop collapse(3) async(rec_queue)
       do k=1,n3
          do j=i1,i2
             do i=1,n1
                out(i,j,k) = in(pos+(i-1)+(j-i1)*n1+(k-1)*n1*(i2-i1+1))
             end do
          end do
       end do
    end do
!$acc end data
  ...
  end subroutine cumem_merge_xy_complex
