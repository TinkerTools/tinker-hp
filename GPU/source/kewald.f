...
#include "tinker_precision.h"
      ...
      real(r_p) delta,edens,ddens
      real(t_p) rmax
      real(t_p) siz,slope
      real(t_p) fft1,fft2,fft3
 ...
 120  format('2d parallel FFT grid : ','Ngrid1 = ',I5,2x,'Ngrid2 = ',I5)
     ...
     &           600, 640, 648, 720, 750, 768, 800, 810, 864 /))
      ...
      nprocloc = merge(nrec,nproc,use_pmecore)
c
...
c
      if (use_polar) then
         apewald = aeewald
         siz     = min(xbox,ybox,zbox)
         if (siz .lt. 6.0_ti_p) then
            slope = (1.0_ti_p-apewald) / 2.0_ti_p
            apewald = apewald + slope*(6.0_ti_p-siz)
            minfft = 64
            if (verbose) then
               write (iout,10)
...
!$acc update device(bsorder,orthogonal,xbox,ybox,
!$acc& zbox,alpha,beta,gamma)
!$acc update device(nrpts,nlpts,grdoff)
!$acc update device(nfft1,nfft2,nfft3)
c
...
c
      call moduli
!$acc update device(bsmod1,bsmod2,bsmod3)
      n1mpimax = 0
      n2mpimax = 0
      n3mpimax = 0
      if (use_pmecore) then
...
!$acc update device(istart1,iend1,jstart1,jend1,kstart1,kend1,
!$acc$ istart2,iend2,isize2,jstart2,jend2,jsize2,kstart2,kend2,
!$acc$ ksize2,nfft1,nfft2,nfft3)
c
...
!$acc update device(n1mpimax,n2mpimax,n3mpimax)
c
        if (rank.gt.ndir-1) then
          call prmem_request(qfac_2d,isize2(rank_bis+1),
     $             jsize2(rank_bis+1),ksize2(rank_bis+1),
     &             config=ior(mhostacc,2**(memrealloc)))
        end if
      else
...
c         call MPI_BCAST(istart1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(iend1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(isize1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jstart1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jend1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jsize1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(kstart1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(kend1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(ksize1(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c
c         call MPI_BCAST(istart2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(iend2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(isize2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jstart2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jend2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(jsize2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(kstart2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(kend2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c         call MPI_BCAST(ksize2(iproc+1),1,MPI_INT,iproc,
c    $     COMM_TINKER,ierr)
c       end do

        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      istart1,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      iend1  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      isize1 ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jstart1,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jend1  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jsize1 ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      kstart1,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      kend1  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      ksize1 ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      istart2,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      iend2  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      isize2 ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jstart2,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jend2  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      jsize2 ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      kstart2,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      kend2  ,1,MPI_INT,COMM_TINKER,ierr)
        call MPI_AllGather (MPI_IN_PLACE,1,MPI_DATATYPE_NULL,
     $                      ksize2 ,1,MPI_INT,COMM_TINKER,ierr)
...
!$acc update device(n1mpimax,n2mpimax,n3mpimax)
c
        call prmem_request(qfac_2d,
     &           isize2(rank+1),jsize2(rank+1),ksize2(rank+1),
     &           config=ior(mhostacc,2**(memrealloc)))
      end if
...
c
      ...
      implicit none
      integer i,k
      real(t_p) alpha,cutoff,eps
      real(t_p) x,xlo,xhi,y
      real(t_p) ratio
c
...
c
      ...
      integer i
      real(t_p) x
      real(t_p) array(maxorder)
      real(t_p) bsarray(maxfft)
c
c
...
c
      do k = 3, n
         denom = 1.0_ti_p / real(k-1,t_p)
         c(k) = x * c(k-1) * denom
         do i = 1, k-2
            c(k-i) = ((x+real(i,t_p))*c(k-i-1)
     &                  + (real(k-i,t_p)-x)*c(k-i)) * denom
         end do
         c(1) = (1.0_ti_p-x) * c(1) * denom
      end do
      return
      end
c
...
c
      factor = 2.0_ti_p * pi / real(nfft,t_p)
      do i = 1, nfft
         sum1 = 0.0_ti_p
         sum2 = 0.0_ti_p
         do j = 1, nfft
            arg = factor * real((i-1)*(j-1),t_p)
...
c
      eps = 1.0d-7
      if (bsmod(1) .lt. eps)  bsmod(1) = 0.5_ti_p * bsmod(2)
      do i = 2, nfft-1
         if (bsmod(i) .lt. eps)
     &      bsmod(i) = 0.5_ti_p * (bsmod(i-1)+bsmod(i+1))
      end do
      if (bsmod(nfft) .lt. eps)  bsmod(nfft) = 0.5_ti_p * bsmod(nfft-1)
c
...
c
      ...
      do i = 1, nfft
         ...
         if (k .eq. 0) then
            ...
            sum1 = 1.0_ti_p
            sum2 = 1.0_ti_p
            factor = pi * real(k,t_p) / real(nfft,t_p)
            do j = 1, jcut
               arg = factor / (factor+pi*real(j,t_p))
               sum1 = sum1 + arg**order
