...
c
      do i = 1, nmut
         ...
         symbol(it) = 'HYB'
         atmnum(it) = 0
         weight(it) = lambda*weight(it1) + (one-lambda)*weight(it0)
         ligand(it) = 0
...
!$acc data present(type,atomic)
!$acc update device(type,atomic)
!$acc end data
      return
      end
c
...
c
            if (bl0 .eq. zero)  bl0 = bl1
            if (bl1 .eq. zero)  bl1 = bl0
            bk(i) = lambda*bk1 + (one-lambda)*bk0
            bl(i) = lambda*bl1 + (one-lambda)*bl0
            if (verbose) then
               if (header) then
                  header = .false.
                  write (iout,30)
...
c
            if (anat0 .eq. zero)  anat0 = anat1
            if (anat1 .eq. zero)  anat1 = anat0
            ak(i) = lambda*ak1 + (one-lambda)*ak0
            anat(i) = lambda*anat1 + (one-lambda)*anat0
            if (verbose) then
               if (header) then
                  header = .false.
                  write (iout,30)
...
!$acc data present(anat,ak)
!$acc update device(anat(:),ak(:))
!$acc end data
      return
      end
c
...
c
            if (used) then
               ...
               isb(2,k) = nba
               isb(3,k) = nbc
               sbk(1,k) = lambda*sbk1(1) + (one-lambda)*sbk0(1)
               sbk(2,k) = lambda*sbk1(2) + (one-lambda)*sbk0(2)
               if (verbose) then
                  if (header) then
                     header = .false.
                     write (iout,30)
...
c
      ...
      real(t_p) zero,one,two,six
      real(t_p) angle,symm
      real(t_p) v1_0,v2_0,v3_0
      real(t_p) s1_0,s2_0,s3_0
      real(t_p) v1_1,v2_1,v3_1
...
c
               if (used) then
                  ...
                  iitors(4,k) = id
   30             continue
                  if (s1_0 .eq. zero)  s1_0 = s1_1
                  if (s2_0 .eq. zero)  s2_0 = s2_1
                  if (s3_0 .eq. zero)  s3_0 = s3_1
                  if (s1_1 .eq. zero)  s1_1 = s1_0
                  if (s2_1 .eq. zero)  s2_1 = s2_0
                  if (s3_1 .eq. zero)  s3_1 = s3_0
                  itors1(1,k) = lambda*v1_1 + (one-lambda)*v1_0
                  itors1(2,k) = lambda*s1_1 + (one-lambda)*s1_0
                  angle = itors1(2,k) / radian
                  itors1(3,k) = cos(angle)
                  itors1(4,k) = sin(angle)
                  itors2(1,k) = lambda*v2_1 + (one-lambda)*v2_0
                  itors2(2,k) = lambda*s2_1 + (one-lambda)*s2_0
                  angle = itors2(2,k) / radian
                  itors2(3,k) = cos(angle)
                  itors2(4,k) = sin(angle)
                  itors3(1,k) = lambda*v3_1 + (one-lambda)*v3_0
                  itors3(2,k) = lambda*s3_1 + (one-lambda)*s3_0
...
c
      ...
      real(t_p) angle
      real(t_p) v1_0,v2_0,v3_0
      real(t_p) v4_0,v5_0,v6_0
      real(t_p) s1_0,s2_0,s3_0
      real(t_p) s4_0,s5_0,s6_0
      real(t_p) v1_1,v2_1,v3_1
      real(t_p) v4_1,v5_1,v6_1
      real(t_p) s1_1,s2_1,s3_1
      real(t_p) s4_1,s5_1,s6_1
      logical header
      character*4 pa,pb,pc,pd
      character*4 zeros
      character*16 pt,pt0
...
c
            ...
            if (s2_0 .eq. zero)  s2_0 = s2_1
            if (s3_0 .eq. zero)  s3_0 = s3_1
            if (s4_0 .eq. zero)  s4_0 = s4_1
            if (s5_0 .eq. zero)  s5_0 = s5_1
            if (s6_0 .eq. zero)  s6_0 = s6_1
            if (s1_1 .eq. zero)  s1_1 = s1_0
            if (s2_1 .eq. zero)  s2_1 = s2_0
            if (s3_1 .eq. zero)  s3_1 = s3_0
            if (s4_1 .eq. zero)  s4_1 = s4_0
            if (s5_1 .eq. zero)  s5_1 = s5_0
            if (s6_1 .eq. zero)  s6_1 = s6_0
            tors1(1,i) = lambda*v1_1 + (one-lambda)*v1_0
            tors1(2,i) = lambda*s1_1 + (one-lambda)*s1_0
            angle = tors1(2,i) / radian
            tors1(3,i) = cos(angle)
            tors1(4,i) = sin(angle)
            tors2(1,i) = lambda*v2_1 + (one-lambda)*v2_0
            tors2(2,i) = lambda*s2_1 + (one-lambda)*s2_0
            angle = tors2(2,i) / radian
            tors2(3,i) = cos(angle)
            tors2(4,i) = sin(angle)
            tors3(1,i) = lambda*v3_1 + (one-lambda)*v3_0
            tors3(2,i) = lambda*s3_1 + (one-lambda)*s3_0
            angle = tors3(2,i) / radian
            tors3(3,i) = cos(angle)
            tors3(4,i) = sin(angle)
            tors4(1,i) = lambda*v4_1 + (one-lambda)*v4_0
            tors4(2,i) = lambda*s4_1 + (one-lambda)*s4_0
            angle = tors4(2,i) / radian
            tors4(3,i) = cos(angle)
            tors4(4,i) = sin(angle)
            tors5(1,i) = lambda*v5_1 + (one-lambda)*v5_0
            tors5(2,i) = lambda*s5_1 + (one-lambda)*s5_0
            angle = tors5(2,i) / radian
            tors5(3,i) = cos(angle)
            tors5(4,i) = sin(angle)
            tors6(1,i) = lambda*v6_1 + (one-lambda)*v6_0
            tors6(2,i) = lambda*s6_1 + (one-lambda)*s6_0
...
c
      ...
      use tinheader
      use tors
      use utils,only:front_convert_base
      implicit none
      integer i,j,k,size
      integer ia,ib,ic,id
      integer ita,itb,itc,itd
      real(t_p) zero,one,two,six
      real(t_p) kst0(3),kst1(3)
      logical header
      integer pa,pb,pc,pd
      character*4 zeros
      integer*8 pt,pt0
...
c              pt = pa//pb//pc//pd
               ...
               call front_convert_base(0,0,itb,itc,0,pt0)
            else if (itc .lt. itb) then
c              pt = pd//pc//pb//pa
               call front_convert_base(0,itd,itc,itb,ita,pt)
               call front_convert_base(0,0,itc,itb,0,pt0)
            else if (ita .le. itd) then
c              pt = pa//pb//pc//pd
               call front_convert_base(0,ita,itb,itc,itd,pt)
               call front_convert_base(0,0,itb,itc,0,pt0)
...
c
            do j = 1, 3
               kst(j,i) = lambda*kst1(j) + (one-lambda)*kst0(j)
            end do
            if (kst(1,i).eq.zero .and. kst(2,i).eq.zero
     &                  .and. kst(3,i).eq.zero) then
...
!$acc data present(nstrtor,ist,kst)
!$acc update device(ist,kst,nstrtor)
!$acc end data
      return
      end
c
...
c
      do j = 1, nmut
         ...
         it0 = class0(j)
         it1 = class1(j)
         rad(it) = lambda*rad(it1) + (one-lambda)*rad(it0)
         eps(it) = lambda*eps(it1) + (one-lambda)*eps(it0)
      end do
c
...
c
   ...
   10          format (/,' Hybrid van der Waals Parameters :',
     &                 //,7x,'Atom Number    Radius     Epsilon')
            ...
            radius = rad(it)
            if (radsiz .eq. 'DIAMETER')  radius = two * radius
            if (radtyp .eq. 'SIGMA')  radius = radius / twosix
            write (iout,20)  i,radius,eps(it)
   20       format (6x,i8,f14.4,f12.4)
         end if
      end do
!$acc data present(rad,radmin,epsilon)
!$acc update device(rad,radmin,epsilon) async
!$acc end data
c
...
c
      ...
      do j = 1, nmut
         ...
         chg0 = chg(it0)
         chg1 = chg(it1)
         hybchg = lambda*chg1 + (one-lambda)*chg0
         do k = 1, nion
            if (iion(k) .eq. i) then
               used = .true.
               pchg(k) = hybchg
         ...
         if (chg0.ne.zero .or. chg1.ne.zero) then
            used = .true.
            nion = nion + 1
            iion(nion) = i
            kion(nion) = i
...
!$acc data present(iion,pchg)
!$acc update device(iion(:),pchg(:))
!$acc end data
      return
      end
