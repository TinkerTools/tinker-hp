...
#include "tinker_precision.h"
      ...
      M_subroutine
     &             groups2_inl (weigh,ia,ib,grplist,wgrp)
!$acc routine seq
      use sizes ,only: maxgrp
      implicit none
      integer  ,intent(in) :: ia,ib
      integer  ,intent(in) :: grplist(*)
      real(t_p),intent(in) :: wgrp(maxgrp+1,*)
      real(t_p),intent(out):: weigh
      weigh = wgrp(grplist(ia)+1,grplist(ib)+1)
      end subroutine

      subroutine groups_inl (weigh,ia,ib,ic,id,ie,ig,grplist,wgrp)
!$acc routine seq
      use sizes ,only: maxgrp
      implicit none
      integer,intent(in):: ia,ib,ic,id,ie,ig
      integer,intent(in):: grplist(*)
      real(t_p),intent(in) :: wgrp(0:maxgrp,0:maxgrp)
      real(t_p),intent(out):: weigh
