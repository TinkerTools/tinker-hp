...
c
     ...
     $     10*buflen1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
     $     reqrecdir(tag),ierr)
        end do
      else
c
...
c
      ...
      integer req(nproc*nproc), reqend(nproc*nproc)
...
c
      if (rank.le.ndir-1) then
        allocate (buffermpi1(3,max(npoleloc,1)))
        buffermpi1 = 0.0d0
        allocate (buffermpi2(3,max(npoleloc,1)))
        buffermpi2 = 0.0d0
        allocate (buffermpimu(3,nrhs,max(npoleloc,1)))
        buffermpimu = 0.0d0
        allocate (dipfield1(3,npoleloc))
        allocate (dipfield2(3,npoleloc))
        dipfield1 = 0.0d0
        dipfield2 = 0.0d0
        allocate (res(3,nrhs,npoleloc))
        allocate (zr(3,nrhs,npoleloc))
        allocate (diag(npoleloc))
        allocate (h(3,nrhs,npolebloc))
        allocate (pp(3,nrhs,npolebloc))
      else
        allocate (buffermpi1(3,npolerecloc))
        buffermpi1 = 0.0d0
        allocate (buffermpi2(3,npolerecloc))
        buffermpi2 = 0.0d0
        allocate (buffermpimu(3,nrhs,npolerecloc))
        buffermpimu = 0.0d0
        allocate (dipfield1bis(3,npolerecloc))
        allocate (dipfield2bis(3,npolerecloc))
        dipfield1bis = 0.d0
        dipfield2bis = 0.0d0
      end if
...
c
        do i = 1, nrecdir_send1
          proc  = precdir_send1(i)
          tag = nproc*rank + proc + 1
         call MPI_IRECV(buffermpi1(1,bufbeg1(proc+1)),3*buflen1(proc+1),
     $     MPI_REAL8,proc,tag,MPI_COMM_WORLD,reqrecdir1(tag),ierr)
         call MPI_IRECV(buffermpi2(1,bufbeg1(proc+1)),3*buflen1(proc+1),
...
c
        time0 = mpi_wtime()
        call tmatxbrecip(murec,nrhs,dipfield1bis,dipfield2bis)
        time1 = mpi_wtime()
     ...
     $        buffermpi1(1,bufbeg2(proc+1)+j))
            call amove(3,dipfield2bis(1,polerecloc(
     $        buf2(bufbeg2(proc+1)+j))),
     $        buffermpi2(1,bufbeg2(proc+1)+j))
          end do
          call MPI_ISEND(buffermpi1(1,bufbeg2(proc+1)),
     $      3*buflen2(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
     $      reqrecdir1(tag),ierr)
...
c
        do i = 1, nrecdir_send1
          ...
          call MPI_WAIT(reqrecdir2(tag),status,ierr)
          do j = 0, buflen1(proc+1)-1
            call amove(3,buffermpi1(1,bufbeg1(proc+1)+j),
     $        dipfield1(1,poleloc(buf1(bufbeg1(proc+1)+j))))
            call amove(3,buffermpi2(1,bufbeg1(proc+1)+j),
...
c
     ...
     $       3*buflen1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
     $       reqrecdir1(tag),ierr)
           call MPI_IRECV(buffermpi2(1,bufbeg1(proc+1)),
     $       3*buflen1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,
     $       reqrecdir2(tag),ierr)
          end do
c
...
c
      ...
      real*8, allocatable :: buffermpimu(:,:,:)
      integer ierr, tag, proc
      integer reqnorm,reqdiis(2*ndismx+1),req(nproc*nproc),
     $ req2(nproc*nproc)
      integer, allocatable :: reqrecdir1(:), reqrecdir2(:)
      integer status(MPI_STATUS_SIZE)
c
...
c
      if (rank.le.ndir-1) then
        ...
        if (dodiis) then
          nmat = 1
          lenb = ndismx + 1
          allocate (xdiis(3*nrhs*npoleloc,ndismx))
          allocate (ediis(3*nrhs*npoleloc,ndismx))
        ...
        allocate (buffermpi1(3,max(1,npolerecloc)))
        buffermpi1 = 0.0d0
        allocate (buffermpi2(3,max(1,npolerecloc)))
        buffermpi2 = 0.0d0
...
c
     ...
     $      buflen1(proc+1),MPI_REAL8,proc,tag,MPI_COMM_WORLD,req2(tag),
     $      ierr)
          end do
c
          time0 = mpi_wtime()
...
c
c$omp parallel do default(shared) private(i,ii,iglob,pdi,pti,ci,dix)
c$omp+ private(diy,diz,qixx,qixy,qixz,qiyy,qiyz,qizz,j,kkk,k,kglob,kbis)
c$omp+ private(dx,dy,dz,d2,d,ralpha,bn,alsq2,alsq2n,exp2a,bfac,d3)
...
c$omp+ private(dkr,qkx,qky,qkz,qkr,fim,fid,fip,iipole,kkpole)
...
c$omp+ private(ii,i,iipole,iglob,pdi,pti,kkk,kbis,kglob,dx,dy,dz,d2,d)
...
c$omp+ private(bfac,alsq2,alsq2n,fip,j,kkpole,kkpoleloc,inl,fkp)
c$omp+ private(ipoleloc)
c$omp+ reduction(+:efi)
c$omp+ schedule(dynamic) firstprivate(dscale)
      do ii = 1, npolelocnl
        iipole = poleglobnl(ii)
        iglob = ipole(iipole)
        i = loc(iglob)
...
c     zero out the PME charge grid
      call aclear(2*n1mpimax*n2mpimax*n3mpimax*(nrec_send+1),
     $ qgridin_2d)
c
...
c
     ...
     $   prec_send(i),tag,comm_rec,req(tag),ierr)
      end do
c
      do i = 1, nrec_recep
        tag = nrec*rank_bis + prec_recep(i) + 1
...
c
     ...
     $   2*n1mpimax*n2mpimax*n3mpimax,MPI_REAL8,
     $   prec_recep(i),tag,comm_rec,req(tag),ierr)
      end do
c
      do i = 1, nrec_send
        tag = nrec*rank_bis + prec_send(i) + 1
...
c     get field
c
c$omp parallel do default(shared) private(i,iipole,iglob)
      do i = 1, npolerecloc
        iipole = polerecglob(i)
        iglob = ipole(iipole)
...
c
c
      subroutine tmatxbrecip(murec,nrhs,dipfield1bis,dipfield2bis)
c
...
c     get fields
c
c$omp parallel do default(shared) private(i,iipole,iglob,k)
