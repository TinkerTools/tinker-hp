...
c
        ...
        if (associated(vdwlocnl)) deallocate(vdwlocnl)
        allocate (vdwlocnl(nvdw))
      end if
c
      if (associated(vdwglob)) deallocate(vdwglob)
...
c
      ...
      if (associated(vdwglobnl)) deallocate(vdwglobnl)
      allocate (vdwglobnl(nlocnl))
c
c      nvdwlocnl = 0
c      do i = 1, nlocnl
c        iglob = ineignl(i)
c        vdwcount = nbvdw(iglob)
c        if (rad(jvdw(iglob)) .ne. 0.0d0) then
c          nvdwlocnl = nvdwlocnl + 1
c          vdwglobnl(nvdwlocnl) = vdwcount + 1
c          vdwlocnl(vdwcount+1) = nvdwlocnl
c        end if
c      end do
      nvdwlocnl = 0
      do i = 1, nlocnl
...
c
      if (associated(jvdw)) deallocate (jvdw)
      if (associated(ivdw)) deallocate (ivdw)
      if (associated(ired)) deallocate (ired)
      if (associated(kred)) deallocate (kred)
      if (associated(ivt)) deallocate (ivt)
      if (associated(jvt)) deallocate (jvt)
      if (associated(radmin)) deallocate (radmin)
      if (associated(epsilon)) deallocate (epsilon)
      if (associated(radmin4)) deallocate (radmin4)
      if (associated(epsilon4)) deallocate (epsilon4)
      if (associated(radhbnd)) deallocate (radhbnd)
      if (associated(epshbnd)) deallocate (epshbnd)
      if (associated(nbvdw)) deallocate (nbvdw)
c
c     jvdw
c
      arrayshape=(/n/)
