...
c
      if (.not.associated(x)) allocate (x(n))
      if (.not.associated(y)) allocate (y(n))
      if (.not.associated(z)) allocate (z(n))
      if (.not.associated(xold)) allocate (xold(n))
      if (.not.associated(yold)) allocate (yold(n))
      if (.not.associated(zold)) allocate (zold(n))
      if (.not.associated(type)) allocate (type(n))
      if (.not.associated(i12)) allocate (i12(maxval,n))
