...
c      include 'pme.i'
c      include 'openmp.i'
c      integer i,ifr,impi,iglob
c      real*8 xi,yi,zi
c      real*8 w,fr,eps
cc
cc     offset used to shift sites off exact lattice bounds
cc
c      eps = 1.0d-8
cc
cc     get the B-spline coefficients for the i-th atomic site
cc
c      do i = 1, nlocrec
c        iglob = globrec(i)
c        xi = x(iglob)
c        yi = y(iglob)
c        zi = z(iglob)
c        w = xi*recip(1,1) + yi*recip(2,1) + zi*recip(3,1)
c        fr = dble(nfft1) * (w-anint(w)+0.5d0)
c        ifr = int(fr-eps)
c        w = fr - dble(ifr)
c        igrid(1,iglob) = ifr - bsorder
c        w = xi*recip(1,2) + yi*recip(2,2) + zi*recip(3,2)
c        fr = dble(nfft2) * (w-anint(w)+0.5d0)
c        ifr = int(fr-eps)
c        w = fr - dble(ifr)
c        igrid(2,iglob) = ifr - bsorder
c        w = xi*recip(1,3) + yi*recip(2,3) + zi*recip(3,3)
c        fr = dble(nfft3) * (w-anint(w)+0.5d0)
c        ifr = int(fr-eps)
c        w = fr - dble(ifr)
c        igrid(3,iglob) = ifr - bsorder
c      end do
c      return
c      end
c
...
c
      if (init) then
        ...
        precrecep = 0
c
        if (.not.associated(prec_send))
     $    allocate (prec_send(nproc))
        prec_send = 0
        if (.not.associated(prec_recep))
     $    allocate (prec_recep(nproc))
        prec_recep = 0
        if (.not.associated(repartrec)) allocate (repartrec(n))
        if (.not.associated(domlenrec)) allocate (domlenrec(nproc))
c
        precdir_recep = 0
        precdir_send = 0
        precdir_recep1 = 0
...
c
     ...
     $       ((j.ge.jstart1(iproc+1)).and.(j.le.jend1(iproc+1)))) then
        ...
        domlentemp = domlen
        call orderbuffer(.true.,domlentemp)
        call orderbufferrec
c
